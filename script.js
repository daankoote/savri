...
async function handleEvFormSubmit(event) {
    ...
    const chargerCountStr = form.querySelector('input[name="charger_count"]')?.value.trim();
    ...
    const installerRefRaw = form.querySelector('input[name="installer_ref"]')?.value.trim();
    ...
    if (!fullName || !email || !chargerCountStr || !eigenTerrein || !akkoord ) {
        showFormError(form, "voornaam, achternaam, e-mail, aantal laadpunten, eigen terrein en akkoord zijn verplicht.");
        return;
    }
    ...
    const payload = {
        ...
        annual_kwh_estimate: null,      // niet meer gebruikt
    ...
    const emailPayload = {
        ...
        "Je voorinschrijving is ontvangen.\n\n" +
        "We zullen weer contact opnemen zodra de Nea haar plannen en regulering heeft bekend gemaakt.\n\n" +
    ...
    if (emailErr) {
        console.error("Kon outbound email niet wegschrijven:", emailErr);
        // geen alert nodig, frontend blijft gewoon succesvol
    }
...
async function handleInstallateurFormSubmit(event) {
    ...
    const firstName = form.querySelector('input[name="klant_voornaam"]')?.value.trim();
    const lastName  = form.querySelector('input[name="klant_achternaam"]')?.value.trim();
    const fullName  = [firstName, lastName].filter(Boolean).join(" ");
    const chargerCountStr = form.querySelector('input[name="charger_count"]')?.value.trim();
    ...
    if (!installerRef || !fullName || !email || !akkoord) {
        showFormError(form, "Code, voornaam, achternaam, e-mail en akkoord zijn verplicht.");
        return;
    }
    ...
    if (chargerCountStr) {
        const n = parseInt(chargerCountStr, 10);
        if (Number.isNaN(n) || n < 0) {
            showFormError(form, "Aantal laadpunten moet groter of gelik aan nul zijn.");
            return;
        }
        chargerCount = n;
    }
...
async function handleInstallerSignup(event) {
    ...
    // email sturen met gegevens en de code zodat wanneer installateur code heeft gebruikt de email gevalideerd kan worden
    const emailPayload = {
        to_email: email,
