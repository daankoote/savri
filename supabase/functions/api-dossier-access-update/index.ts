...
import { insertAuditFailOpen, tryGetIdempotentResponse, storeIdempotentResponseFailOpen } from "../_shared/audit.ts";
import { authSession, auditSessionRejectFailOpen } from "../_shared/sessions.ts";
...
serve(async (req) => {
  ...
  // CORS preflight vóór idempotency
  if (req.method === "OPTIONS") return new Response("ok", { headers: corsHeadersFor(req) });
  if (req.method !== "POST") return json(req, 405, { ok: false, error: "Method not allowed" });

  // Idempotency verplicht voor write endpoint (zelfde contract als access-save)
  ...
  if (!r.ok) {
    await auditSessionRejectFailOpen(SB, dossier_id, meta, "access_update_rejected", r.reason, r.error);
  ...
  async function reject(
    ...
    await insertAuditFailOpen(
      SB,
      {
        dossier_id,
        actor_type: "customer",
        event_type: "access_update_rejected",
        event_data: {
          stage,
          status,
          message,
  ...
  if (!dossier) {
    return reject("db_read", 404, "Dossier niet gevonden.");
  }
  ...
  // -------------------- Input (PATCH-style) --------------------
  // Alleen velden die aanwezig zijn updaten (consistent met access-save)
