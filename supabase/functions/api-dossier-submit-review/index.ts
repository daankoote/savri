// supabase/functions/api-dossier-submit-review/index.ts
...
serve(async (req) => {
  ...
  // Idempotency required (writes status/lock + dossier_checks + audit)
  ...
  // Audit: submit requested (best-effort)
  await insertAuditFailOpen(
    SB,
    {
      dossier_id,
      actor_type: "customer",
      event_type: "dossier_submit_review_requested",
      event_data: {},
    },
    meta,
  ...
  if (!dossier) {
    await insertAuditFailOpen(
      SB,
      {
        dossier_id,
        actor_type: "customer",
        event_type: "dossier_submit_review_rejected",
  ...
  // Already locked / in review => idempotent OK
  if (dossier.locked_at || ["in_review", "ready_for_booking"].includes(String(dossier.status || ""))) {
    return finalize(200, {
      ok: true,
      dossier_id,
      status: dossier.status || "in_review",
      locked_at: dossier.locked_at || null,
      message: "Dossier is already locked / in review.",
    });
  }
  ...
  if (cErr) return finalize(500, { ok: false, error: `Consents read failed: ${cErr.message}` });
  if (chErr) return finalize(500, { ok: false, error: `Chargers read failed: ${chErr.message}` });
  if (docErr) return finalize(500, { ok: false, error: `Documents read failed: ${docErr.message}` });

  // Latest-consent reducer (deterministisch)
  ...
  // Docs per charger: ONLY confirmed
  ...
  if (upChkErr) return finalize(500, { ok: false, error: `Checks upsert failed: ${upChkErr.message}` });

  if (!allRequiredPass) {
    // keep / set incomplete
    const newStatus: DossierStatus = "incomplete";
    try {
      await SB.from("dossiers").update({ status: newStatus, updated_at: ts }).eq("id", dossier_id).is("locked_at", null);
    } catch (_e) {}
    ...
    await insertAuditFailOpen(
      SB,
      {
        dossier_id,
        actor_type: "system",
        event_type: "dossier_submit_review_rejected_incomplete",
  ...
  // Lock dossier for review
  const newStatus: DossierStatus = "in_review";

  const { data: lockedRow, error: lockErr } = await SB
    .from("dossiers")
  ...
  if (lockErr) return finalize(500, { ok: false, error: `Dossier lock failed: ${lockErr.message}` });

  // Double-check lock state (race-safe)
  ...
  if (!finalLockedAt || finalStatus !== "in_review") {
    ...
    if (fErr) return finalize(500, { ok: false, error: `Dossier re-read failed: ${fErr.message}` });

    if (fresh?.locked_at) {
      finalLockedAt = fresh.locked_at;
      finalStatus = String(fresh.status || "in_review");
    } else {
      return finalize(500, { ok: false, error: "Indienen lijkt gelukt, maar dossier is niet vergrendeld. Probeer opnieuw." });
    }
  }
  ...
  await insertAuditFailOpen(
    SB,
    {
      ...
      event_data: { new_status: "in_review", via: "api-dossier-submit-review" },
    },
    meta,
    { actor_ref, environment: ENVIRONMENT },
  );
