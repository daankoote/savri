...
-- REVIEW10 extends the existing private REVIEW07 projection only. Historical
-- canonical-signing-facts-v1 snapshots receive the explicit compatibility
-- fallback; v2 facts must carry a closed immutable reason when review is needed.

create or replace function public.app_evidence_review_case_detail_read_v1(
  p_auth_user_id uuid,
  p_case_ref text
)
returns jsonb
...
begin
  ...
  with latest_lifecycle as (
    ...
    where role_version.case_id = v_case_id
      ...
      and not exists (
        select 1 from public.app_case_party_roles successor
    ...
    where relation.case_id = v_case_id
      ...
      and not exists (
        select 1 from public.app_case_location_relations successor
        where successor.supersedes_relation_event_id = relation.id
      )
      and not exists (
        select 1 from public.app_case_location_relations unlink_event
  ...
  with latest_evidence_version as (
    ...
    select distinct
      ...
      case
        when fact.value ->> 'resolution_state' = 'confirmed' then null
        when snapshot.canonical_snapshot #>> '{canonical_facts,schema_version}' =
          'canonical-signing-facts-v1' then 'GENERIC_REVIEW_REQUIRED'
        when snapshot.canonical_snapshot #>> '{canonical_facts,schema_version}' =
          'canonical-signing-facts-v2'
          and fact.value #>> '{resolution_provenance,review_reason}' in (
            'USER_OVERRIDE',
            'USER_SUPPLIED_WITHOUT_DOCUMENT',
            'DOCUMENT_CONFLICT_RESOLVED',
            'PROBABLE_IDENTITY_MATCH',
            'PROBABLE_ADDRESS_MATCH'
          ) then fact.value #>> '{resolution_provenance,review_reason}'
        else null
...
comment on function public.app_evidence_review_case_detail_read_v1(uuid, text) is
  'Service-role-only exact-case read boundary for evidence-review detail. It hard-binds evidence.review.view and exact active case scope through the central evaluator, projects only declared case context, current evidence metadata, signed canonical fact states including closed immutable review provenance, and REVIEW02 status; it exposes no storage path, credential, reviewer identity, parser envelope or preview URL.';
