...
declare
  ...
  v_candidates jsonb;
begin
  v_base_read := public.app_customer_correction_handoff_read_v3(
    p_auth_user_id,
    p_case_ref
  );
  if v_base_read->>'ok' <> 'true'
     or coalesce(
       pg_catalog.jsonb_typeof(v_base_read->'handoff'),
       'missing'
     ) <> 'object' then
    return v_base_read;
  end if;
  ...
  if v_read->>'ok' <> 'true'
     or coalesce(
       pg_catalog.jsonb_typeof(v_read->'handoff'),
       'missing'
     ) <> 'object' then
    return v_read;
  end if;
...
is
  'Customer-safe correction handoff read; v3 preflight preserves successful answered or absent handoff projections before active v4 fact and candidate reconstruction.';
