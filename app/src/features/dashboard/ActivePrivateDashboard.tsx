...
} from "../documents/documentSlotPresentation";
import type {
  DashboardCharger,
...
export function ActivePrivateDashboard({
  ...
  function toggleCharger(chargerId: string) {
    setSelectedChargerId((
      currentChargerId,
    ) => (currentChargerId === chargerId ? null : chargerId));
      ...
      {dashboardRead.status === "loading" || dashboardRead.status === "retrying"
        ? (
          <DashboardNotice
            ...
            note="We halen uw dossiergegevens op."
          />
        )
        : null}
...
function ChargerInformation({
  ...
  const locationStatus = row.location
    ? statusLabel(row.location.status)
    : "Ontbreekt";
  const documentStatus =
    getDocumentSectionStatusPresentation(row.documentSlots).label;
...
function AccordionRow({
  ...
  return (
    <div className="charger-accordion-row">
      ...
      {isExpanded
        ? <div className="charger-accordion-content">{children}</div>
        : null}
    </div>
  );
}
...
function LocationSection({ location }: { location: DashboardLocation | null }) {
  if (!location) {
    return (
      <UnavailableSection
        label="Locatie"
        note="Geen locatiegegevens gevonden voor deze laadpaal."
      />
    );
  }
...
function DocumentsSection({
  ...
  if (!slots.length) {
    return (
      <UnavailableSection
        label="Documenten"
        note="Er zijn nog geen document-slots voor deze laadpaal."
      />
    );
  }
...
function ConsentsSection(
  { acceptances }: { acceptances: DashboardLegalAcceptance[] },
) {
  if (!acceptances.length) {
    return (
      <UnavailableSection
        label="Toestemmingen"
        note="Er zijn nog geen toestemmingen gevonden voor dit dossier."
      />
    );
  }
    ...
    <div className="portal-evidence-grid">
      {acceptances.map((acceptance) => (
        <DownloadCard
          ...
          note={`Versie ${acceptance.version}${
...
function ReadOnlyOverview(
  { status, children }: { status: string; children: ReactNode },
) {
  return (
...
function buildPortalChargers(model: DashboardReadModel): PortalCharger[] {
  const locationsById = new Map(
    model.locations.map((location) => [location.location_id, location]),
  );
  ...
  return model.chargers.map((charger, index) => ({
    ...
    documentSlots: model.document_slots.filter((slot) =>
