...
const DOSSIER_A = "10000000-0000-4000-8000-000000000001";
const DOSSIER_B = "10000000-0000-4000-8000-000000000002";
const DOSSIER_C = "10000000-0000-4000-8000-000000000003";
...
function dashboardBody(selectedDossierId = DOSSIER_A) {
  return {
    ok: true,
...
export async function runDashboardReadClientProof(): Promise<
  DashboardReadClientProofResult
> {
  const endpointUrl =
    "http://localhost:54321/functions/v1/api-app-dashboard-get";
  ...
  assert(
    getHeader(calls[0].init, "Authorization") === `Bearer ${accessToken}`,
    "bearer token header missing",
  );
  assert(
    getHeader(calls[0].init, "apikey") === anonKey,
    "apikey header missing",
  );
  assert(
    getHeader(calls[0].init, "Idempotency-Key") === "",
    "dashboard read must not send Idempotency-Key",
  );
  assert(
    JSON.stringify(JSON.parse(String(calls[0].init?.body))) ===
      JSON.stringify({ dossier_id: DOSSIER_A }),
    "body must contain only dossier_id",
  );
  assert(
    success.model.dossiers.map((dossier: DashboardDossierSummary) =>
  ...
  assert(
    ...
    "case fields must parse without fallback",
  );
  assert(
    success.model.locations.length === 2 && success.model.chargers.length === 2,
    "locations and chargers must parse",
  );
  assert(
    success.model.chargers[1].location_id === "location-proof-2",
    "charger-location link must be preserved",
  );
  assert(
    success.model.document_slots[0].charger_id === "charger-proof-1",
  ...
  assert(
    notFound.error.code === "dossier_inaccessible",
    "not found must map to inaccessible dossier",
  );
  assert(
    !/Raw detail/i.test(notFound.error.message),
    "safe error must not expose raw backend detail",
  );
  assert(
  ...
  ): Promise<{ ok: true; model: DashboardReadModel }> => {
    fetchCount += 1;
    return { ok: true, model: makeModel(dossierId) };
  };

  const first = await loadDashboardReadOnce({
    accessToken,
    cacheScope,
    dossierId: "dossier-proof-a",
    fetcher: cacheFetcher,
  });
  const second = await loadDashboardReadOnce({
    accessToken,
    cacheScope,
    dossierId: "dossier-proof-a",
    fetcher: cacheFetcher,
  ...
  assert(
    first === second && getFetchCount() === 1,
    "duplicate selection must use cache",
  );
  ...
  assert(
    concurrent[0] === concurrent[1] && getFetchCount() === 2,
    "duplicate pending read must dedupe",
  );
  ...
  const sharedFetcher = async (): Promise<
  ...
  clearDashboardReadCache();
  const sharedFirst = loadDashboardReadOnce({
    accessToken,
    cacheScope,
    dossierId: "dossier-proof-shared",
    fetcher: sharedFetcher,
  });
  const sharedSecond = loadDashboardReadOnce({
    accessToken,
    cacheScope,
    dossierId: "dossier-proof-shared",
    fetcher: sharedFetcher,
  ...
  assert(
    sharedFirst === sharedSecond && getSharedFetchCount() === 1,
    "same-scope consumers must share pending request",
  );
  sharedDeferred.resolve({
    ok: true,
    model: makeModel("dossier-proof-shared"),
  });
  const sharedModel = await sharedFirst;
  assert(
    sharedModel.selected_dossier.dossier_id === "dossier-proof-shared",
    "shared request must complete without manual retry",
  );
  await loadDashboardReadOnce({
    accessToken,
    cacheScope,
  ...
  assert(
    getSharedFetchCount() === 1,
    "completed first request must populate cache",
  );
  ...
  assert(
    getSharedFetchCount() === 2,
    "different auth/customer scope must not reuse prior cache",
  );
  ...
  assert(
    getSharedFetchCount() === 3,
    "clearing scope must remove prior cache entries",
  );
  ...
  const failingFetcher = async (): Promise<
    { ok: false; error: DashboardReadSafeError }
  > => {
    failureFetchCount += 1;
  ...
  assert(
    failureFetchCount === 2,
    "failed pending request must be removed in finally",
  );
  ...
  const invalidatedFetcher = async (): Promise<
    { ok: true; model: DashboardReadModel }
  > => {
  ...
  clearDashboardReadCache(cacheScope);
  invalidatedDeferred.resolve({
    ok: true,
    model: makeModel("dossier-proof-invalidated"),
  });
  ...
  assert(
    invalidatedFetchCount === 1 && getFetchCount() === 4,
