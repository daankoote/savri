...
export function createSignupSubmitContractProofDraft(runId = "local"): SignupDraft {
  return {
    personalInfo: {
...
export async function runSignupSubmitContractProof(
  ...
  const runId = options.runId?.trim() || idempotencyKey.replace(/[^a-zA-Z0-9_-]/g, "-");
  ...
  const payload = mapSignupDraftToSubmitPayload(createSignupSubmitContractProofDraft(runId));

  const valid = await postJson(endpointUrl, anonKey, idempotencyKey, payload);
  assert(valid.status === 200, `valid write_v3 request must return 200, got ${valid.status}`);
  assert(valid.body.ok === true, "valid write_v3 response must include ok true");
  assert(valid.body.mode === "write_v3", "valid response must include mode write_v3");
  assert(stringValue(valid.body.customer_id), "valid response must include customer_id");
  assert(stringValue(valid.body.dossier_id), "valid response must include dossier_id");
  assert(numberValue(valid.body.location_count) === 1, "valid response must include location_count 1");
  assert(numberValue(valid.body.charger_count) === 2, "valid response must include charger_count 2");
  assert(numberValue(valid.body.document_slot_count) === 5, "valid response must include document_slot_count 5");
  assert(numberValue(valid.body.legal_acceptance_count) === 2, "valid response must include legal_acceptance_count 2");
  assert(stringValue(valid.body.payload_hash), "valid response must include payload_hash");

  const replay = await postJson(endpointUrl, anonKey, idempotencyKey, payload);
  assert(replay.status === 200, `replay write_v3 request must return 200, got ${replay.status}`);
  assert(replay.body.dossier_id === valid.body.dossier_id, "replay must return same dossier_id");
  assert(replay.body.payload_hash === valid.body.payload_hash, "replay must return same payload_hash");
  ...
  const conflict = await postJson(endpointUrl, anonKey, idempotencyKey, conflictPayload);
  assert(conflict.status === 409, `changed payload with same Idempotency-Key must return 409, got ${conflict.status}`);
  assert(conflict.body.code === "idempotency_conflict", "conflict response must include idempotency_conflict");
