...
- White-label / multi-tenant target architecture:
  ...
  - WL04 selects the smallest physical foundation in `docs/app/architecture/platform-control-plane-physical-foundation.md`: a second Supabase CLI workdir at `platform/control-plane/`, physically separate database/Auth/credentials/migrations, explicit fail-closed target selection, four-table first schema and provider-independent server-only resolver/locator seam. One core supports LabelUP managed, inboeker SaaS and white-label/standalone through orthogonal deployment, branding, support and conflict-participation configuration; standalone tenant-local runtime does not require the live ENVAL control plane. WL05 remains a local-only managed-foundation/adapter batch and authorizes no remote provider or runtime here.
  ...
  - OPEN after the WL04 repository/local boundary: remote control-plane provider/project/region/hosting, provisioning/fleet operations, real ENVAL bootstrap values, detailed support-elevation workflow, federation, and conflict-signal policy/key/legal/retention decisions.
