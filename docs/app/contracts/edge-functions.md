...
| `api-app-evidence-review-correction-publish` | CURRENT / LOCAL SCHEMA + SOURCE PROOF; FIXTURE/BROWSER PENDING | CORS / META / IDEM / AUD / AUTH / SRV | Requires the customer-safe cover message plus one exact document-first requirement per correction subject and calls only publish v3. Database authority stores immutable bundle v4 with strict primary document/parser binding. Older publish versions are not service-callable. |
| `api-app-evidence-review-correction-supersede` | CURRENT / LOCAL SCHEMA + SOURCE PROOF; FIXTURE/BROWSER PENDING | CORS / META / IDEM / AUD / AUTH / SRV | Requires a new cover message and only `DOCUMENT_REPLACEMENT` or `VALUE_PLUS_DOCUMENT_REPLACEMENT` for every successor item, then calls supersede v3. Existing capability, lineage, item-set, lock, replay and conflict rules remain authoritative. Older supersede versions are not service-callable. |
| `api-app-customer-correction-handoff` | CURRENT / LOCAL SCHEMA + RUNTIME + BROWSER PROOF; FINAL FIREFOX ACCEPTANCE PENDING | CORS / META / AUTH / SRV | Calls the R7-authorized read v8. It preserves readable legacy and active handoffs plus terminal `not_available`/`handoff=null`, and separately projects only current primary customer-safe facts. Per-fact `Akkoord` requires an exact current manifest/evidence-version binding, exactly one matching direct parser observation across all envelopes for the evidence/profile/fact or an exact immutable document-first submission binding, and a fully accepted current review round. Unproven source relationships project as supporting; supporting, missing, ambiguous and stale facts remain non-accepted. Applied forward migration `20260918081541` corrects only the handoff customer-column binding. It exposes no parser profile, method, confidence, limitation, hash, technical ID, actor, policy, scope, audit or internal review data. |
| `api-app-customer-correction-upload-url` | CURRENT / LOCAL SCHEMA + SOURCE PROOF; FIXTURE/BROWSER PENDING | CORS / META / IDEM / AUD / AUTH / SRV | Issues only a v2 replacement upload bound to the exact current document-first target and customer/case authority. |
| `api-app-customer-correction-upload-confirm` | CURRENT / LOCAL SCHEMA + SOURCE PROOF; FIXTURE/BROWSER PENDING | CORS / META / IDEM / AUD / AUTH / SRV | Resolves and confirms only the exact selected replacement candidate through the v2 document-first contracts; parser and hash validation remain server-owned. |
| `api-app-customer-correction-upload-remove` | CURRENT / LOCAL SCHEMA + SOURCE PROOF; FIXTURE/BROWSER PENDING | CORS / META / IDEM / AUD / AUTH / SRV | Withdraws only the exact current replacement candidate through the v2 target-bound contract. |
| `api-app-customer-correction-signing-challenge` | CURRENT / LOCAL SCHEMA + SOURCE PROOF; FIXTURE/BROWSER PENDING | CORS / META / IDEM / AUD / AUTH / SRV | Calls challenge v5 only after the document-first fact resolutions are complete and server-valid. |
| `api-app-customer-correction-signing-finalize` | CURRENT / LOCAL SCHEMA + SOURCE PROOF; FIXTURE/BROWSER PENDING | CORS / META / IDEM / AUD / AUTH / SRV | Calls finalize v4; the immutable customer submission remains target- and evidence-bound and always returns changed or transcribed facts to workforce review. |
