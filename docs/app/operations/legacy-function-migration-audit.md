...
|---|---|---|---|---|---|---|
| `api-app-signup-submit` | app-new | Current `/app` signup submit write v3 endpoint. | `app_customers`, `app_customer_identities`, `app_customer_dossiers`, locations, chargers, document slots, legal acceptances, app audit, app idempotency. | Medium: foundation is proven locally; production auth/bootstrap and account-specific document expansion remain open. | keep/extend | Uses `app_foundation.ts`; frontend submit wiring exists locally, but dashboard bootstrap and document upload are separate. |
