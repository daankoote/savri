...
- New app direction: `api-app-*` endpoints, including locally proven `api-app-signup-submit` write v3, `api-app-document-upload-url`, and `api-app-document-upload-confirm`.
...
|---|---|---|---|---|---|---|
| `api-app-signup-submit` | app-new | Current `/app` signup submit write v3 endpoint. | `app_customers`, `app_customer_identities`, `app_customer_dossiers`, locations, chargers, document slots, legal acceptances, app audit, app idempotency. | Medium: foundation is proven locally; production Auth configuration, dashboard read projection, and account-specific document expansion remain open. | keep/extend | Uses `app_foundation.ts`; frontend submit and Auth/session bootstrap wiring exist locally; dashboard read projection and document upload wiring are separate. |
...
- New app signup submit: `api-app-signup-submit` write v3 proves CORS, gateway auth boundary, idempotency, payload hash, customer/dossier shell creation, locations/chargers, document slots, legal acceptances, and app audit locally.
