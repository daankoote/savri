...
| Current work package | WP3D inventories CURRENT location truth and records a bounded draft location contract; documentation is COMPLETE — UNCOMMITTED; verdict is `BLOCKED — CURRENT LOCATION OBJECTS CONFLICT WITH TARGET` and no implementation, proof, DDL, cutover or retirement is authorized |
| Last completed work package | WP3C — connection/EAN internal domain decisions — DOCS ONLY |
| Next executable work package | Explicit WP3D location domain decisions and attributable external validation — DOCS ONLY; only after a separately DDL-ready contract and proof may a limited connection-root/claim DDL-readiness assessment occur |
| Current blockers | Populated mutable `app_dossier_locations` conflicts with stable root/version/observation TARGET; root identity, version acceptance, correction/relocation, link cardinalities, supersession, concurrency, grants, mapping/cutover and PDOK/BAG/verifier/DSO/MLOEA/evidence/retention/privacy answers remain open. Connection remains location-dependent. Authority validation is independent and representation authority is NOT SCHEMA READY |
...
| 3 | Location, connection/EAN and validity periods | Complete location/connection truth, temporal facts, aangeslotene claims, and safe review/read boundaries. | BLOCKED — DECISION | NEA-EAN; NEA-AUD-002; NEA-SEC-001/002 | Legal-party connection truth is separate from account, case role, authority, mandate, location, charger/MID and kWh; CAR/DSO truth remains external. | Hybrid | Committed WP3A-WP3C evidence; WP2 party/case boundaries; WP3D location decisions and later accepted evidence/source decisions | WP3D proof-only audit and bounded draft contract complete; no schema or runtime module approved | Current location has 44 mutable dossier-bound rows and live signup/dashboard/charger/document dependencies; connection objects remain dispositioned for replacement; no mutation performed | No CURRENT PROVEN location or connection proof; existing dependent proofs are `PROVE AGAIN` and future proof must add immutable history, no-inference, concurrency, protected-truth and caller/cutover evidence | WP3C HEAD `da961fa84da73ecc320b55b2cb83881a12d658f3`; `operations/wp3d-location-current-truth-readiness-audit.md`; `contracts/location-foundation.md`; proofs not run | Resolve Daan's WP3D matrix and applicable PDOK/BAG/verifier/DSO/MLOEA/evidence/retention/privacy blockers; do not start DDL, cutover or retirement. |
...
| Internal after GO | locations | IN PROGRESS | WP3D inventories the populated mutable dossier snapshot, live callers and exact TARGET conflict; the draft root/version/observation contract is not approved or DDL-ready and no implementation/proof exists. |
...
Work package 3 is `IN PROGRESS`. WP3A is committed proof-only current-truth evidence, WP3B is committed reconciliation/source disposition, and WP3C records Daan's approval of internal TARGET packages A–E. WP3D now inventories the populated CURRENT dossier-location object and live callers and adds a bounded draft contract. Its verdict is `BLOCKED — CURRENT LOCATION OBJECTS CONFLICT WITH TARGET`; this is not implementation, proof or DDL authorization. Location decisions and applicable external/evidence dependencies remain blocking. Authority validation continues externally and independently.

| checklist item | status | evidence / gate |
|---|---|---|
| design/contract | COMPLETE — UNCOMMITTED | WP3D adds `operations/wp3d-location-current-truth-readiness-audit.md` and `contracts/location-foundation.md` with exact draft/non-DDL status, candidate responsibilities only, a Daan decision matrix and external blockers. |
| existing module inventory | COMPLETE — UNCOMMITTED | Read-only source/catalog inspection records current migration provenance, 44 location rows, inbound charger/document/connection FKs, signup write, dashboard read, Auth boundary, PDOK observation, legacy source material, proofs and hashes. |
...
| database | BLOCKED — DECISION | `app_dossier_locations` is mutable, dossier-bound and populated; it has no stable root, immutable versions, observations, business validity or supersession. No DDL, row mutation, migration-history write, cutover or retirement occurred. |
| Edge Function/service | COMPLETE — UNCOMMITTED | Signup writes and dashboard reads the current object; Auth/bootstrap authorizes only customer/dossier; document endpoints consume slots rather than location truth. No runtime changed. |
| frontend/UI | COMPLETE — UNCOMMITTED | Signup address lookup/mapper and dashboard projection/rendering depend on current shapes. PDOK is an observation only. No frontend or CSS changed. |
| tests | BLOCKED — DECISION | Existing dashboard/document/connection proofs are source evidence only and must `PROVE AGAIN` for TARGET location semantics after explicit contract and batch approval. |
| SQL proof | COMPLETE — UNCOMMITTED | Explicit read-only local catalog/aggregate queries confirmed object shape, grants, dependencies and protected counts. No proof script or migration ran; this is inventory evidence, not implementation proof. |
| browser proof | TODO | Required only when approved customer-visible behavior is implemented. |
| documentation update | COMPLETE — UNCOMMITTED | Two WP3D documents plus bounded roadmap, TODO and append-only changelog updates; no prior audit/contract, requirement, architecture, traceability, MVP plan, code or schema changed. |
| accepted by Daan | BLOCKED — DECISION | WP3C package B approves only the direction and bounded context. The WP3D contract, decision matrix, external answers, DDL, implementation, proof, cutover, retirement and execution are not approved. |
