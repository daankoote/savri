...
Next bounded gates: WP3F-B is committed in HEAD `e6aac0119c5e545673a07c6a985e1921a663ba49`. WP3G finds the empty additive three-table responsibility and sequential proof surface bounded, but implementation is not safe without an additional exact physical-catalog decision for observation provenance/freshness/hashes/descriptors and version-to-observation/acceptance provenance. Verdict: `BLOCKED — BOUNDED LOCATION FOUNDATION IMPLEMENTATION NOT SAFE`. The 42-row proof contract is TARGET / NOT IMPLEMENTED. No migration or proof exists; operationele write-RPC/concurrency, all 44-row population, caller cutover and retirement remain separately blocked. Connection DDL remains dependent on a proven locationfoundation.
...
| Current work package | WP3G — bounded location foundation implementation-readiness and proof contract — PROOF ONLY / DOCS ONLY |
| Last completed work package | WP3F-B — bounded location DDL decisions 1–18 — APPROVED TARGET — COMMITTED in HEAD `e6aac0119c5e545673a07c6a985e1921a663ba49` |
| Next executable work package | Separate docs-only approval of the exact observation/descriptor/acceptance physical catalog; do not create migration or proof until that catalog and a later implementation batch are explicitly authorized |
| Current blockers | Exact observation actor/request fields, source-hash shape, freshness representation, normalized descriptor columns, version-to-observation cardinality, acceptance provenance and recording defaults are not approved. Operationele write-RPC/concurrency, all 44-row population, physical-site matching, PDOK/BAG, verifier acceptance, relation links, projection, cutover, retirement, privacy and final retention remain separately blocked. |
...
| 3 | Location, connection/EAN and validity periods | Complete location/connection truth, temporal facts, aangeslotene claims, and safe review/read boundaries. | IN PROGRESS | NEA-EAN; NEA-AUD-002; NEA-SEC-001/002 | Legal-party connection truth is separate from account, case role, authority, mandate, location, charger/MID and kWh; TKV-aligned internal controls do not establish regulatory acceptance. | Hybrid | WP3F-B committed at `e6aac0119c5e545673a07c6a985e1921a663ba49`; WP3G readiness/proof contract; WP2 party/case patterns | Three bounded table responsibilities and invariants are approved; WP3G records a recommended catalog but finds physical observation/acceptance choices still unapproved | `BLOCKED — BOUNDED LOCATION FOUNDATION IMPLEMENTATION NOT SAFE`; no migration/proof; all target tables absent; all 44 current rows and callers unchanged | TARGET 42-row proof matrix covers catalog, emptiness, immutability, vocabularies, descriptors/hashes, temporal/supersession, deferred sequential validation, RLS/grants and cleanup; operationele concurrency excluded | `operations/wp3g-location-foundation-implementation-readiness.md`; `operations/wp3g-location-foundation-proof-contract.md`; read-only local catalog; TKV guard | Approve the exact physical observation/descriptor/acceptance catalog, then separately authorize migration/proof implementation. Write-RPC, population, cutover and retirement remain separate. |
...
| Internal after GO | locations | IN PROGRESS | WP3F-B is committed in `e6aac0119c5e545673a07c6a985e1921a663ba49`. WP3G readiness is BLOCKED on exact physical observation/descriptor/acceptance choices; its 42-row proof contract is TARGET / NOT IMPLEMENTED. No migration, proof, population or cutover exists. |
...
| checklist item | status | evidence / gate |
|---|---|---|
| design/contract | BLOCKED — DECISION | WP3F-B approves responsibilities/invariants, but WP3G proves the exact observation/descriptor/acceptance catalog still requires approval. |
...
| database | BLOCKED — DECISION | Read-only catalog: all three names free, zero TARGET dependencies, `pgcrypto`/`gen_random_uuid()` available, 24/24 RLS/deny-all convention, local migration history empty. Exact physical catalog and implementation authorization remain absent. |
...
| documentation update | COMPLETE — UNCOMMITTED | WP3G adds exactly two operations documents and updates roadmap, TODO and append-only changelog only. Protected contracts, prior WP3 records, architecture, traceability, code and schema remain unchanged. |
| accepted by Daan | BLOCKED — DECISION | WP3F-B remains approved TARGET; the recommended exact WP3G physical catalog is not approved and is not implementation authority. |
