...
Next bounded gates: WP3F is committed in HEAD `c5a46faa26d94ad22adbd2b3748f411e1b37e51e` and remains the historical proof that DDL was unsafe before the physical decisions. Daan approved WP3F-B package 1–18 as TARGET for an empty additive `app_locations`, `app_location_address_observations`, and `app_location_versions` foundation. Implementation remains `NOT IMPLEMENTED`, proof `NOT PROVEN`, migration and data population `NOT AUTHORIZED`, caller cutover `BLOCKED`, and retirement `NOT AUTHORIZED`. A later migration/proof requires its own explicit authorization; all 44 current rows remain excluded and require manual mapping. Connection DDL remains dependent on a proven locationfoundation. Independently obtain written Dutch corporate-law and external-verifier validation of the representation-authority pilot brief and resolve the connection/EAN external blockers.
...
| Current work package | WP3F-B — bounded location DDL decisions 1–18 — APPROVED TARGET / DOCS ONLY; exact status `DECISION RECORD — WP3F-B BOUNDED LOCATION DDL PACKAGE APPROVED — NO IMPLEMENTATION AUTHORIZATION` |
| Last completed work package | WP3F — DDL-readiness and 44-row classification audit — PROOF ONLY — COMMITTED in HEAD `c5a46faa26d94ad22adbd2b3748f411e1b37e51e` |
| Next executable work package | Separate authorization decision for an empty additive three-table migration and proof; do not create migration, SQL or proof until explicitly authorized |
| Current blockers | Migration/proof authorization is absent. All 44 current rows remain outside the foundation and require manual mapping; data population and retirement are not authorized and caller cutover is blocked. Physical-site matching, PDOK/BAG, verifier acceptance, relation links, projection, write-RPC, privacy and final retention remain open. Connection remains location-dependent. Authority validation is independent and representation authority is NOT SCHEMA READY |
...
| Evidence reference | WP2B-I commit `5a5265adc516e8198cc25757654920d4aa3316bd`; authority brief commit `d44379f52125370eb91b0de52098a1df69bd2f92`; WP3A commit `f3b39aafb2e6817e64401ccb2c47eed285552869`; WP3B commit `ee3f6b59c937f0c39a67ba09936e9ef688bcea59`; WP3C commit `da961fa84da73ecc320b55b2cb83881a12d658f3`; WP3D commit `88e8c0b754c7d44e769f89037676d9732e6fe63c`; WP3E commit `e04f4a695d983c71a52f48d0c3c26ca605bb4402`; WP3F HEAD `c5a46faa26d94ad22adbd2b3748f411e1b37e51e`; `operations/wp3fb-location-bounded-ddl-decisions.md`; official TKV snapshot and mapped requirements |
...
| 3 | Location, connection/EAN and validity periods | Complete location/connection truth, temporal facts, aangeslotene claims, and safe review/read boundaries. | IN PROGRESS | NEA-EAN; NEA-AUD-002; NEA-SEC-001/002 | Legal-party connection truth is separate from account, case role, authority, mandate, location, charger/MID and kWh; TKV-aligned internal controls do not establish regulatory acceptance. | Hybrid | WP3F committed at `c5a46faa26d94ad22adbd2b3748f411e1b37e51e`; approved WP3F-B package 1–18; WP2 party/case boundaries; later source/evidence decisions | Exact bounded foundation is `app_locations`, `app_location_address_observations`, `app_location_versions`; immutable accepted-only/observation/temporal/supersession/RLS/minimum-grant invariants approved; relation tables, projection and write-RPC excluded | `NOT IMPLEMENTED` / `NOT PROVEN`; first future migration must be empty/additive but is not authorized; all 44 current rows remain unmapped and unchanged; caller cutover blocked | Future proof must cover exact catalog, immutable root/observations/versions, descriptors/hashes, half-open time, operational overlap, same-root correction supersession, deterministic locking, deferred validation, audit/idempotency, exact RLS/grants and cleanup | WP3F HEAD `c5a46faa26d94ad22adbd2b3748f411e1b37e51e`; `operations/wp3fb-location-bounded-ddl-decisions.md`; `TKV ALIGNMENT GUARD — INTERNAL ARCHITECTURE, NOT REGULATORY ACCEPTANCE` | Seek separate explicit migration/proof authorization. Do not create DDL, populate rows, cut over callers or retire current objects. Connection DDL waits for proven locationfoundation. |
...
| Internal after GO | locations | IN PROGRESS | WP3F is committed in `c5a46faa26d94ad22adbd2b3748f411e1b37e51e`. WP3F-B 1–18 approves the exact empty additive three-table TARGET shape and invariants. Implementation/proof remain absent, migration/population/retirement unauthorized, and all 44-row mapping plus caller cutover blocked. |
...
| checklist item | status | evidence / gate |
|---|---|---|
| design/contract | TARGET | WP3F-B package 1–18 approves the bounded three-table physical TARGET and internal invariants. It is NOT IMPLEMENTED / NOT PROVEN and grants no migration, proof, population, RPC, cutover or retirement authorization. |
| existing module inventory | COMPLETE — CURRENT PROVEN | WP3F and WP3F-B inspect the current table/references/callers and reuse only WP2A/WP2B-I temporal, immutable, RLS/grant/concurrency patterns plus app audit/idempotency and safe projection boundaries. |
...
| database | BLOCKED — DECISION | Migration/proof authorization is absent. `app_dossier_locations` remains mutable, dossier-bound and populated. The approved first migration would be empty/additive only, but no DDL may be created yet. No current row, migration history, cutover or retirement may change. |
...
| tests | TODO | Future proof must cover the WP3F-B exact catalog, immutability, vocabularies/descriptors/hashes, half-open time, supersession, operational overlap, locking/deferred validation, idempotency/audit, concurrency, RLS/grants and cleanup after separate authorization. |
| SQL proof | COMPLETE — CURRENT PROVEN | WP3F's committed proof-only read transactions classified current state without writes; this is not implementation proof for WP3F-B. |
| browser proof | TODO | Required only when approved customer-visible behavior is implemented. |
| documentation update | COMPLETE — UNCOMMITTED | WP3F-B adds one decision record and synchronizes the bounded location contract, architecture, traceability, roadmap, TODO and changelog only. Protected prior records, code and schema remain unchanged. |
| accepted by Daan | TARGET | WP3F-B package 1–18 is APPROVED TARGET with exact no-implementation status. External blockers remain open. |
| commit | TODO | This docs-only decision batch is uncommitted. No staging or commit is authorized. |
