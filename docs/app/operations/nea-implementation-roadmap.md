...
Next bounded gates: obtain written Dutch corporate-law and external-verifier validation of the simple-majority representation-authority pilot brief; independently reconcile the WP3A connection/EAN current-truth conflicts through a docs-only WP3B contract/disposition batch. Neither gate authorizes schema work.
...
| Current work package | WP3A connection/EAN current-truth readiness audit is COMPLETE — UNCOMMITTED; authority validation continues externally |
| Last completed work package | WP3A — connection/EAN current-truth and readiness audit — DOCS ONLY |
| Next executable work package | WP3B connection/EAN domain contract reconciliation and object disposition — DOCS ONLY; authority written validation proceeds independently |
| Current blockers | Connection migrations are ignored/untracked, proofs are untracked, local migration history is empty and current account/dossier ownership conflicts with WP2 party/profile/case boundaries. Authority legal/verifier answers remain pending and representation authority is NOT SCHEMA READY |
...
| Evidence reference | WP2B-I commit `5a5265adc516e8198cc25757654920d4aa3316bd`; authority brief commit `d44379f52125370eb91b0de52098a1df69bd2f92`; `proofs/wp2b-i-case-party-role-foundation.md`; `legal/representation-authority-pilot-validation-brief.md`; `operations/wp3a-connection-ean-current-truth-readiness-audit.md`; official TKV snapshot and mapped requirements |
...
| 3 | Location, connection/EAN and validity periods | Complete location/connection truth, temporal facts, aangeslotene claims, and safe review/read boundaries. | BLOCKED — DECISION | NEA-EAN; NEA-AUD-002; NEA-SEC-001/002 | Legal-party connection truth is separate from account, case role, authority, mandate, location, charger/MID and kWh; CAR/DSO truth remains external. | Hybrid | WP3A current-truth audit; WP2 party/case boundaries; later accepted evidence/source decisions | Contract reconciliation first; no schema or runtime module approved | Three empty local tables and five write functions exist outside committed migration history; disposition is unresolved | No CURRENT PROVEN connection proof; both existing proof files require contract-driven revision and proof again | `operations/wp3a-connection-ean-current-truth-readiness-audit.md`; read-only local catalog; historical proof claims only | Run WP3B docs-only contract reconciliation and exact keep/repair/replace disposition; do not start read projection or DDL. |
...
Work package 3 is `BLOCKED — DECISION`. WP3A proves current catalog presence but finds that ignored/untracked migration and proof sources, empty migration history, direct account/dossier ownership, absent party/profile/case links and incomplete history/evidence/concurrency rules prevent a `CURRENT PROVEN` or target-compatible claim. Authority validation continues externally and remains separate.

| checklist item | status | evidence / gate |
|---|---|---|
| design/contract | BLOCKED — DECISION | WP3B must reconcile legal-party/profile, case, location/allocation, calendar-year, evidence, history, supersession and concurrency semantics before DDL. |
| existing module inventory | COMPLETE — UNCOMMITTED | WP3A inventories ignored migrations, untracked proofs, local catalog, migration history, functions, grants and absence of runtime callers. |
...
| database | BLOCKED — DECISION | Three empty local tables plus thirteen connection-named functions exist, but the source chain is not committed, migration history has zero rows and the physical model conflicts with WP2 boundaries. |
...
| tests | BLOCKED — DECISION | Existing untracked Q1-Q34/Q1-Q36 sources encode the conflicting model and cannot be reused unchanged. |
| SQL proof | PROOF ONLY | Historical PASS claims exist, but there is no committed proof/migration pair, raw output, source hashes tied to the run or evidence page. Proofs were not run in WP3A. |
| browser proof | TODO | Required only when approved customer-visible behavior is implemented. |
| documentation update | COMPLETE — UNCOMMITTED | WP3A audit plus minimal roadmap/TODO/changelog updates only; contracts, requirements, traceability and MVP plan remain unchanged. |
| accepted by Daan | BLOCKED — DECISION | WP3B contract/disposition requires a later explicit decision. |
