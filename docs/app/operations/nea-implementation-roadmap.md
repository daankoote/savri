...
Next bounded gates: WP3G is committed in HEAD `c021d57aacc5d8beb4aa2043bc963839fa38da07`; its historical verdict `BLOCKED — BOUNDED LOCATION FOUNDATION IMPLEMENTATION NOT SAFE` and 42-row proof contract remain unchanged. Daan approved WP3G-B exact physical schema package 1–8 as TARGET, closing the six internal catalog-decision gaps without authorizing implementation. A new bounded readiness reconciliation is now required. No migration or proof exists or is authorized; operationele write-RPC/advisory-lock/two-transaction concurrency, all 44-row mapping/population, caller cutover and retirement remain separately blocked. Connection DDL remains dependent on a proven locationfoundation.
...
| Current work package | WP3G-B — exact physical location schema decisions package 1–8 — APPROVED TARGET / DOCS ONLY |
| Last completed work package | WP3G — bounded location foundation implementation-readiness and proof contract — PROOF ONLY — COMMITTED in HEAD `c021d57aacc5d8beb4aa2043bc963839fa38da07` |
| Next executable work package | New docs-only readiness reconciliation against approved WP3G-B package 1–8; do not create a migration or proof until a later implementation batch is explicitly authorized |
| Current blockers | Migration, proof and database writes remain unauthorized. Operationele write-RPC, advisory-lock concurrency, true two-transaction concurrency proof, all 44-row mapping/population, physical-site matching, PDOK/BAG source, verifier acceptance, relation links, split/merge, projection, cutover, retirement, privacy and final retention remain blocked/open. |
...
| 3 | Location, connection/EAN and validity periods | Complete location/connection truth, temporal facts, aangeslotene claims, and safe review/read boundaries. | IN PROGRESS | NEA-EAN; NEA-AUD-002; NEA-SEC-001/002 | Legal-party connection truth is separate from account, case role, authority, mandate, location, charger/MID and kWh; TKV-aligned internal controls do not establish regulatory acceptance. | Hybrid | WP3G committed at `c021d57aacc5d8beb4aa2043bc963839fa38da07`; WP3G-B exact physical schema package 1–8 approved TARGET; WP2 party/case patterns | Three exact physical tables, columns/defaults, normalized descriptors, hash/freshness, primary-observation/acceptance provenance and same-root keys are decided; operationele write boundary and external relations remain separate | Exact physical shape APPROVED TARGET; `NOT IMPLEMENTED` / `NOT PROVEN`; migration/proof/database write not authorized; all target tables absent; all 44 current rows and callers unchanged | Historical WP3G Q01-Q42 proof contract remains TARGET / NOT IMPLEMENTED; a new readiness reconciliation must align it to package 1–8; operationele concurrency remains excluded pending a separate write-RPC batch | `operations/wp3gb-location-physical-schema-decisions.md`; unchanged WP3G readiness/proof contract; TKV guard | Perform a new docs-only readiness reconciliation; only then seek separate migration/proof authorization. Write-RPC, concurrency, mapping/population, cutover and retirement remain separate. |
...
| Internal after GO | locations | IN PROGRESS | WP3G is committed in `c021d57aacc5d8beb4aa2043bc963839fa38da07` and remains unchanged historical readiness evidence. WP3G-B exact physical schema package 1–8 is APPROVED TARGET. A new readiness reconciliation is required; no migration, proof, database write, population or cutover exists or is authorized. |
...
write. A new readiness reconciliation is required.

| checklist item | status | evidence / gate |
|---|---|---|
| design/contract | TARGET + RECONCILED | WP3G-B approves exact physical schema package 1–8 and closes the six WP3G catalog-decision gaps. Historical WP3G remains unchanged; a new implementation-readiness reconciliation is required. |
...
| database | BLOCKED — DECISION | Read-only WP3G catalog evidence remains: all three names free, zero TARGET dependencies, `pgcrypto`/`gen_random_uuid()` available, 24/24 RLS/deny-all convention, local migration history empty. Exact physical schema is now approved TARGET, but migration/proof/database-write authorization remains absent. |
...
| tests | TARGET | WP3G defines Q01-Q42. Foundation proof covers sequential invariants only; advisory-lock/two-transaction concurrency follows with a separately approved write-RPC. |
...
| documentation update | COMPLETE — UNCOMMITTED | WP3G-B adds one operations decision record and synchronizes the location contract, architecture, traceability, roadmap, TODO and append-only changelog only. Historical WP3 records, code and schema remain unchanged. |
| accepted by Daan | TARGET | Exact physical schema package 1–8 is APPROVED TARGET. This is not implementation, proof, database-write, remote, NEa or verifier acceptance. |
