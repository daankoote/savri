...
async function onReviewClicked() {
  ...
  try {
    ...
    const idem = newIdempotencyKey();
    const res = await fetch(`${window.ENVAL.API_BASE}/api-dossier-submit-review`, {
      method: "POST",
      headers: window.ENVAL.edgeHeaders({ "Idempotency-Key": idem }),
      body: JSON.stringify({ dossier_id, token }),
    });
    ...
    if (!js.locked_at) {
      // Dit mag eigenlijk niet meer gebeuren, maar we zijn streng: geen nep-succes.
      if ($("reviewState")) $("reviewState").textContent = "Review lijkt gelukt, maar dossier is niet vergrendeld. Probeer opnieuw.";
