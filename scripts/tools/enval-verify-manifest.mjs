import { TENANT_ENVAL_MIGRATION_CHAIN } from "./enval-migration-chain-manifest.mjs";
import { COMPILER_AUTHORITY } from "./enval-verify-ownership.mjs";

export const MODES = Object.freeze([
  "QUICK",
  "TARGETED",
  "LOCAL_SERVICE",
  "INTEGRATION",
  "RELEASE",
]);

export const SAFETY = Object.freeze({
  SAFE_PURE: "SAFE_PURE",
  SAFE_LOCAL_READ: "SAFE_LOCAL_READ",
  SAFE_LOCAL_CONTROL_PLANE_WRITE: "SAFE_LOCAL_CONTROL_PLANE_WRITE",
  SAFE_LOCAL_TENANT_EPHEMERAL_WRITE: "SAFE_LOCAL_TENANT_EPHEMERAL_WRITE",
  SAFE_GENERATED_WRITE: "SAFE_GENERATED_WRITE",
  LOCAL_MUTATING_GATED: "LOCAL_MUTATING_GATED",
  DESTRUCTIVE_GATED: "DESTRUCTIVE_GATED",
  REMOTE_GATED: "REMOTE_GATED",
  MANUAL_GATED: "MANUAL_GATED",
});

function check({
  id,
  argv = null,
  runner = "process",
  domain,
  applicablePaths,
  safety,
  minimumMode,
  serviceRequirements = [],
  mutatesState = false,
  destructive = false,
  remote = false,
  expectedDurationMs,
  dedupeKey = id,
  perPath = false,
  expectedMarker = null,
  requiredForRelease = false,
  compilerAuthority = null,
}) {
  return Object.freeze({
    id,
    argv: argv ? Object.freeze(argv) : null,
    runner,
    domain,
    applicablePaths: Object.freeze(applicablePaths),
    safety,
    minimumMode,
    serviceRequirements: Object.freeze(serviceRequirements),
    mutatesState,
    destructive,
    remote,
    expectedDurationMs,
    dedupeKey,
    perPath,
    expectedMarker,
    requiredForRelease,
    compilerAuthority,
  });
}

const CHECK_LIST = [
  check({
    id: "git-diff-check",
    argv: ["git", "diff", "--check", "--", "."],
    domain: "repository-integrity",
    applicablePaths: ["all changed tracked paths"],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "QUICK",
    expectedDurationMs: 50,
  }),
  check({
    id: "node-check-changed",
    argv: ["node", "--check", "{path}"],
    domain: "javascript-static",
    applicablePaths: ["**/*.js", "**/*.mjs", "**/*.cjs"],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "QUICK",
    expectedDurationMs: 150,
    dedupeKey: "node-check:{path}",
    perPath: true,
  }),
  check({
    id: "git-diff-cached-check",
    argv: ["git", "diff", "--cached", "--check", "--", "."],
    domain: "pre-commit-index-integrity",
    applicablePaths: ["all staged paths"],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "INTEGRATION",
    expectedDurationMs: 50,
  }),
  check({
    id: "archived-migration-reference-pure",
    argv: [
      "node",
      "scripts/proofs/enval-archived-migration-reference.proof.mjs",
    ],
    domain: "tenant-migration-archive-reference-integrity",
    applicablePaths: [
      "scripts/proofs/**",
      "scripts/tools/**",
      "supabase/migrations/**",
      "supabase/migration-archive/**",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 300,
    expectedMarker: "ARCHIVED_MIGRATION_REFERENCE_GATE=PASS",
  }),
  check({
    id: "deno-check-changed",
    argv: [
      "deno",
      "check",
      "--deny-import",
      "--no-lock",
      "--unstable-sloppy-imports",
      "{path}",
    ],
    domain: "typescript-static",
    applicablePaths: [
      "platform/**/*.ts",
      "platform/**/*.tsx",
      "platform/**/*.mts",
      "platform/**/*.cts",
      "scripts/**/*.ts",
      "scripts/**/*.tsx",
      "scripts/**/*.mts",
      "scripts/**/*.cts",
      "supabase/functions/**/*.ts",
      "supabase/functions/**/*.tsx",
      "supabase/functions/**/*.mts",
      "supabase/functions/**/*.cts",
      "non-app TypeScript fallbacks",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "QUICK",
    expectedDurationMs: 800,
    dedupeKey: "deno-check:{path}",
    perPath: true,
    compilerAuthority: COMPILER_AUTHORITY.DENO_NATIVE,
  }),
  check({
    id: "deno-check-app-changed",
    argv: [
      "deno",
      "check",
      "--deny-import",
      "--no-lock",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "--unstable-sloppy-imports",
      "{path}",
    ],
    domain: "react-typescript-static",
    applicablePaths: ["app/src/**/*.ts", "app/src/**/*.tsx"],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "QUICK",
    expectedDurationMs: 1_500,
    dedupeKey: "deno-check-app:{path}",
    perPath: true,
    compilerAuthority: COMPILER_AUTHORITY.APP_TYPESCRIPT,
  }),
  check({
    id: "deno-check-browser-proof",
    argv: [
      "deno",
      "check",
      "--deny-import",
      "--no-lock",
      "--config",
      "scripts/tools/deno-browser-proof.json",
      "--unstable-sloppy-imports",
      "{path}",
    ],
    domain: "browser-proof-typescript-static",
    applicablePaths: [
      "scripts/proofs/app-signup-signing-kiss.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "QUICK",
    expectedDurationMs: 800,
    dedupeKey: "deno-browser-proof:{path}",
    perPath: true,
    compilerAuthority: COMPILER_AUTHORITY.DENO_NATIVE,
  }),
  check({
    id: "signup-journey-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--unstable-sloppy-imports",
      "scripts/proofs/app-signup-journey.proof.ts",
    ],
    domain: "signup-client-contract",
    applicablePaths: ["app/src/features/signup/**"],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 800,
    expectedMarker: "signup-journey-proof-ok",
  }),
  check({
    id: "legacy-dev-unlock-caller-retirement-pure",
    argv: [
      "node",
      "scripts/proofs/legacy-dev-unlock-caller-retirement.proof.mjs",
    ],
    domain: "legacy-dev-unlock-caller-retirement",
    applicablePaths: [
      "dossier.html",
      "assets/js/pages/dossier.js",
      "scripts/proofs/legacy-dev-unlock-caller-retirement.proof.mjs",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 150,
    expectedMarker: "LEGACY_DEV_UNLOCK_CALLER_REMOVED_SOURCE_RETAINED=PASS",
  }),
  check({
    id: "legacy-access-update-caller-retirement-pure",
    argv: [
      "node",
      "scripts/proofs/legacy-access-update-caller-retirement.proof.mjs",
    ],
    domain: "legacy-access-update-caller-retirement",
    applicablePaths: [
      "assets/js/pages/dossier.js",
      "scripts/proofs/legacy-access-update-caller-retirement.proof.mjs",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 150,
    expectedMarker: "LEGACY_ACCESS_UPDATE_CALLER_REMOVED_PRIMARY_RETAINED=PASS",
  }),
  check({
    id: "legacy-ev-direct-caller-retirement-pure",
    argv: [
      "node",
      "scripts/proofs/legacy-ev-direct-caller-retirement.proof.mjs",
    ],
    domain: "legacy-ev-direct-caller-retirement",
    applicablePaths: [
      "_redirects",
      "aanmelden.html",
      "assets/js/eligibility.js",
      "assets/js/script.js",
      "dossier.html",
      "hoe-het-werkt.html",
      "index.html",
      "pricing.html",
      "privacyverklaring.html",
      "regelgeving.html",
      "voorwaarden.html",
      "scripts/proofs/legacy-ev-direct-caller-retirement.proof.mjs",
      "sitemap.xml",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 150,
    expectedMarker:
      "LEGACY_EV_DIRECT_CALLER_REMOVED_CURRENT_SIGNUP_RETAINED=PASS",
  }),
  check({
    id: "signup-unified-presentation-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--unstable-sloppy-imports",
      "scripts/proofs/app-signup-unified-presentation.proof.ts",
    ],
    domain: "signup-presentation-contract",
    applicablePaths: ["app/src/features/signup/**"],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 800,
    expectedMarker: "signup-unified-presentation-08-proof-ok",
  }),
  check({
    id: "document-fact-matrix-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=NODE_ENV",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "app/src/features/documents/DocumentFactMatrix.proof.tsx",
    ],
    domain: "shared-document-fact-matrix-presentation",
    applicablePaths: [
      "app/src/features/documents/DocumentFactMatrix.tsx",
      "app/src/features/documents/DocumentFactMatrix.proof.tsx",
      "app/src/features/signup/presentation/FactTable.tsx",
      "app/src/features/dashboard/CustomerCorrectionHandoffPanel.tsx",
      "app/src/styles/components.css",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "DOCUMENT_FACT_MATRIX_Q01_Q09=PASS",
  }),
  check({
    id: "customer-document-fact-registry-c3c7-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--unstable-sloppy-imports",
      "app/src/features/documents/customerDocumentFactRegistry.proof.ts",
    ],
    domain: "shared-signup-correction-fact-registry",
    applicablePaths: [
      "app/src/features/documents/customerDocumentFactRegistry.proof.ts",
      "app/src/features/signup/documentFactRegistry.ts",
      "app/src/features/signup/presentation/factPresentationModel.ts",
      "app/src/features/dashboard/customerCorrectionWorkspace.ts",
      "app/src/features/dashboard/CustomerCorrectionHandoffPanel.tsx",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "QUICK",
    expectedDurationMs: 1_000,
    expectedMarker: "CUSTOMER04C3C7_CANONICAL_FACTS_Q01_Q10=PASS",
  }),
  check({
    id: "document-evidence-workflow-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=NODE_ENV",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "app/src/features/documents/DocumentEvidenceWorkflow.proof.tsx",
    ],
    domain: "shared-signup-correction-document-workflow-presentation",
    applicablePaths: [
      "app/src/features/documents/DocumentEvidenceWorkflow.tsx",
      "app/src/features/documents/DocumentEvidenceWorkflow.proof.tsx",
      "app/src/features/documents/DocumentEvidenceUploadCard.tsx",
      "app/src/features/signup/DocumentFirstDocumentsStep.tsx",
      "app/src/features/dashboard/CustomerCorrectionHandoffPanel.tsx",
      "app/src/styles/components.css",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "CUSTOMER04C3C7_ONE_EXTERNAL_WORKFLOW_Q01_Q06=PASS",
  }),
  check({
    id: "customer-document-interaction-c3c8-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=NODE_ENV",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "app/src/features/documents/CustomerDocumentFactInteraction.proof.tsx",
    ],
    domain: "shared-customer-document-interaction-presentation",
    applicablePaths: [
      "app/src/features/documents/CustomerDocumentFactInteraction.tsx",
      "app/src/features/documents/CustomerDocumentFactInteraction.proof.tsx",
      "app/src/features/documents/DocumentFactMatrix.tsx",
      "app/src/features/signup/DocumentFirstCheckMatrix.tsx",
      "app/src/features/signup/DocumentFirstDocumentsStep.tsx",
      "app/src/features/signup/documentFactRegistry.ts",
      "app/src/features/signup/documentFirstSignupModel.ts",
      "app/src/features/signup/presentation/FactReviewControls.tsx",
      "app/src/features/signup/presentation/FactTable.tsx",
      "app/src/features/dashboard/CustomerCorrectionHandoffPanel.tsx",
      "app/src/features/dashboard/DashboardPageShell.tsx",
      "app/src/features/dashboard/DashboardSidebar.tsx",
      "app/src/features/dashboard/customerCorrectionWorkspace.ts",
      "app/src/styles/components.css",
      "app/src/styles/layout.css",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "QUICK",
    expectedDurationMs: 1_500,
    expectedMarker: "CUSTOMER04C3C8_INTERACTION_SOURCE_SIDEBAR_Q01_Q15=PASS",
  }),
  check({
    id: "local-readonly-catalog",
    argv: [
      "node",
      "scripts/tools/enval-readonly-sql.mjs",
      "--proof-id",
      "enval-local-readonly-catalog",
    ],
    runner: "local-readonly-sql",
    domain: "local-postgres-catalog",
    applicablePaths: [
      "scripts/proofs/enval-local-readonly-catalog.proof.sql",
      "supabase/config.toml",
    ],
    safety: SAFETY.SAFE_LOCAL_READ,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: ["ENVAL PostgreSQL on configured loopback db.port"],
    expectedDurationMs: 500,
    expectedMarker: "ENVAL_LOCAL_READONLY_CATALOG_OK",
  }),
  check({
    id: "control-plane-foundation-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-run=node,psql",
      "scripts/proofs/platform-control-plane-foundation.proof.ts",
    ],
    domain: "isolated-control-plane-foundation",
    applicablePaths: [
      "platform/control-plane/**",
      "platform/runtime/tenant-resolution/**",
      "platform/runtime/presentation/presentation_brand_source.ts",
      "platform/runtime/presentation/presentation_source_composition.ts",
      "platform/runtime/presentation/adapters/platform_control_plane_presentation_v1.ts",
      "platform/runtime/presentation/adapters/static_presentation_config_v1.ts",
      "scripts/tools/enval-supabase-target.mjs",
      "scripts/proofs/platform-control-plane-foundation.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_CONTROL_PLANE_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "CONTROL_PLANE PostgreSQL on loopback port 56322",
      "TENANT_ENVAL PostgreSQL read-only on loopback port 54322",
    ],
    mutatesState: true,
    expectedDurationMs: 2_500,
    expectedMarker: "CONTROL_PLANE_FOUNDATION_Q01_Q18=PASS",
  }),
  check({
    id: "workforce-policy-foundation-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read=supabase/migrations/20260816160000_app_workforce_policy_foundation.sql",
      "--allow-run=docker",
      "scripts/proofs/app-workforce-policy-foundation.proof.ts",
    ],
    domain: "tenant-workforce-policy-foundation",
    applicablePaths: [
      "supabase/migrations/20260816160000_app_workforce_policy_foundation.sql",
      "scripts/proofs/app-workforce-policy-foundation.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available through local Docker",
      "disposable proof database create/drop authority",
    ],
    mutatesState: true,
    expectedDurationMs: 15_000,
    expectedMarker: "WORKFORCE_POLICY_FOUNDATION_Q01_Q14=PASS",
  }),
  check({
    id: "compliance-workforce-view-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read=supabase/migrations/20260816150000_app_current_baseline.sql,supabase/migrations/20260816160000_app_workforce_policy_foundation.sql,supabase/migrations/20260817120000_app_compliance_workforce_view.sql,platform/runtime/compliance/delivery_year_compliance.ts,platform/runtime/compliance/compliance_action_plan.ts,platform/runtime/compliance/compliance_worklist.ts",
      "--allow-run=docker",
      "scripts/proofs/app-compliance-workforce-view.proof.ts",
    ],
    domain: "tenant-compliance-workforce-view",
    applicablePaths: [
      "supabase/migrations/20260817120000_app_compliance_workforce_view.sql",
      "scripts/proofs/app-compliance-workforce-view.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available through local Docker",
      "disposable proof database create/drop authority",
    ],
    mutatesState: true,
    expectedDurationMs: 20_000,
    expectedMarker: "COMPLIANCE_WORKFORCE_VIEW_Q01_Q16=PASS",
  }),
  check({
    id: "compliance-source-event-ledger-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read=supabase/migrations/20260816150000_app_current_baseline.sql,supabase/migrations/20260816160000_app_workforce_policy_foundation.sql,supabase/migrations/20260817120000_app_compliance_workforce_view.sql,supabase/migrations/20260817160000_app_compliance_source_event_ledger.sql,platform/runtime/compliance/delivery_year_compliance.ts,platform/runtime/compliance/delivery_year_compliance_source_event.ts,platform/runtime/compliance/compliance_action_plan.ts,platform/runtime/compliance/compliance_worklist.ts",
      "--allow-run=docker",
      "scripts/proofs/app-compliance-source-event-ledger.proof.ts",
    ],
    domain: "tenant-compliance-source-event-ledger",
    applicablePaths: [
      "supabase/migrations/20260817160000_app_compliance_source_event_ledger.sql",
      "platform/runtime/compliance/delivery_year_compliance_source_event.ts",
      "scripts/proofs/app-compliance-source-event-ledger.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available through local Docker",
      "disposable proof database create/drop authority",
    ],
    mutatesState: true,
    expectedDurationMs: 20_000,
    expectedMarker: "COMPLIANCE_SOURCE_EVENT_LEDGER_Q01_Q15=PASS",
  }),
  check({
    id: "compliance-source-event-capture-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "scripts/proofs/app-compliance-source-event-capture.proof.ts",
      "--endpoint-only",
    ],
    domain: "tenant-compliance-source-event-capture",
    applicablePaths: [
      "supabase/functions/_shared/app_foundation.ts",
      "supabase/functions/_shared/app_workforce_authorization.ts",
      "supabase/functions/api-app-compliance-source-event/index.ts",
      "scripts/proofs/app-compliance-source-event-capture.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "COMPLIANCE_SOURCE_EVENT_CAPTURE_ENDPOINT=PASS",
  }),
  check({
    id: "compliance-source-event-capture-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "--allow-run=docker",
      "scripts/proofs/app-compliance-source-event-capture.proof.ts",
    ],
    domain: "tenant-compliance-source-event-capture",
    applicablePaths: [
      "supabase/migrations/20260817190000_app_compliance_source_event_capture.sql",
      "supabase/functions/_shared/app_foundation.ts",
      "supabase/functions/_shared/app_workforce_authorization.ts",
      "supabase/functions/api-app-compliance-source-event/index.ts",
      "scripts/proofs/app-compliance-source-event-capture.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available through local Docker",
      "disposable proof database create/drop authority",
      "active TENANT_ENVAL remains read-only",
    ],
    mutatesState: true,
    expectedDurationMs: 20_000,
    expectedMarker: "COMPLIANCE_SOURCE_EVENT_CAPTURE_Q01_Q13=PASS",
  }),
  check({
    id: "compliance-worklist-read-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "scripts/proofs/app-compliance-worklist-read.proof.ts",
      "--endpoint-only",
    ],
    domain: "tenant-compliance-worklist-read",
    applicablePaths: [
      "platform/runtime/compliance/delivery_year_compliance_calendar_registry.ts",
      "supabase/functions/_shared/app_compliance_worklist.ts",
      "supabase/functions/api-app-compliance-worklist/index.ts",
      "scripts/proofs/app-compliance-worklist-read.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "COMPLIANCE_WORKLIST_ENDPOINT=PASS",
  }),
  check({
    id: "compliance-worklist-read-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "--allow-run=docker",
      "scripts/proofs/app-compliance-worklist-read.proof.ts",
    ],
    domain: "tenant-compliance-worklist-read",
    applicablePaths: [
      "supabase/migrations/20260817210000_app_compliance_worklist_read.sql",
      "platform/runtime/compliance/delivery_year_compliance_calendar_registry.ts",
      "supabase/functions/_shared/app_compliance_worklist.ts",
      "supabase/functions/api-app-compliance-worklist/index.ts",
      "scripts/proofs/app-compliance-worklist-read.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available through local Docker",
      "disposable proof database create/drop authority",
      "active TENANT_ENVAL remains read-only",
    ],
    mutatesState: true,
    expectedDurationMs: 20_000,
    expectedMarker: "COMPLIANCE_WORKLIST_READ_Q01_Q20=PASS",
  }),
  check({
    id: "evidence-review-decision-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "scripts/proofs/app-evidence-review-decision.proof.ts",
    ],
    domain: "tenant-evidence-review-decision-runtime",
    applicablePaths: [
      "supabase/functions/_shared/app_evidence_review_case_detail.ts",
      "supabase/functions/api-app-evidence-review-decision/index.ts",
      "scripts/proofs/app-evidence-review-decision.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "EVIDENCE_REVIEW_DECISION_ENDPOINT_Q01_Q18=PASS",
  }),
  check({
    id: "evidence-review-foundation-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read=supabase/migrations",
      "scripts/proofs/app-evidence-review-foundation.proof.ts",
      "--source-only",
    ],
    domain: "tenant-evidence-review-foundation",
    applicablePaths: [
      "supabase/migrations/20260817230000_app_evidence_review_foundation.sql",
      "supabase/migrations/20260818210000_app_evidence_review_correction_details.sql",
      "supabase/migrations/20260818220000_app_evidence_review_case_detail_decide_affordance.sql",
      "scripts/proofs/app-evidence-review-foundation.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 500,
    expectedMarker: "EVIDENCE_REVIEW_FOUNDATION_SOURCE=PASS",
  }),
  check({
    id: "evidence-review-foundation-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read=supabase/migrations",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "--allow-run=docker",
      "scripts/proofs/app-evidence-review-foundation.proof.ts",
    ],
    domain: "tenant-evidence-review-foundation",
    applicablePaths: [
      "supabase/migrations/20260817230000_app_evidence_review_foundation.sql",
      "supabase/migrations/20260818210000_app_evidence_review_correction_details.sql",
      "supabase/migrations/20260818220000_app_evidence_review_case_detail_decide_affordance.sql",
      "supabase/functions/_shared/app_evidence_review_case_detail.ts",
      "supabase/functions/api-app-evidence-review-decision/index.ts",
      "scripts/proofs/app-evidence-review-decision.proof.ts",
      "scripts/proofs/app-evidence-review-foundation.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available through local Docker",
      "disposable proof database create/drop authority",
      "active TENANT_ENVAL remains read-only",
    ],
    mutatesState: true,
    expectedDurationMs: 20_000,
    expectedMarker: "EVIDENCE_REVIEW_FOUNDATION_Q01_Q14=PASS",
  }),
  check({
    id: "evidence-review-worklist-read-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "scripts/proofs/app-evidence-review-worklist-read.proof.ts",
      "--endpoint-only",
    ],
    domain: "tenant-evidence-review-worklist-read",
    applicablePaths: [
      "supabase/functions/_shared/app_evidence_review_overall_status.ts",
      "supabase/functions/_shared/app_evidence_review_worklist.ts",
      "supabase/functions/api-app-evidence-review-worklist/index.ts",
      "scripts/proofs/app-evidence-review-worklist-read.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "EVIDENCE_REVIEW_WORKLIST_ENDPOINT=PASS",
  }),
  check({
    id: "evidence-review-worklist-read-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "--allow-run=docker",
      "scripts/proofs/app-evidence-review-worklist-read.proof.ts",
    ],
    domain: "tenant-evidence-review-worklist-read",
    applicablePaths: [
      "supabase/migrations/20260818090000_app_evidence_review_worklist_read.sql",
      "supabase/migrations/20260818210000_app_evidence_review_correction_details.sql",
      "supabase/migrations/20260819120000_app_evidence_review_worklist_fact_cutover.sql",
      "supabase/migrations/20260819123000_app_evidence_review_worklist_activity_fix.sql",
      "supabase/migrations/20260819160000_app_evidence_review_overall_status.sql",
      "supabase/functions/_shared/app_evidence_review_overall_status.ts",
      "supabase/functions/_shared/app_evidence_review_worklist.ts",
      "supabase/functions/api-app-evidence-review-worklist/index.ts",
      "scripts/proofs/app-evidence-review-worklist-read.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available through local Docker",
      "disposable proof database create/drop authority",
      "active TENANT_ENVAL remains read-only",
    ],
    mutatesState: true,
    expectedDurationMs: 25_000,
    expectedMarker: "EVIDENCE_REVIEW_WORKLIST_READ_Q01_Q14=PASS",
  }),
  check({
    id: "signup-resolution-provenance-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--unstable-sloppy-imports",
      "scripts/proofs/app-signup-resolution-provenance.proof.ts",
    ],
    domain: "signup-signing-resolution-provenance",
    applicablePaths: [
      "app/src/features/signup/presentation/factPresentationModel.ts",
      "app/src/features/signup/signing/canonicalSigningFacts.ts",
      "app/src/features/signup/signing/signingIntent.ts",
      "app/src/features/signup/DocumentFirstSigningSummary.tsx",
      "app/src/features/signup/signupSigningClient.ts",
      "supabase/functions/_shared/signup_resolution_provenance.ts",
      "supabase/functions/api-app-signup-signing-finalize/index.ts",
      "supabase/migrations/20260818180000_app_signup_resolution_provenance_projection.sql",
      "scripts/proofs/app-signup-resolution-provenance.proof.ts",
      "scripts/proofs/app-signup-signing-runtime.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "REVIEW10_SERVER_DERIVED_HASHED_PROVENANCE=PASS",
  }),
  check({
    id: "parser-observation-foundation-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ENVIRONMENT,ENV,APP_ENV,ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "scripts/proofs/app-parser-observation-foundation.proof.ts",
    ],
    domain: "provider-neutral-document-parser",
    applicablePaths: [
      "platform/runtime/document-parsing/**",
      "app/src/features/invoice-analysis/**",
      "app/src/features/signup/DocumentFirstDocumentsStep.tsx",
      "app/src/features/signup/documentFactRegistry.ts",
      "app/src/features/signup/signupQuarantineUploadClient.ts",
      "supabase/functions/_shared/app_document_parser_pdf_adapter.ts",
      "supabase/functions/_shared/app_parser_observation_persistence.ts",
      "supabase/functions/_shared/signup_quarantine.ts",
      "supabase/functions/api-app-signup-upload-confirm/index.ts",
      "scripts/proofs/app-parser-observation-foundation.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "PARSER_SHARED_CORE_Q01_Q14=PASS",
  }),
  check({
    id: "document-parser-normalization-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--unstable-sloppy-imports",
      "scripts/proofs/app-document-parser-normalization.proof.ts",
    ],
    domain: "shared-document-parser-canonical-value-normalization",
    applicablePaths: [
      "platform/runtime/document-parsing/document_parser_core.ts",
      "app/src/features/invoice-analysis/invoicePdfParserAdapter.ts",
      "supabase/functions/_shared/app_document_parser_pdf_adapter.ts",
      "scripts/proofs/app-document-parser-normalization.proof.ts",
      "scripts/proofs/app-document-parser-fresh-installation.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "CUSTOMER04C3C3_PARSER_NORMALIZATION=PASS",
  }),
  check({
    id: "parser-observation-foundation-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ENVIRONMENT,ENV,APP_ENV,ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "--allow-run=docker",
      "scripts/proofs/app-parser-observation-foundation.proof.ts",
      "--local",
    ],
    domain: "tenant-parser-observation-persistence",
    applicablePaths: [
      "supabase/migrations/20260820180000_app_parser_observation_foundation.sql",
      "supabase/migrations/20260820183000_app_parser_observation_acl_fix.sql",
      "supabase/functions/_shared/app_parser_observation_persistence.ts",
      "supabase/functions/api-app-signup-upload-confirm/index.ts",
      "scripts/proofs/app-parser-observation-foundation.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available through local Docker",
      "active migration applied; proof writes rolled back",
    ],
    mutatesState: true,
    expectedDurationMs: 2_000,
    expectedMarker: "PARSER_OBSERVATION_LOCAL_Q15_Q22=PASS",
  }),
  check({
    id: "parser-current-intake-served-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-run=supabase",
      "--allow-net=127.0.0.1,localhost",
      "scripts/proofs/app-parser-current-intake-served.proof.ts",
    ],
    domain: "served-current-energy-intake-parser",
    applicablePaths: [
      "platform/runtime/document-parsing/**",
      "app/src/features/invoice-analysis/parserObservationProjection.ts",
      "app/src/features/signup/DocumentFirstDocumentsStep.tsx",
      "app/src/features/signup/signupQuarantineUploadClient.ts",
      "supabase/functions/_shared/app_document_parser_pdf_adapter.ts",
      "supabase/functions/_shared/app_parser_observation_persistence.ts",
      "supabase/functions/_shared/signup_quarantine.ts",
      "supabase/functions/api-app-signup-upload-confirm/index.ts",
      "supabase/migrations/20260820180000_app_parser_observation_foundation.sql",
      "scripts/proofs/app-parser-current-intake-served.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "LOCAL01 TENANT_ENVAL Kong, Storage, Edge and PostgreSQL",
      "disposable example.invalid intake fixture",
    ],
    mutatesState: true,
    expectedDurationMs: 5_000,
    expectedMarker: "SERVED_CURRENT_INTAKE_PARSER_Q01_Q12=PASS",
  }),
  check({
    id: "evidence-review-case-detail-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "scripts/proofs/app-evidence-review-case-detail.proof.ts",
      "--endpoint-only",
    ],
    domain: "tenant-evidence-review-case-detail",
    applicablePaths: [
      "supabase/migrations/20260819090000_app_evidence_review_current_round_read.sql",
      "supabase/migrations/20260819160000_app_evidence_review_overall_status.sql",
      "supabase/migrations/20260819210000_app_evidence_review_correction_publish_affordance.sql",
      "supabase/functions/_shared/app_evidence_review_overall_status.ts",
      "supabase/functions/_shared/app_evidence_review_case_detail.ts",
      "supabase/functions/api-app-evidence-review-case-detail/index.ts",
      "scripts/proofs/app-evidence-review-case-detail.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "EVIDENCE_REVIEW_CASE_DETAIL_ENDPOINT=PASS",
  }),
  check({
    id: "customer-information-request-client-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--no-lock",
      "--allow-read",
      "--allow-env=NODE_ENV",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "app/src/features/customer-information-request/customerInformationRequest.proof.tsx",
    ],
    domain: "customer-information-request-client-contract",
    applicablePaths: [
      "app/src/features/customer-information-request/**",
      "app/src/features/dashboard/ActivePrivateDashboard.tsx",
      "app/src/features/dashboard/dashboardReadClient.ts",
      "app/src/features/dashboard/dashboardStatusPresentation.ts",
      "app/src/features/dashboard/dashboardTypes.ts",
      "app/src/features/evidence-review/EvidenceReviewCaseDetailPage.tsx",
      "app/src/features/evidence-review/evidenceReviewDetailClient.ts",
      "supabase/functions/_shared/app_customer_information_request.ts",
      "supabase/functions/_shared/app_workflow_email_context.ts",
      "supabase/functions/api-app-customer-information-request/index.ts",
      "supabase/functions/api-app-evidence-review-correction-publish/index.ts",
      "supabase/migrations/20260912100926_app_customer_information_request_integration_guards.sql",
      "supabase/migrations/20260912121643_app_customer_information_request_history_v1.sql",
      "supabase/migrations/20260913192155_information_request_email_v1.sql",
      "supabase/migrations/20260914153311_customer_information_request_terminal_at_v1.sql",
      "supabase/migrations/20260921130223_brand_configuration_workflow_email_sender_snapshot_v1.sql",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "CUSTOMER_INFORMATION_REQUEST_CLIENT_Q01_Q04=PASS",
    compilerAuthority: COMPILER_AUTHORITY.APP_TYPESCRIPT,
  }),
  check({
    id: "local-runtime-pure",
    argv: ["node", "scripts/proofs/enval-local-dev.proof.mjs"],
    domain: "local-development-runtime-contract",
    applicablePaths: [
      "scripts/proofs/enval-local-dev.proof.mjs",
      "scripts/tools/enval-local-dev.mjs",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "LOCAL_RUNTIME06_REGRESSION=PASS",
  }),
  check({
    id: "customer-information-request-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-run=docker",
      "scripts/proofs/app-customer-information-request.proof.ts",
    ],
    domain: "customer-information-request-database-contract",
    applicablePaths: [
      "scripts/proofs/app-customer-information-request.proof.ts",
      "supabase/migrations/20260911151223_app_customer_information_request_v1.sql",
      "supabase/migrations/20260912100926_app_customer_information_request_integration_guards.sql",
      "supabase/migrations/20260912121643_app_customer_information_request_history_v1.sql",
      "supabase/migrations/20260913192155_information_request_email_v1.sql",
      "supabase/migrations/20260914153311_customer_information_request_terminal_at_v1.sql",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available through local Docker",
      "disposable proof database create/drop authority",
    ],
    mutatesState: true,
    expectedDurationMs: 20_000,
    expectedMarker: "CUSTOMER_INFORMATION_REQUEST_DB_Q01_Q10=PASS",
  }),
  check({
    id: "information-request-email-context-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--no-lock",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN,ENVAL_WORKFLOW_EMAIL_PORTAL_ORIGIN",
      "scripts/proofs/app-information-request-email-context.proof.ts",
    ],
    domain: "information-request-email-server-context",
    applicablePaths: [
      "app/src/features/dashboard/ActivePrivateDashboard.tsx",
      "scripts/proofs/app-information-request-email-context.proof.ts",
      "supabase/functions/_shared/app_control_plane_runtime_reader.ts",
      "supabase/functions/_shared/app_workflow_email_context.ts",
      "supabase/functions/api-app-customer-information-request/index.ts",
      "supabase/functions/api-app-presentation-bootstrap/index.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "INFORMATION_REQUEST_EMAIL_CONTEXT_Q01_Q07=PASS",
    compilerAuthority: COMPILER_AUTHORITY.APP_TYPESCRIPT,
  }),
  check({
    id: "workflow-email-worker-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env",
      "--allow-net=127.0.0.1",
      "scripts/proofs/app-workflow-email-worker.proof.ts",
    ],
    domain: "workflow-email-worker-contract",
    applicablePaths: [
      "scripts/proofs/app-workflow-email-worker.proof.ts",
      "supabase/functions/_shared/local_mailpit_smtp.ts",
      "supabase/functions/_shared/workflow_email_transport.ts",
      "supabase/functions/_shared/workflow_email_worker.ts",
      "supabase/functions/workflow-email-worker/index.ts",
      "supabase/migrations/20260913192155_information_request_email_v1.sql",
      "supabase/migrations/20260921130223_brand_configuration_workflow_email_sender_snapshot_v1.sql",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "WORKFLOW_EMAIL_WORKER_Q01_Q10=PASS",
  }),
  check({
    id: "workflow-email-foundation-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-run=docker",
      "scripts/proofs/app-workflow-email-foundation.proof.ts",
    ],
    domain: "workflow-email-database-contract",
    applicablePaths: [
      "scripts/proofs/app-workflow-email-foundation.proof.ts",
      "supabase/migrations/20260913161612_app_workflow_email_foundation_v1.sql",
      "supabase/migrations/20260913192155_information_request_email_v1.sql",
      "supabase/migrations/20260921130223_brand_configuration_workflow_email_sender_snapshot_v1.sql",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available through local Docker",
      "disposable proof database create/drop authority",
    ],
    mutatesState: true,
    expectedDurationMs: 20_000,
    expectedMarker: "WORKFLOW_EMAIL_FOUNDATION_DB_Q01_Q14=PASS",
  }),
  check({
    id: "evidence-fact-review-round-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "scripts/proofs/app-evidence-fact-review-round.proof.ts",
    ],
    domain: "tenant-evidence-fact-review-round",
    applicablePaths: [
      "supabase/migrations/20260818230000_app_evidence_fact_review_rounds.sql",
      "supabase/migrations/20260819090000_app_evidence_review_current_round_read.sql",
      "supabase/migrations/20260819160000_app_evidence_review_overall_status.sql",
      "supabase/migrations/20260819210000_app_evidence_review_correction_publish_affordance.sql",
      "supabase/functions/_shared/app_evidence_review_overall_status.ts",
      "supabase/functions/_shared/app_evidence_review_case_detail.ts",
      "supabase/functions/api-app-evidence-review-case-detail/index.ts",
      "supabase/functions/api-app-evidence-review-round-finalize/index.ts",
      "scripts/proofs/app-evidence-review-case-detail.proof.ts",
      "scripts/proofs/app-evidence-fact-review-round.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "EVIDENCE_FACT_REVIEW_ROUND_ENDPOINT_Q01_Q12=PASS",
  }),
  check({
    id: "evidence-review-case-detail-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "--allow-run=docker",
      "scripts/proofs/app-evidence-review-case-detail.proof.ts",
    ],
    domain: "tenant-evidence-review-case-detail",
    applicablePaths: [
      "supabase/migrations/20260818120000_app_evidence_review_case_detail_read.sql",
      "supabase/migrations/20260818180000_app_signup_resolution_provenance_projection.sql",
      "supabase/migrations/20260818210000_app_evidence_review_correction_details.sql",
      "supabase/migrations/20260818220000_app_evidence_review_case_detail_decide_affordance.sql",
      "supabase/migrations/20260818230000_app_evidence_fact_review_rounds.sql",
      "supabase/migrations/20260819090000_app_evidence_review_current_round_read.sql",
      "supabase/migrations/20260819160000_app_evidence_review_overall_status.sql",
      "supabase/functions/_shared/app_evidence_review_overall_status.ts",
      "supabase/functions/_shared/app_evidence_review_case_detail.ts",
      "supabase/functions/api-app-evidence-review-case-detail/index.ts",
      "supabase/functions/api-app-evidence-review-round-finalize/index.ts",
      "scripts/proofs/app-evidence-review-case-detail.proof.ts",
      "scripts/proofs/app-evidence-fact-review-round.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available through local Docker",
      "disposable proof database create/drop authority",
      "active TENANT_ENVAL remains read-only during proof",
    ],
    mutatesState: true,
    expectedDurationMs: 25_000,
    expectedMarker: "EVIDENCE_REVIEW_CASE_DETAIL_Q01_Q19=PASS",
  }),
  check({
    id: "evidence-fact-review-round-served-local",
    argv: [
      "node",
      "scripts/proofs/app-evidence-fact-review-round-served.proof.mjs",
    ],
    domain: "tenant-evidence-fact-review-round-served-runtime",
    applicablePaths: [
      "app/src/features/dashboard/ActivePrivateDashboard.tsx",
      "app/src/features/dashboard/CustomerCorrectionHandoffPanel.tsx",
      "app/src/features/dashboard/DashboardPageShell.tsx",
      "app/src/features/dashboard/customerCorrectionHandoffClient.ts",
      "app/src/features/dashboard/customerCorrectionHandoff.proof.tsx",
      "app/src/features/dashboard/customerCorrectionReplacementUpload.ts",
      "app/src/features/dashboard/customerCorrectionWorkspace.ts",
      "app/src/features/documents/DocumentFactMatrix.tsx",
      "app/src/features/documents/DocumentFactMatrix.proof.tsx",
      "app/src/features/dashboard/useCustomerCorrectionHandoff.ts",
      "app/src/features/documents/documentUploadTransport.ts",
      "app/src/features/signup/signing/SignerPanel.tsx",
      "app/src/features/signup/signing/methods/typedNameOtpV1.ts",
      "scripts/tools/enval-local-dev.mjs",
      "supabase/functions/_shared/app_evidence_review_correction_handoff.ts",
      "supabase/functions/api-app-customer-correction-handoff/index.ts",
      "supabase/functions/api-app-evidence-review-correction-publish/index.ts",
      "supabase/functions/api-app-evidence-review-round-finalize/index.ts",
      "supabase/functions/_shared/app_customer_correction_submission.ts",
      "supabase/functions/_shared/signing_otp_transport.ts",
      "supabase/functions/api-app-customer-correction-signing-challenge/index.ts",
      "supabase/functions/api-app-customer-correction-signing-finalize/index.ts",
      "supabase/functions/api-app-evidence-review-worklist/index.ts",
      "supabase/migrations/20260820090000_app_customer_correction_submissions.sql",
      "supabase/migrations/20260820180000_app_parser_observation_foundation.sql",
      "supabase/migrations/20260820120000_app_customer_correction_handoff_contract_v2.sql",
      "supabase/migrations/20260820150000_app_customer_correction_signer_authority.sql",
      "scripts/proofs/app-customer-correction-submission.proof.ts",
      "scripts/proofs/app-customer-correction-handoff-contract-v2.proof.ts",
      "scripts/proofs/app-evidence-fact-review-round-served.proof.mjs",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL Kong, Auth, Edge and PostgreSQL available locally",
      "served functions started through scripts/tools/enval-local-dev.mjs",
      "isolated active-local fixture is removed and pilot remains unchanged",
    ],
    mutatesState: true,
    expectedDurationMs: 15_000,
    expectedMarker: "EVIDENCE_FACT_REVIEW_SERVED_Q01_Q20=PASS",
  }),
  check({
    id: "correction-handoff-supersession-pure",
    argv: [
      "deno",
      "run",
      "--allow-read",
      "--allow-env",
      "scripts/proofs/app-correction-handoff-supersession.proof.ts",
    ],
    domain: "tenant-correction-handoff-supersession",
    applicablePaths: [
      "supabase/functions/api-app-evidence-review-correction-supersede/index.ts",
      "supabase/migrations/20260820210000_app_correction_handoff_supersession.sql",
      "scripts/proofs/app-correction-handoff-supersession.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "CUSTOMER04C3A_Q01_Q09=PASS",
  }),
  check({
    id: "correction-handoff-supersession-served-local",
    argv: [
      "node",
      "scripts/proofs/app-correction-handoff-supersession-served.proof.mjs",
    ],
    domain: "tenant-correction-handoff-supersession-served-runtime",
    applicablePaths: [
      "supabase/functions/api-app-evidence-review-correction-supersede/index.ts",
      "supabase/migrations/20260820210000_app_correction_handoff_supersession.sql",
      "scripts/proofs/app-correction-handoff-supersession.proof.ts",
      "scripts/proofs/app-correction-handoff-supersession-served.proof.mjs",
      "scripts/proofs/app-evidence-fact-review-round-served.proof.mjs",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL Kong, Auth, Edge and PostgreSQL available locally",
      "disposable admin, customer, case and handoff fixtures are removed",
      "real pilot remains read-only and unchanged",
    ],
    mutatesState: true,
    expectedDurationMs: 20_000,
    expectedMarker: "CUSTOMER04C3A_SERVED_Q01_Q11=PASS",
  }),
  check({
    id: "customer-document-active-source-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--unstable-sloppy-imports",
      "scripts/proofs/customer-document-active-source.proof.ts",
    ],
    domain: "customer-document-active-source-projection",
    applicablePaths: [
      "app/src/features/documents/customerDocumentFactActiveSourceProjector.ts",
      "app/src/features/signup/documentFactRegistry.ts",
      "app/src/features/signup/documentSemanticProjector.ts",
      "app/src/features/signup/presentation/FactReviewControls.tsx",
      "app/src/features/signup/presentation/FactTable.tsx",
      "app/src/features/dashboard/CustomerCorrectionHandoffPanel.tsx",
      "app/src/features/dashboard/customerCorrectionWorkspace.ts",
      "scripts/proofs/customer-document-active-source.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "DISPLAY_POLICY_BASE_SOURCE_OBJECT_PARITY=PASS",
  }),
  check({
    id: "customer-correction-withdrawal-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "--unstable-sloppy-imports",
      "scripts/proofs/app-customer-correction-withdrawal.proof.ts",
    ],
    domain: "customer-correction-candidate-withdrawal",
    applicablePaths: [
      "supabase/functions/_shared/app_customer_correction_replacement.ts",
      "supabase/functions/api-app-customer-correction-upload-remove/index.ts",
      "supabase/migrations/20260823120000_app_customer_correction_candidate_selection.sql",
      "supabase/migrations/20260823123000_app_customer_correction_resolution_null_projection.sql",
      "supabase/migrations/20260823130000_app_customer_correction_withdraw_scope_fix.sql",
      "supabase/migrations/20260823133000_app_customer_correction_handoff_null_fix.sql",
      "supabase/migrations/20260823134500_app_customer_correction_handoff_preflight_fix.sql",
      "scripts/proofs/app-customer-correction-withdrawal.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "WITHDRAWAL_APPEND_ONLY=PASS",
  }),
  check({
    id: "customer-correction-replacement-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "--unstable-sloppy-imports",
      "scripts/proofs/app-customer-correction-replacement.proof.ts",
    ],
    domain: "customer-correction-replacement-staging",
    applicablePaths: [
      "platform/runtime/document-parsing/document_parser_contract.ts",
      "supabase/functions/_shared/app_customer_correction_replacement.ts",
      "supabase/functions/_shared/app_evidence_review_correction_handoff.ts",
      "supabase/functions/_shared/app_parser_observation_persistence.ts",
      "supabase/functions/_shared/signup_quarantine.ts",
      "supabase/functions/api-app-customer-correction-upload-url/index.ts",
      "supabase/functions/api-app-customer-correction-upload-confirm/index.ts",
      "supabase/functions/api-app-customer-correction-upload-remove/index.ts",
      "supabase/functions/api-app-customer-correction-handoff/index.ts",
      "supabase/migrations/20260820220000_app_customer_correction_replacement_staging.sql",
      "supabase/migrations/20260823120000_app_customer_correction_candidate_selection.sql",
      "supabase/migrations/20260823123000_app_customer_correction_resolution_null_projection.sql",
      "supabase/migrations/20260823130000_app_customer_correction_withdraw_scope_fix.sql",
      "supabase/migrations/20260823133000_app_customer_correction_handoff_null_fix.sql",
      "supabase/migrations/20260823134500_app_customer_correction_handoff_preflight_fix.sql",
      "scripts/proofs/app-customer-correction-withdrawal.proof.ts",
      "scripts/proofs/app-customer-correction-replacement.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "CUSTOMER04C3B1_PURE_Q01_Q24=PASS",
  }),
  check({
    id: "customer-correction-replacement-served-local",
    argv: [
      "node",
      "scripts/proofs/app-customer-correction-replacement-served.proof.mjs",
    ],
    domain: "customer-correction-replacement-served-runtime",
    applicablePaths: [
      "platform/runtime/document-parsing/document_parser_contract.ts",
      "supabase/functions/_shared/app_customer_correction_replacement.ts",
      "supabase/functions/_shared/app_evidence_review_correction_handoff.ts",
      "supabase/functions/_shared/app_parser_observation_persistence.ts",
      "supabase/functions/_shared/signup_quarantine.ts",
      "supabase/functions/api-app-customer-correction-upload-url/index.ts",
      "supabase/functions/api-app-customer-correction-upload-confirm/index.ts",
      "supabase/functions/api-app-customer-correction-upload-remove/index.ts",
      "supabase/functions/api-app-customer-correction-handoff/index.ts",
      "supabase/migrations/20260820220000_app_customer_correction_replacement_staging.sql",
      "supabase/migrations/20260823120000_app_customer_correction_candidate_selection.sql",
      "supabase/migrations/20260823123000_app_customer_correction_resolution_null_projection.sql",
      "supabase/migrations/20260823130000_app_customer_correction_withdraw_scope_fix.sql",
      "supabase/migrations/20260823133000_app_customer_correction_handoff_null_fix.sql",
      "supabase/migrations/20260823134500_app_customer_correction_handoff_preflight_fix.sql",
      "scripts/proofs/app-customer-correction-withdrawal.proof.ts",
      "scripts/proofs/app-customer-correction-replacement.proof.ts",
      "scripts/proofs/app-customer-correction-replacement-served.proof.mjs",
      "scripts/proofs/app-correction-handoff-supersession-served.proof.mjs",
      "scripts/proofs/app-evidence-fact-review-round-served.proof.mjs",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL Kong, Auth, Edge, Storage and PostgreSQL available locally",
      "disposable customer, workforce, case, handoff and private object fixtures are removed",
      "real pilot remains read-only and unchanged",
    ],
    mutatesState: true,
    expectedDurationMs: 20_000,
    expectedMarker: "CUSTOMER04C3B1_SERVED_Q01_Q09=PASS",
  }),
  check({
    id: "customer-correction-resolution-routing-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "scripts/proofs/app-customer-correction-resolution-routing.proof.ts",
    ],
    domain: "customer-correction-resolution-routing",
    applicablePaths: [
      "app/src/features/dashboard/CustomerCorrectionHandoffPanel.tsx",
      "app/src/features/dashboard/customerCorrectionHandoffClient.ts",
      "app/src/features/dashboard/customerCorrectionWorkspace.ts",
      "supabase/functions/_shared/app_customer_correction_resolution.ts",
      "supabase/functions/_shared/app_customer_correction_submission.ts",
      "supabase/functions/api-app-customer-correction-signing-challenge/index.ts",
      "supabase/functions/api-app-customer-correction-signing-finalize/index.ts",
      "supabase/migrations/20260824100000_app_customer_correction_resolution_routing.sql",
      "scripts/proofs/app-customer-correction-resolution-routing.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "CUSTOMER04C3C10_Q01_Q12=PASS",
  }),
  check({
    id: "customer-correction-resolution-routing-served",
    argv: [
      "node",
      "scripts/proofs/app-customer-correction-resolution-routing-served.proof.mjs",
    ],
    domain: "customer-correction-resolution-routing",
    applicablePaths: [
      "supabase/functions/_shared/app_customer_correction_resolution.ts",
      "supabase/functions/_shared/app_customer_correction_submission.ts",
      "supabase/functions/api-app-customer-correction-signing-challenge/index.ts",
      "supabase/functions/api-app-customer-correction-signing-finalize/index.ts",
      "supabase/migrations/20260824100000_app_customer_correction_resolution_routing.sql",
      "scripts/proofs/app-customer-correction-resolution-routing.proof.ts",
      "scripts/proofs/app-customer-correction-resolution-routing-served.proof.mjs",
      "scripts/proofs/app-evidence-fact-review-round-served.proof.mjs",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL Kong, Auth, Edge, Storage and PostgreSQL available locally",
      "disposable routing cases, Auth users and private objects are removed",
      "real pilot remains read-only and unchanged",
    ],
    mutatesState: true,
    expectedDurationMs: 45_000,
    expectedMarker: "CUSTOMER04C3C10_SERVED_Q01_Q07=PASS",
  }),
  check({
    id: "customer-correction-document-finalization-pure",
    argv: [
      "deno",
      "run",
      "--allow-read",
      "scripts/proofs/app-customer-correction-document-finalization.proof.ts",
    ],
    domain: "customer-correction-document-finalization",
    applicablePaths: [
      "supabase/functions/_shared/app_customer_correction_submission.ts",
      "supabase/functions/api-app-customer-correction-signing-challenge/index.ts",
      "supabase/functions/api-app-customer-correction-signing-finalize/index.ts",
      "supabase/functions/api-app-customer-correction-upload-confirm/index.ts",
      "supabase/migrations/20260821100000_app_customer_correction_document_finalization.sql",
      "scripts/proofs/app-customer-correction-document-finalization.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "CUSTOMER04C3B2_Q01_Q24=PASS",
  }),
  check({
    id:
      "customer-correction-document-finalization-finalize-reentry-served-local",
    argv: [
      "node",
      "scripts/proofs/app-customer-correction-document-finalization-finalize-reentry-served.proof.mjs",
    ],
    domain: "customer-correction-document-finalization-served-runtime",
    applicablePaths: [
      "supabase/functions/_shared/app_customer_correction_submission.ts",
      "supabase/functions/api-app-customer-correction-signing-challenge/index.ts",
      "supabase/functions/api-app-customer-correction-signing-finalize/index.ts",
      "supabase/functions/api-app-customer-correction-upload-confirm/index.ts",
      "supabase/migrations/20260821100000_app_customer_correction_document_finalization.sql",
      "scripts/proofs/app-customer-correction-document-finalization.proof.ts",
      "scripts/proofs/app-customer-correction-document-finalization-served.proof.mjs",
      "scripts/proofs/app-customer-correction-document-finalization-finalize-reentry-served.proof.mjs",
      "scripts/proofs/app-customer-correction-replacement-served.proof.mjs",
      "scripts/proofs/app-correction-handoff-supersession-served.proof.mjs",
      "scripts/proofs/app-evidence-fact-review-round-served.proof.mjs",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL Kong, Auth, Edge, Storage and PostgreSQL available locally",
      "disposable signed correction fixtures and private objects are removed",
      "real pilot remains read-only and unchanged",
    ],
    mutatesState: true,
    expectedDurationMs: 30_000,
    expectedMarker: "CUSTOMER04C3B2_SERVED_FINALIZE_REENTRY=PASS",
  }),
  check({
    id:
      "customer-correction-document-finalization-upload-candidate-served-local",
    argv: [
      "node",
      "scripts/proofs/app-customer-correction-document-finalization-upload-candidate-served.proof.mjs",
    ],
    domain:
      "customer-correction-document-finalization-upload-candidate-runtime",
    applicablePaths: [
      "supabase/functions/api-app-customer-correction-upload-confirm/index.ts",
      "scripts/proofs/app-customer-correction-document-finalization-served.proof.mjs",
      "scripts/proofs/app-customer-correction-document-finalization-upload-candidate-served.proof.mjs",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL Kong, Auth, Edge, Storage and PostgreSQL available locally",
      "disposable upload fixtures and private objects are removed",
    ],
    mutatesState: true,
    expectedDurationMs: 15_000,
    expectedMarker: "CUSTOMER04C3B2_SERVED_UPLOAD_CANDIDATE=PASS",
  }),
  check({
    id:
      "customer-correction-document-finalization-challenge-replay-served-local",
    argv: [
      "node",
      "scripts/proofs/app-customer-correction-document-finalization-challenge-replay-served.proof.mjs",
    ],
    domain:
      "customer-correction-document-finalization-challenge-replay-runtime",
    applicablePaths: [
      "supabase/functions/api-app-customer-correction-signing-challenge/index.ts",
      "scripts/proofs/app-customer-correction-document-finalization-served.proof.mjs",
      "scripts/proofs/app-customer-correction-document-finalization-challenge-replay-served.proof.mjs",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL Kong, Auth, Edge, Storage and PostgreSQL available locally",
      "disposable challenge fixtures and private objects are removed",
    ],
    mutatesState: true,
    expectedDurationMs: 15_000,
    expectedMarker: "CUSTOMER04C3B2_SERVED_CHALLENGE_REPLAY=PASS",
  }),
  check({
    id:
      "customer-correction-document-finalization-transactional-rollback-served-local",
    argv: [
      "node",
      "scripts/proofs/app-customer-correction-document-finalization-transactional-rollback-served.proof.mjs",
    ],
    domain:
      "customer-correction-document-finalization-transactional-rollback-runtime",
    applicablePaths: [
      "supabase/functions/api-app-customer-correction-signing-finalize/index.ts",
      "scripts/proofs/app-customer-correction-document-finalization-served.proof.mjs",
      "scripts/proofs/app-customer-correction-document-finalization-transactional-rollback-served.proof.mjs",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL Kong, Auth, Edge, Storage and PostgreSQL available locally",
      "disposable rollback fixtures and private objects are removed",
    ],
    mutatesState: true,
    expectedDurationMs: 20_000,
    expectedMarker: "CUSTOMER04C3B2_SERVED_TRANSACTIONAL_ROLLBACK=PASS",
  }),
  check({
    id: "customer-correction-submission-pure",
    argv: [
      "deno",
      "run",
      "--allow-read",
      "scripts/proofs/app-customer-correction-submission.proof.ts",
      "--source-only",
    ],
    domain: "tenant-customer-correction-submission",
    applicablePaths: [
      "supabase/functions/_shared/app_customer_correction_submission.ts",
      "supabase/functions/_shared/signing_otp_transport.ts",
      "supabase/functions/api-app-customer-correction-signing-challenge/index.ts",
      "supabase/functions/api-app-customer-correction-signing-finalize/index.ts",
      "supabase/functions/api-app-evidence-review-worklist/index.ts",
      "supabase/migrations/20260820090000_app_customer_correction_submissions.sql",
      "supabase/migrations/20260820120000_app_customer_correction_handoff_contract_v2.sql",
      "supabase/migrations/20260820150000_app_customer_correction_signer_authority.sql",
      "scripts/proofs/app-customer-correction-submission.proof.ts",
      "scripts/proofs/app-customer-correction-handoff-contract-v2.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "CUSTOMER_CORRECTION_SOURCE_ONLY=PASS",
  }),
  check({
    id: "customer-correction-submission-local-read",
    argv: [
      "deno",
      "run",
      "--allow-read",
      "--allow-run=psql",
      "scripts/proofs/app-customer-correction-submission.proof.ts",
    ],
    domain: "tenant-customer-correction-submission-local-read",
    applicablePaths: [
      "supabase/functions/_shared/app_customer_correction_submission.ts",
      "supabase/functions/api-app-customer-correction-signing-challenge/index.ts",
      "supabase/functions/api-app-customer-correction-signing-finalize/index.ts",
      "supabase/migrations/20260820090000_app_customer_correction_submissions.sql",
      "supabase/migrations/20260820120000_app_customer_correction_handoff_contract_v2.sql",
      "supabase/migrations/20260820150000_app_customer_correction_signer_authority.sql",
      "scripts/proofs/app-customer-correction-submission.proof.ts",
      "scripts/proofs/app-customer-correction-handoff-contract-v2.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_READ,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available on the guarded local port",
      "read-only pilot scope probe; no OTP or finalization",
    ],
    expectedDurationMs: 2_000,
    expectedMarker: "CUSTOMER_CORRECTION_PILOT_SUBMISSIONS_ZERO=PASS",
  }),
  check({
    id: "evidence-review-correction-handoff-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "scripts/proofs/app-evidence-review-correction-handoff.proof.ts",
      "--source-only",
    ],
    domain: "tenant-evidence-review-correction-handoff",
    applicablePaths: [
      "supabase/migrations/20260819190000_app_evidence_review_correction_handoff.sql",
      "supabase/migrations/20260819220000_app_evidence_review_correction_publication_target_fix.sql",
      "supabase/migrations/20260820120000_app_customer_correction_handoff_contract_v2.sql",
      "supabase/migrations/20260820150000_app_customer_correction_signer_authority.sql",
      "supabase/migrations/20260822120000_app_customer_correction_handoff_fact_projection.sql",
      "supabase/functions/_shared/app_evidence_review_correction_handoff.ts",
      "supabase/functions/api-app-customer-correction-handoff/index.ts",
      "supabase/functions/api-app-evidence-review-correction-publish/index.ts",
      "scripts/proofs/app-evidence-review-correction-handoff.proof.ts",
      "scripts/proofs/app-customer-correction-handoff-contract-v2.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "REVIEW19_ENDPOINT_Q01_Q09=PASS",
  }),
  check({
    id: "correction-cover-message-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "scripts/proofs/app-correction-cover-message.proof.ts",
    ],
    domain: "tenant-correction-cover-message-v1",
    applicablePaths: [
      "app/src/features/dashboard/CustomerCorrectionHandoffPanel.tsx",
      "app/src/features/dashboard/customerCorrectionHandoffClient.ts",
      "app/src/features/evidence-review/EvidenceReviewCaseDetailPage.tsx",
      "app/src/features/evidence-review/evidenceReviewDetailClient.ts",
      "app/src/features/evidence-review/useEvidenceCorrectionPublish.ts",
      "supabase/functions/_shared/app_evidence_review_correction_handoff.ts",
      "supabase/functions/api-app-customer-correction-handoff/index.ts",
      "supabase/functions/api-app-evidence-review-correction-publish/index.ts",
      "supabase/functions/api-app-evidence-review-correction-supersede/index.ts",
      "supabase/migrations/20260916120526_correction_cover_message_v1.sql",
      "scripts/proofs/app-correction-cover-message.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "CORRECTION_COVER_MESSAGE_V1_PURE=PASS",
  }),
  check({
    id: "document-driven-customer-correction-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "scripts/proofs/app-document-driven-customer-correction.proof.ts",
    ],
    domain: "tenant-document-driven-customer-correction-v1",
    applicablePaths: [
      "app/src/features/dashboard/CustomerCorrectionHandoffPanel.tsx",
      "app/src/features/dashboard/customerCorrectionHandoffClient.ts",
      "app/src/features/documents/CustomerDocumentFactInteraction.tsx",
      "app/src/features/documents/CustomerDocumentWorkflowController.ts",
      "app/src/features/evidence-review/evidenceReviewDetailClient.ts",
      "app/src/features/evidence-review/useEvidenceCorrectionPublish.ts",
      "supabase/functions/_shared/app_customer_correction_resolution.ts",
      "supabase/functions/_shared/app_customer_correction_submission.ts",
      "supabase/functions/_shared/app_evidence_review_correction_handoff.ts",
      "supabase/functions/api-app-customer-correction-handoff/index.ts",
      "supabase/functions/api-app-customer-correction-signing-challenge/index.ts",
      "supabase/functions/api-app-customer-correction-signing-finalize/index.ts",
      "supabase/functions/api-app-customer-correction-upload-confirm/index.ts",
      "supabase/functions/api-app-customer-correction-upload-remove/index.ts",
      "supabase/functions/api-app-customer-correction-upload-url/index.ts",
      "supabase/functions/api-app-evidence-review-correction-publish/index.ts",
      "supabase/functions/api-app-evidence-review-correction-supersede/index.ts",
      "supabase/migrations/20260917104334_document_driven_customer_correction_v1.sql",
      "supabase/migrations/20260917134619_document_driven_subject_ref_evidence_binding_v1.sql",
      "supabase/migrations/20260917180214_customer_correction_terminal_projection_null_v1.sql",
      "supabase/migrations/20260918073504_customer_correction_terminal_fact_projection_v1.sql",
      "supabase/migrations/20260918081541_customer_correction_terminal_fact_projection_customer_binding_fix_v1.sql",
      "scripts/proofs/app-document-driven-customer-correction.proof.ts",
      "scripts/proofs/app-evidence-fact-review-round-served.proof.mjs",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "DOCUMENT_DRIVEN_CUSTOMER_CORRECTION_V1_PURE=PASS",
  }),
  check({
    id: "evidence-review-correction-handoff-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "--allow-run=docker",
      "scripts/proofs/app-evidence-review-correction-handoff.proof.ts",
    ],
    domain: "tenant-evidence-review-correction-handoff",
    applicablePaths: [
      "supabase/migrations/20260819190000_app_evidence_review_correction_handoff.sql",
      "supabase/migrations/20260819220000_app_evidence_review_correction_publication_target_fix.sql",
      "supabase/migrations/20260820120000_app_customer_correction_handoff_contract_v2.sql",
      "supabase/migrations/20260820150000_app_customer_correction_signer_authority.sql",
      "supabase/migrations/20260822120000_app_customer_correction_handoff_fact_projection.sql",
      "supabase/functions/_shared/app_evidence_review_correction_handoff.ts",
      "supabase/functions/api-app-customer-correction-handoff/index.ts",
      "supabase/functions/api-app-evidence-review-correction-publish/index.ts",
      "scripts/proofs/app-evidence-review-correction-handoff.proof.ts",
      "scripts/proofs/app-customer-correction-handoff-contract-v2.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available through local Docker",
      "disposable proof database create/drop authority",
      "active TENANT_ENVAL remains read-only during proof",
    ],
    mutatesState: true,
    expectedDurationMs: 30_000,
    expectedMarker: "EVIDENCE_REVIEW_CORRECTION_HANDOFF_Q01_Q24=PASS",
  }),
  check({
    id: "correction-cover-message-served",
    argv: [
      "node",
      "scripts/proofs/app-correction-cover-message-served.proof.mjs",
    ],
    domain: "tenant-correction-cover-message-v1",
    applicablePaths: [
      "app/src/features/dashboard/CustomerCorrectionHandoffPanel.tsx",
      "app/src/features/dashboard/customerCorrectionHandoffClient.ts",
      "app/src/features/evidence-review/EvidenceReviewCaseDetailPage.tsx",
      "app/src/features/evidence-review/evidenceReviewDetailClient.ts",
      "app/src/features/evidence-review/useEvidenceCorrectionPublish.ts",
      "supabase/functions/_shared/app_evidence_review_correction_handoff.ts",
      "supabase/functions/api-app-customer-correction-handoff/index.ts",
      "supabase/functions/api-app-evidence-review-correction-publish/index.ts",
      "supabase/functions/api-app-evidence-review-correction-supersede/index.ts",
      "supabase/migrations/20260916120526_correction_cover_message_v1.sql",
      "scripts/proofs/app-correction-cover-message.proof.ts",
      "scripts/proofs/app-correction-cover-message-served.proof.mjs",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "canonical ENVAL local runtime on ports 5175 and 54321",
      "one exact current-run fixture with finally cleanup",
    ],
    mutatesState: true,
    expectedDurationMs: 10_000,
    expectedMarker: "CORRECTION_COVER_ACTIVE_SCHEMA=PASS",
  }),
  check({
    id: "customer-correction-handoff-v2-pure",
    argv: [
      "deno",
      "run",
      "--allow-read",
      "scripts/proofs/app-customer-correction-handoff-contract-v2.proof.ts",
    ],
    domain: "tenant-customer-correction-handoff-contract-v2",
    applicablePaths: [
      "app/src/features/dashboard/customerCorrectionHandoffClient.ts",
      "supabase/functions/_shared/app_evidence_review_correction_handoff.ts",
      "supabase/functions/_shared/app_customer_correction_submission.ts",
      "supabase/functions/api-app-customer-correction-handoff/index.ts",
      "supabase/functions/api-app-customer-correction-signing-challenge/index.ts",
      "supabase/migrations/20260820120000_app_customer_correction_handoff_contract_v2.sql",
      "supabase/migrations/20260820150000_app_customer_correction_signer_authority.sql",
      "supabase/migrations/20260822120000_app_customer_correction_handoff_fact_projection.sql",
      "scripts/proofs/app-customer-correction-handoff-contract-v2.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "CUSTOMER_CORRECTION_HANDOFF_V3_Q01_Q08=PASS",
  }),
  check({
    id: "customer-correction-handoff-v2-local-read",
    argv: [
      "deno",
      "run",
      "--allow-read",
      "--allow-run=psql",
      "scripts/proofs/app-customer-correction-handoff-contract-v2.proof.ts",
      "--local-read",
    ],
    domain: "tenant-customer-correction-handoff-contract-v2-local-read",
    applicablePaths: [
      "app/src/features/dashboard/customerCorrectionHandoffClient.ts",
      "supabase/functions/_shared/app_evidence_review_correction_handoff.ts",
      "supabase/functions/_shared/app_customer_correction_submission.ts",
      "supabase/functions/api-app-customer-correction-handoff/index.ts",
      "supabase/functions/api-app-customer-correction-signing-challenge/index.ts",
      "supabase/migrations/20260820120000_app_customer_correction_handoff_contract_v2.sql",
      "supabase/migrations/20260820150000_app_customer_correction_signer_authority.sql",
      "supabase/migrations/20260822120000_app_customer_correction_handoff_fact_projection.sql",
      "scripts/proofs/app-customer-correction-handoff-contract-v2.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_READ,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available on the guarded local port",
      "read-only pilot contract projection; no challenge or finalization",
    ],
    expectedDurationMs: 2_000,
    expectedMarker: "CUSTOMER_CORRECTION_HANDOFF_V3_PILOT_LOCAL_READ=PASS",
  }),
  check({
    id: "evidence-review-preview-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "scripts/proofs/app-evidence-review-preview.proof.ts",
      "--endpoint-only",
    ],
    domain: "tenant-evidence-review-preview",
    applicablePaths: [
      "supabase/functions/_shared/app_evidence_review_preview.ts",
      "supabase/functions/api-app-evidence-review-preview/index.ts",
      "scripts/proofs/app-evidence-review-preview.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "EVIDENCE_REVIEW_PREVIEW_ENDPOINT=PASS",
  }),
  check({
    id: "evidence-review-preview-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "--allow-run=docker,supabase",
      "--allow-net=127.0.0.1:54321",
      "scripts/proofs/app-evidence-review-preview.proof.ts",
    ],
    domain: "tenant-evidence-review-preview",
    applicablePaths: [
      "supabase/migrations/20260818150000_app_evidence_review_preview_read.sql",
      "supabase/functions/_shared/app_evidence_review_preview.ts",
      "supabase/functions/api-app-evidence-review-preview/index.ts",
      "scripts/proofs/app-evidence-review-preview.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_READ,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL and Storage available through local Supabase",
      "active TENANT_ENVAL remains read-only during proof",
    ],
    expectedDurationMs: 5_000,
    expectedMarker: "EVIDENCE_REVIEW_PREVIEW_Q01_Q14=PASS",
  }),
  check({
    id: "compliance-worklist-ui-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=NODE_ENV",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "app/src/features/compliance/ComplianceWorklistPage.proof.tsx",
    ],
    domain: "authenticated-compliance-worklist-ui",
    applicablePaths: [
      "app/src/App.tsx",
      "app/src/features/compliance/ComplianceWorklistPage.proof.tsx",
      "app/src/features/compliance/ComplianceWorklistPage.tsx",
      "app/src/features/compliance/complianceWorklistClient.ts",
      "app/src/features/compliance/useComplianceWorklist.ts",
      "app/src/pages/ComplianceWorklistPage.tsx",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "COMPLIANCE_WORKLIST_UI_Q01_Q18=PASS",
  }),
  check({
    id: "authenticated-internal-navigation-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=NODE_ENV",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "app/src/features/auth/postLoginNavigation.proof.ts",
    ],
    domain: "authenticated-internal-navigation",
    applicablePaths: [
      "app/src/App.tsx",
      "app/src/routes/types.ts",
      "app/src/features/auth/AccountPage.tsx",
      "app/src/features/auth/AuthProvider.tsx",
      "app/src/features/auth/DashboardRouteGuard.tsx",
      "app/src/features/auth/authBootstrapClient.ts",
      "app/src/features/auth/authTypes.ts",
      "app/src/features/auth/authorizedPortalAccess.ts",
      "app/src/features/auth/postLoginNavigation.ts",
      "app/src/features/auth/postLoginNavigation.proof.ts",
      "app/src/features/dashboard/DashboardSidebar.tsx",
      "app/src/features/evidence-review/evidenceReviewWorklistClient.ts",
      "app/src/features/evidence-review/evidenceReviewRoutes.ts",
      "app/src/features/operator/OperatorRouteGuard.tsx",
      "app/src/features/operator/operatorNavigation.ts",
      "app/src/features/operator/operatorContextClient.ts",
      "app/src/shared/components/AppHeader.tsx",
      "app/src/pages/ComplianceWorklistPage.tsx",
      "app/src/pages/EvidenceReviewCaseDetailPage.tsx",
      "app/src/pages/EvidenceReviewWorklistPage.tsx",
      "app/src/pages/OperatorOverviewPage.tsx",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "AUTHENTICATED_INTERNAL_NAVIGATION_Q01_Q13=PASS",
  }),
  check({
    id: "auth-ux-recovery-resend-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=NODE_ENV",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "app/src/features/auth/authUxFlow.proof.ts",
    ],
    domain: "auth-ux-recovery-resend",
    applicablePaths: [
      "app/src/App.tsx",
      "app/src/pages/AccountPage.tsx",
      "app/src/features/auth/AccountPage.tsx",
      "app/src/features/auth/AuthEmailRequestPage.tsx",
      "app/src/features/auth/AuthPageLayout.tsx",
      "app/src/features/auth/AuthProvider.tsx",
      "app/src/features/auth/PasswordRecoveryPage.tsx",
      "app/src/features/auth/authClient.ts",
      "app/src/features/auth/authErrorMapping.ts",
      "app/src/features/auth/authTypes.ts",
      "app/src/features/auth/authUxFlow.ts",
      "app/src/features/auth/authUxFlow.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "AUTH_UX_RECOVERY_RESEND_Q01_Q12=PASS",
  }),
  check({
    id: "public-auth-identity-v1-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=NODE_ENV",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "app/src/features/auth/authShell.proof.tsx",
    ],
    domain: "public-auth-route-and-presentation-identity",
    applicablePaths: [
      "app/src/App.tsx",
      "app/src/pages/AccountPage.tsx",
      "app/src/pages/HomePage.tsx",
      "app/src/features/auth/AuthEmailRequestPage.tsx",
      "app/src/features/auth/AuthPageLayout.tsx",
      "app/src/features/auth/DashboardRouteGuard.tsx",
      "app/src/features/auth/authShell.proof.tsx",
      "app/src/features/auth/authUxFlow.ts",
      "app/src/features/auth/PasswordRecoveryPage.tsx",
      "app/src/features/dashboard/DashboardSidebar.tsx",
      "app/src/features/signup/SignupPageShell.tsx",
      "app/src/shared/components/AppHeader.tsx",
      "app/src/shared/presentation/PresentationBrandProvider.tsx",
      "scripts/proofs/sl01c-browser-fixture.ts",
      "scripts/tools/enval-ui-review-collect.mjs",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "PUBLIC_AUTH_IDENTITY_V1_Q01_Q12=PASS",
  }),
  check({
    id: "evidence-review-worklist-ui-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=NODE_ENV",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "app/src/features/evidence-review/EvidenceReviewWorklistPage.proof.tsx",
    ],
    domain: "authenticated-evidence-review-worklist-ui",
    applicablePaths: [
      "app/src/App.tsx",
      "app/src/features/evidence-review/EvidenceReviewWorklistPage.proof.tsx",
      "app/src/features/evidence-review/EvidenceReviewWorklistPage.tsx",
      "app/src/features/evidence-review/evidenceReviewRoutes.ts",
      "app/src/features/evidence-review/evidenceReviewWorklistClient.ts",
      "app/src/features/evidence-review/useEvidenceReviewWorklist.ts",
      "app/src/pages/EvidenceReviewWorklistPage.tsx",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "EVIDENCE_REVIEW_WORKLIST_UI_Q01_Q19=PASS",
  }),
  check({
    id: "evidence-review-case-detail-ui-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=NODE_ENV",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "app/src/features/evidence-review/EvidenceReviewCaseDetailPage.proof.tsx",
    ],
    domain: "authenticated-evidence-review-case-detail-ui",
    applicablePaths: [
      "app/src/App.tsx",
      "app/src/features/auth/postLoginNavigation.proof.ts",
      "app/src/features/auth/postLoginNavigation.ts",
      "app/src/features/documents/documentDownloadClient.ts",
      "app/src/features/evidence-review/EvidenceReviewCaseDetailPage.proof.tsx",
      "app/src/features/evidence-review/EvidenceReviewCaseDetailPage.tsx",
      "app/src/features/evidence-review/EvidenceReviewPreviewPane.tsx",
      "app/src/features/evidence-review/EvidenceReviewWorklistPage.proof.tsx",
      "app/src/features/evidence-review/EvidenceReviewWorklistPage.tsx",
      "app/src/features/evidence-review/evidenceReviewDetailClient.ts",
      "app/src/features/evidence-review/evidenceReviewRoutes.ts",
      "app/src/features/evidence-review/useEvidenceFactReviewDraft.ts",
      "app/src/features/evidence-review/useEvidenceCorrectionPublish.ts",
      "app/src/features/evidence-review/useEvidenceReviewCaseDetail.ts",
      "app/src/pages/EvidenceReviewCaseDetailPage.tsx",
      "app/src/pages/EvidenceReviewWorklistPage.tsx",
      "app/src/styles/components.css",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 2_000,
    expectedMarker: "EVIDENCE_REVIEW_CASE_DETAIL_UI_Q01_Q20=PASS",
  }),
  check({
    id: "customer-timeline-client-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--no-lock",
      "--allow-read",
      "--allow-env",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "scripts/proofs/app-customer-timeline-client.proof.mjs",
    ],
    domain: "customer-timeline-client-contract",
    applicablePaths: [
      "app/src/features/dashboard/ActivePrivateDashboard.tsx",
      "app/src/features/dashboard/CustomerTimeline.tsx",
      "app/src/features/dashboard/dashboardReadClient.proof.ts",
      "app/src/features/dashboard/dashboardReadClient.ts",
      "app/src/features/dashboard/dashboardStatusPresentation.ts",
      "app/src/features/dashboard/dashboardTypes.ts",
      "app/src/styles/components.css",
      "scripts/proofs/app-customer-timeline-client.proof.mjs",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "DASHBOARD_READ_CLIENT_TIMELINE=PASS",
    compilerAuthority: COMPILER_AUTHORITY.APP_TYPESCRIPT,
  }),
  check({
    id: "customer-portal-ia-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=NODE_ENV",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "app/src/features/dashboard/customerPortalIa.proof.tsx",
    ],
    domain: "customer-portal-application-navigation",
    applicablePaths: [
      "app/src/App.tsx",
      "app/src/features/dashboard/ActivePrivateDashboard.tsx",
      "app/src/features/dashboard/CustomerApplicationList.tsx",
      "app/src/features/dashboard/DashboardPageShell.tsx",
      "app/src/features/dashboard/DashboardSidebar.tsx",
      "app/src/features/dashboard/customerPortalIa.proof.tsx",
      "app/src/features/dashboard/dashboardReadCache.ts",
      "app/src/features/dashboard/dashboardReadClient.proof.ts",
      "app/src/features/dashboard/dashboardReadClient.ts",
      "app/src/features/dashboard/dashboardRoutes.ts",
      "app/src/features/dashboard/dashboardTypes.ts",
      "app/src/features/dashboard/useDashboardRead.ts",
      "app/src/pages/DashboardPage.tsx",
      "supabase/functions/api-app-dashboard-get/index.ts",
      "supabase/migrations/20260911103144_app_customer_application_index_read_v1.sql",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "CUSTOMER_PORTAL_IA_V1_Q01_Q08=PASS",
    compilerAuthority: COMPILER_AUTHORITY.APP_TYPESCRIPT,
  }),
  check({
    id: "portal-context-authority-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "scripts/proofs/app-portal-context-authority.proof.ts",
    ],
    domain: "customer-portal-r7-authority",
    applicablePaths: [
      "app/src/features/dashboard/ActivePrivateDashboard.tsx",
      "scripts/proofs/app-portal-context-authority.proof.ts",
      "supabase/functions/_shared/app_customer_auth.ts",
      "supabase/functions/api-app-dashboard-get/index.ts",
      "supabase/migrations/20260909115342_app_portal_context_authority.sql",
      "supabase/migrations/20260911103144_app_customer_application_index_read_v1.sql",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "APP_PORTAL_CONTEXT_AUTHORITY_PROOF=PASS",
  }),
  check({
    id: "customer-portal-application-index-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--no-lock",
      "--allow-read",
      "--allow-env",
      "--allow-net=127.0.0.1:54321,localhost:54321",
      "--allow-run=psql",
      "scripts/proofs/api-app-dashboard-get.proof.ts",
    ],
    domain: "customer-portal-application-index-runtime",
    applicablePaths: [
      "scripts/proofs/api-app-dashboard-get.proof.ts",
      "supabase/functions/_shared/app_customer_auth.ts",
      "supabase/functions/api-app-dashboard-get/index.ts",
      "supabase/migrations/20260911103144_app_customer_application_index_read_v1.sql",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL Auth, PostgreSQL and Edge available locally",
      "exact disposable fixture cleanup through local supabase_admin",
    ],
    mutatesState: true,
    expectedDurationMs: 12_000,
    expectedMarker: "APP_DASHBOARD_GET_LOCAL=PASS",
  }),
  check({
    id: "customer-timeline-local",
    argv: ["node", "scripts/proofs/app-customer-case-timeline.proof.mjs"],
    domain: "customer-case-timeline-service-projection",
    applicablePaths: [
      "scripts/proofs/app-customer-case-timeline.proof.mjs",
      "supabase/migrations/20260910195608_app_customer_case_timeline_read_v1.sql",
      "supabase/migrations/20260911103144_app_customer_application_index_read_v1.sql",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL Auth and PostgreSQL available locally",
      "exact synthetic fixture cleanup through local supabase_admin",
    ],
    mutatesState: true,
    expectedDurationMs: 5_000,
    expectedMarker: "CUSTOMER_TIMELINE_CLEANUP=PASS",
  }),
  check({
    id: "tenant-migration-chain-local",
    argv: ["node", "scripts/proofs/enval-migration-chain.proof.mjs"],
    domain: "tenant-migration-chain",
    applicablePaths: [
      "supabase/migrations/20260816150000_app_current_baseline.sql",
      "supabase/migrations/20260816160000_app_workforce_policy_foundation.sql",
      "supabase/migrations/20260817120000_app_compliance_workforce_view.sql",
      "supabase/migrations/20260817160000_app_compliance_source_event_ledger.sql",
      "supabase/migrations/20260817190000_app_compliance_source_event_capture.sql",
      "supabase/migrations/20260817210000_app_compliance_worklist_read.sql",
      "supabase/migrations/20260817230000_app_evidence_review_foundation.sql",
      "supabase/migrations/20260818090000_app_evidence_review_worklist_read.sql",
      "supabase/migrations/20260819210000_app_evidence_review_correction_publish_affordance.sql",
      "supabase/migrations/20260820090000_app_customer_correction_submissions.sql",
      "supabase/migrations/20260820210000_app_correction_handoff_supersession.sql",
      "supabase/migrations/20260820220000_app_customer_correction_replacement_staging.sql",
      "supabase/migrations/20260910195608_app_customer_case_timeline_read_v1.sql",
      "supabase/migrations/20260913161612_app_workflow_email_foundation_v1.sql",
      "supabase/migrations/20260916120526_correction_cover_message_v1.sql",
      "supabase/migration-archive/**",
      "scripts/tools/enval-migration-chain-manifest.mjs",
      "scripts/proofs/enval-migration-chain.proof.mjs",
    ],
    safety: SAFETY.SAFE_LOCAL_TENANT_EPHEMERAL_WRITE,
    minimumMode: "LOCAL_SERVICE",
    serviceRequirements: [
      "TENANT_ENVAL PostgreSQL available through local Docker",
      "disposable proof database create/drop authority",
      "active TENANT_ENVAL remains read-only",
    ],
    mutatesState: true,
    expectedDurationMs: 30_000,
    expectedMarker: "MIG02_CHAIN_PROOF=PASS",
  }),
  check({
    id: "tenant-resolution-composition-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "scripts/proofs/tenant-resolution-composition.proof.ts",
    ],
    domain: "provider-neutral-tenant-resolution",
    applicablePaths: [
      "platform/runtime/tenant-resolution/tenant_resolution.ts",
      "platform/runtime/tenant-resolution/tenant_resolution_composition.ts",
      "platform/runtime/tenant-resolution/trusted_ingress.ts",
      "platform/runtime/tenant-resolution/adapters/platform_control_plane_v1.ts",
      "platform/runtime/tenant-resolution/adapters/static_single_tenant_v1.ts",
      "scripts/proofs/tenant-resolution-composition.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "TENANT_RESOLUTION_COMPOSITION_Q01_Q14=PASS",
  }),
  check({
    id: "delivery-year-compliance-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read=platform/runtime/compliance/delivery_year_compliance.ts",
      "scripts/proofs/delivery-year-compliance.proof.ts",
    ],
    domain: "provider-neutral-delivery-year-compliance",
    applicablePaths: [
      "platform/runtime/compliance/delivery_year_compliance.ts",
      "scripts/proofs/delivery-year-compliance.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "DELIVERY_YEAR_COMPLIANCE_Q01_Q20=PASS",
  }),
  check({
    id: "compliance-action-plan-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read=platform/runtime/compliance/compliance_action_plan.ts",
      "scripts/proofs/compliance-action-plan.proof.ts",
    ],
    domain: "provider-neutral-compliance-action-plan",
    applicablePaths: [
      "platform/runtime/compliance/delivery_year_compliance.ts",
      "platform/runtime/compliance/compliance_action_plan.ts",
      "scripts/proofs/delivery-year-compliance.proof.ts",
      "scripts/proofs/compliance-action-plan.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "COMPLIANCE_ACTION_PLAN_Q01_Q18=PASS",
  }),
  check({
    id: "compliance-worklist-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read=platform/runtime/compliance/compliance_worklist.ts",
      "scripts/proofs/compliance-worklist.proof.ts",
    ],
    domain: "provider-neutral-compliance-worklist",
    applicablePaths: [
      "platform/runtime/compliance/delivery_year_compliance.ts",
      "platform/runtime/compliance/compliance_action_plan.ts",
      "platform/runtime/compliance/compliance_worklist.ts",
      "scripts/proofs/delivery-year-compliance.proof.ts",
      "scripts/proofs/compliance-action-plan.proof.ts",
      "scripts/proofs/compliance-worklist.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "COMPLIANCE_WORKLIST_Q01_Q16=PASS",
  }),
  check({
    id: "presentation-brand-config-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "scripts/proofs/presentation-brand-config.proof.ts",
    ],
    domain: "provider-neutral-presentation-brand",
    applicablePaths: [
      "platform/runtime/presentation/presentation_brand_config.ts",
      "platform/runtime/presentation/enval_presentation_defaults.ts",
      "scripts/proofs/presentation-brand-config.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "PRESENTATION_BRAND_CONFIG_Q01_Q14=PASS",
  }),
  check({
    id: "presentation-brand-sources-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "scripts/proofs/presentation-brand-sources.proof.ts",
    ],
    domain: "provider-neutral-presentation-brand-source",
    applicablePaths: [
      "platform/runtime/presentation/presentation_brand_source.ts",
      "platform/runtime/presentation/presentation_source_composition.ts",
      "platform/runtime/presentation/adapters/platform_control_plane_presentation_v1.ts",
      "platform/runtime/presentation/adapters/static_presentation_config_v1.ts",
      "scripts/proofs/presentation-brand-sources.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "PRESENTATION_BRAND_SOURCES_Q01_Q12=PASS",
  }),
  check({
    id: "presentation-brand-consumers-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=NODE_ENV",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "app/src/shared/presentation/PresentationBrandProvider.proof.tsx",
    ],
    domain: "react-presentation-brand-consumers",
    applicablePaths: [
      "app/src/main.tsx",
      "app/src/shared/presentation/PresentationBrandProvider.tsx",
      "app/src/shared/presentation/PresentationBrandRuntime.tsx",
      "app/src/shared/presentation/PresentationBrandProvider.proof.tsx",
      "app/src/shared/components/AppHeader.tsx",
      "app/src/pages/HomePage.tsx",
      "app/src/pages/PrivacyPage.tsx",
      "app/src/pages/AccountPage.tsx",
      "app/src/features/auth/AccountPage.tsx",
      "app/src/features/auth/AuthPageLayout.tsx",
      "app/src/features/dashboard/ActivePrivateDashboard.tsx",
      "app/src/features/dashboard/ContactChoicePanel.tsx",
      "app/src/features/dashboard/DashboardSidebar.tsx",
      "app/src/features/documents/DocumentFactMatrix.tsx",
      "app/src/features/signup/SignupConnectionSection.tsx",
      "app/src/features/signup/SignupSubmitStatusPanel.tsx",
      "app/src/features/signup/ConsentSignatureSection.tsx",
      "app/src/features/signup/DocumentFirstSigningSummary.tsx",
      "app/src/features/signup/signing/browserHtmlLegalBundleV1.ts",
      "app/src/features/signup/signing/legalBundleDocument.ts",
      "app/src/features/signup/presentation/FactTable.tsx",
      "app/src/features/evidence-review/evidenceReviewStatusPresentation.ts",
      "app/src/pages/ComplianceWorklistPage.tsx",
      "app/src/pages/EvidenceReviewCaseDetailPage.tsx",
      "app/src/pages/EvidenceReviewWorklistPage.tsx",
      "app/src/pages/OperatorOverviewPage.tsx",
      "app/src/pages/NotFoundPage.tsx",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "PRESENTATION_BRAND_CONSUMERS_Q01_Q22=PASS",
  }),
  check({
    id: "customer-correction-handoff-ui-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=NODE_ENV",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "app/src/features/dashboard/customerCorrectionHandoff.proof.tsx",
    ],
    domain: "authenticated-customer-correction-handoff-ui",
    applicablePaths: [
      "app/src/features/dashboard/ActivePrivateDashboard.tsx",
      "app/src/features/dashboard/CustomerCorrectionHandoffPanel.tsx",
      "app/src/features/dashboard/DashboardPageShell.tsx",
      "app/src/features/dashboard/customerCorrectionHandoffClient.ts",
      "app/src/features/dashboard/customerCorrectionHandoff.proof.tsx",
      "app/src/features/dashboard/customerCorrectionReplacementUpload.ts",
      "app/src/features/dashboard/customerCorrectionWorkspace.ts",
      "app/src/features/dashboard/useCustomerCorrectionHandoff.ts",
      "app/src/features/documents/documentUploadTransport.ts",
      "app/src/features/signup/signing/SignerPanel.tsx",
      "app/src/features/signup/signing/methods/typedNameOtpV1.ts",
      "app/src/styles/components.css",
      "app/src/styles/global.css",
      "assets/css/form-states.css",
      "assets/css/style.css",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "CUSTOMER_CORRECTION_HANDOFF_UI_Q01_Q40=PASS",
  }),
  check({
    id: "app-presentation-runtime-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=NODE_ENV",
      "--node-modules-dir=manual",
      "--config",
      "app/tsconfig.json",
      "app/src/shared/presentation/PresentationBrandRuntime.proof.tsx",
    ],
    domain: "server-owned-app-presentation-bootstrap",
    applicablePaths: [
      "platform/control-plane/supabase/config.toml",
      "supabase/functions/_shared/app_control_plane_runtime_reader.ts",
      "supabase/functions/_shared/app_presentation_bootstrap.ts",
      "supabase/functions/api-app-presentation-bootstrap/index.ts",
      "app/src/features/auth/authRuntimeConfig.ts",
      "app/src/main.tsx",
      "app/src/shared/presentation/PresentationBrandProvider.tsx",
      "app/src/shared/presentation/PresentationBrandRuntime.tsx",
      "app/src/shared/presentation/presentationBootstrapClient.ts",
      "app/src/shared/presentation/PresentationBrandRuntime.proof.tsx",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_500,
    expectedMarker: "APP_PRESENTATION_RUNTIME_Q01_Q19=PASS",
  }),
  check({
    id: "trusted-ingress-boundary-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ENVIRONMENT,ENV,APP_ENV,ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "scripts/proofs/trusted-ingress-boundary.proof.ts",
    ],
    domain: "server-owned-trusted-tenant-ingress",
    applicablePaths: [
      "platform/runtime/tenant-resolution/trusted_ingress.ts",
      "platform/runtime/tenant-resolution/tenant_resolution.ts",
      "supabase/functions/_shared/app_foundation.ts",
      "supabase/functions/_shared/app_tenant_resolution_shadow.ts",
      "scripts/proofs/trusted-ingress-boundary.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "TRUSTED_INGRESS_BOUNDARY_Q01_Q18=PASS",
  }),
  check({
    id: "app-tenant-resolution-shadow-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ENVIRONMENT,ENV,APP_ENV,ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "scripts/proofs/app-tenant-resolution-shadow.proof.ts",
    ],
    domain: "app-server-tenant-resolution-shadow",
    applicablePaths: [
      "supabase/functions/_shared/app_foundation.ts",
      "supabase/functions/_shared/app_tenant_resolution_shadow.ts",
      "scripts/proofs/app-tenant-resolution-shadow.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "TENANT_RESOLUTION_AUTHORITY_Q19_Q43=PASS",
  }),
  check({
    id: "ops-location-authoritative-gate-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ENVIRONMENT,ENV,APP_ENV,ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "scripts/proofs/api-app-ops-location-callers.proof.ts",
      "--tenant-gate-only",
    ],
    domain: "app-ops-location-authoritative-tenant-gate",
    applicablePaths: [
      "supabase/functions/_shared/app_workforce_authorization.ts",
      "supabase/functions/api-app-ops-location-observation-record/index.ts",
      "supabase/functions/api-app-ops-location-root-create/index.ts",
      "supabase/functions/api-app-ops-location-version-accept/index.ts",
      "supabase/functions/api-app-ops-location-version-correct/index.ts",
      "scripts/proofs/api-app-ops-location-callers.proof.ts",
    ],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "TARGETED",
    expectedDurationMs: 1_000,
    expectedMarker: "OPS_LOCATION_AUTHORITATIVE_GATE_Q01_Q12=PASS",
  }),
  check({
    id: "app-tenant-resolution-shadow-local",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--allow-env=ENVIRONMENT,ENV,APP_ENV,ALLOWED_ORIGINS,ALLOWED_ORIGIN",
      "--allow-run=psql",
      "scripts/proofs/app-tenant-resolution-shadow.proof.ts",
      "--local-control-plane",
    ],
    domain: "local-control-plane-tenant-resolution-shadow",
    applicablePaths: [
      "supabase/functions/_shared/app_foundation.ts",
      "supabase/functions/_shared/app_tenant_resolution_shadow.ts",
      "scripts/proofs/app-tenant-resolution-shadow.proof.ts",
    ],
    safety: SAFETY.SAFE_LOCAL_READ,
    minimumMode: "INTEGRATION",
    serviceRequirements: [
      "CONTROL_PLANE PostgreSQL read-only on loopback port 56322",
      "TENANT_ENVAL PostgreSQL read-only on loopback port 54322",
    ],
    expectedDurationMs: 1_500,
    expectedMarker: "TENANT_RESOLUTION_AUTHORITY_LOCAL_Q44_Q46=PASS",
  }),
  check({
    id: "app-typecheck-build",
    argv: ["npm", "--prefix", "app", "run", "build"],
    domain: "frontend-typecheck-build",
    applicablePaths: ["app/src/**", "app/package.json", "app/vite.config.ts"],
    safety: SAFETY.SAFE_GENERATED_WRITE,
    minimumMode: "INTEGRATION",
    serviceRequirements: ["app/node_modules"],
    mutatesState: true,
    expectedDurationMs: 8_000,
    dedupeKey: "frontend-typecheck-and-build",
    compilerAuthority: COMPILER_AUTHORITY.APP_TYPESCRIPT,
  }),
  check({
    id: "edge-static-check",
    argv: [
      "deno",
      "check",
      "--deny-import",
      "--no-lock",
      "--unstable-sloppy-imports",
      "--config",
      "supabase/functions/deno.json",
      "supabase/functions/api-app-auth-bootstrap/index.ts",
      "supabase/functions/api-app-presentation-bootstrap/index.ts",
      "supabase/functions/api-app-dashboard-get/index.ts",
      "supabase/functions/api-app-customer-information-request/index.ts",
      "supabase/functions/api-app-signup-submit/index.ts",
      "supabase/functions/api-app-signup-signing-presentation/index.ts",
      "supabase/functions/api-app-signup-signing-challenge/index.ts",
      "supabase/functions/api-app-signup-signing-finalize/index.ts",
      "supabase/functions/api-app-signup-upload-confirm/index.ts",
      "supabase/functions/api-app-compliance-source-event/index.ts",
      "supabase/functions/api-app-compliance-worklist/index.ts",
      "supabase/functions/api-app-evidence-review-correction-supersede/index.ts",
      "supabase/functions/api-app-customer-correction-upload-url/index.ts",
      "supabase/functions/api-app-customer-correction-upload-confirm/index.ts",
      "supabase/functions/api-app-customer-correction-upload-remove/index.ts",
      "supabase/functions/api-app-customer-correction-handoff/index.ts",
      "supabase/functions/api-app-ops-location-observation-record/index.ts",
      "supabase/functions/api-app-ops-location-root-create/index.ts",
      "supabase/functions/api-app-ops-location-version-accept/index.ts",
      "supabase/functions/api-app-ops-location-version-correct/index.ts",
    ],
    domain: "edge-auth-dashboard-signup-static",
    applicablePaths: ["supabase/functions/**"],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "INTEGRATION",
    expectedDurationMs: 5_000,
  }),
  check({
    id: "signup-signature-core-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--unstable-sloppy-imports",
      "scripts/proofs/app-signup-signature-core.proof.ts",
    ],
    domain: "signing-client-contract",
    applicablePaths: ["app/src/features/signup/**"],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "INTEGRATION",
    expectedDurationMs: 1_000,
    expectedMarker: "signup-signature-core-09a-proof-ok",
  }),
  check({
    id: "signup-signed-receipt-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--unstable-sloppy-imports",
      "scripts/proofs/app-signup-signed-receipt.proof.ts",
    ],
    domain: "signed-receipt-contract",
    applicablePaths: ["app/src/features/signup/**", "supabase/functions/**"],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "INTEGRATION",
    expectedDurationMs: 1_000,
    expectedMarker: "signup-signed-receipt-09b2c-proof-ok",
  }),
  check({
    id: "post-signing-convergence-pure",
    argv: [
      "deno",
      "run",
      "--cached-only",
      "--allow-read",
      "--unstable-sloppy-imports",
      "scripts/proofs/app-post-signing-convergence.proof.ts",
    ],
    domain: "auth-dashboard-source-contract",
    applicablePaths: ["app/src/**", "supabase/functions/**"],
    safety: SAFETY.SAFE_PURE,
    minimumMode: "INTEGRATION",
    expectedDurationMs: 1_500,
    expectedMarker: "POST_SIGNING_CONVERGENCE_Q93_Q112=PASS",
  }),
  check({
    id: "remote-baseline-proof",
    argv: ["node", "scripts/proofs/in-place-baseline-phase0-proof.mjs"],
    domain: "remote-baseline",
    applicablePaths: ["scripts/proofs/in-place-baseline-phase0-proof.mjs"],
    safety: SAFETY.REMOTE_GATED,
    minimumMode: "QUICK",
    serviceRequirements: ["approved remote Supabase access"],
    remote: true,
    expectedDurationMs: 10_000,
  }),
  check({
    id: "remote-postgrest-proof",
    argv: ["node", "scripts/proofs/postgrest-authorized-health.proof.mjs"],
    domain: "remote-postgrest",
    applicablePaths: ["scripts/proofs/postgrest-authorized-health.proof.mjs"],
    safety: SAFETY.REMOTE_GATED,
    minimumMode: "QUICK",
    serviceRequirements: ["approved remote Supabase access"],
    remote: true,
    expectedDurationMs: 10_000,
  }),
  check({
    id: "remote-sql-review",
    runner: "gated",
    domain: "remote-sql",
    applicablePaths: ["**/*-remote-readonly.sql"],
    safety: SAFETY.REMOTE_GATED,
    minimumMode: "QUICK",
    serviceRequirements: ["separately approved remote SQL workflow"],
    remote: true,
    expectedDurationMs: 0,
  }),
  check({
    id: "migration-or-sql-review",
    runner: "gated",
    domain: "migration-sql",
    applicablePaths: ["**/*.sql except classified read-only SQL"],
    safety: SAFETY.LOCAL_MUTATING_GATED,
    minimumMode: "QUICK",
    mutatesState: true,
    expectedDurationMs: 0,
  }),
  check({
    id: "baseline-proposal-review",
    runner: "gated",
    domain: "baseline-proposal",
    applicablePaths: ["supabase/baseline-proposals/**"],
    safety: SAFETY.DESTRUCTIVE_GATED,
    minimumMode: "QUICK",
    mutatesState: true,
    destructive: true,
    expectedDurationMs: 0,
  }),
  check({
    id: "dependency-review",
    runner: "gated",
    domain: "dependencies",
    applicablePaths: ["package manifests and lockfiles"],
    safety: SAFETY.MANUAL_GATED,
    minimumMode: "QUICK",
    mutatesState: true,
    expectedDurationMs: 0,
  }),
  check({
    id: "shell-or-python-review",
    runner: "gated",
    domain: "unreviewed-tooling",
    applicablePaths: ["**/*.sh", "**/*.py"],
    safety: SAFETY.MANUAL_GATED,
    minimumMode: "QUICK",
    expectedDurationMs: 0,
  }),
  check({
    id: "invoice-browser-precheck-gated",
    argv: ["node", "scripts/tools/invoice-image-precheck.mjs"],
    domain: "browser-invoice-fixture",
    applicablePaths: ["scripts/tools/invoice-image-precheck.mjs"],
    safety: SAFETY.LOCAL_MUTATING_GATED,
    minimumMode: "QUICK",
    serviceRequirements: ["Playwright", "ignored private invoice fixtures"],
    mutatesState: true,
    expectedDurationMs: 30_000,
  }),
  check({
    id: "release-local-mutating-proofs",
    runner: "gated",
    domain: "release-local-mutation",
    applicablePaths: ["reviewed local DB/Auth mutation proofs"],
    safety: SAFETY.LOCAL_MUTATING_GATED,
    minimumMode: "RELEASE",
    serviceRequirements: ["explicit human-authorized isolated workflow"],
    mutatesState: true,
    expectedDurationMs: 0,
    requiredForRelease: true,
  }),
  check({
    id: "release-destructive-db-proofs",
    runner: "gated",
    domain: "release-destructive-database",
    applicablePaths: ["migration apply/reset/disposable database proofs"],
    safety: SAFETY.DESTRUCTIVE_GATED,
    minimumMode: "RELEASE",
    serviceRequirements: ["explicit human-authorized destructive workflow"],
    mutatesState: true,
    destructive: true,
    expectedDurationMs: 0,
    requiredForRelease: true,
  }),
  check({
    id: "release-remote-evidence",
    runner: "gated",
    domain: "release-remote",
    applicablePaths: ["remote Supabase and production evidence"],
    safety: SAFETY.REMOTE_GATED,
    minimumMode: "RELEASE",
    serviceRequirements: ["separate remote approval and credentials"],
    remote: true,
    expectedDurationMs: 0,
    requiredForRelease: true,
  }),
];

export const COMMANDS = Object.freeze(Object.fromEntries(
  CHECK_LIST.map((entry) => [entry.id, entry]),
));

export const GLOBAL_CHECKS = Object.freeze([
  Object.freeze({ id: "git-diff-check", minimumMode: "QUICK" }),
  Object.freeze({ id: "local-readonly-catalog", minimumMode: "LOCAL_SERVICE" }),
  Object.freeze({
    id: "archived-migration-reference-pure",
    minimumMode: "TARGETED",
  }),
  Object.freeze({ id: "app-typecheck-build", minimumMode: "INTEGRATION" }),
  Object.freeze({ id: "git-diff-cached-check", minimumMode: "INTEGRATION" }),
  Object.freeze({ id: "edge-static-check", minimumMode: "INTEGRATION" }),
  Object.freeze({ id: "signup-journey-pure", minimumMode: "INTEGRATION" }),
  Object.freeze({
    id: "signup-unified-presentation-pure",
    minimumMode: "INTEGRATION",
  }),
  Object.freeze({
    id: "signup-signature-core-pure",
    minimumMode: "INTEGRATION",
  }),
  Object.freeze({
    id: "signup-signed-receipt-pure",
    minimumMode: "INTEGRATION",
  }),
  Object.freeze({
    id: "post-signing-convergence-pure",
    minimumMode: "INTEGRATION",
  }),
  Object.freeze({
    id: "release-local-mutating-proofs",
    minimumMode: "RELEASE",
  }),
  Object.freeze({
    id: "release-destructive-db-proofs",
    minimumMode: "RELEASE",
  }),
  Object.freeze({ id: "release-remote-evidence", minimumMode: "RELEASE" }),
]);

export const PATH_RULES = Object.freeze([
  Object.freeze({
    id: "verification-harness-javascript",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "scripts/tools/enval-verify.mjs",
        "scripts/tools/enval-verify-manifest.mjs",
        "scripts/tools/enval-verify-ownership.mjs",
        "scripts/proofs/enval-verify-runner.proof.mjs",
        "scripts/proofs/enval-verify-ownership.proof.mjs",
      ]),
    }),
    checks: Object.freeze(["node-check-changed"]),
  }),
  Object.freeze({
    id: "local-development-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "scripts/proofs/enval-local-dev.proof.mjs",
        "scripts/tools/enval-local-dev.mjs",
      ]),
    }),
    checks: Object.freeze([
      "node-check-changed",
      "local-runtime-pure",
      "evidence-fact-review-round-served-local",
    ]),
  }),
  Object.freeze({
    id: "customer-information-request-v1",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/features/customer-information-request/CustomerInformationRequestPanel.tsx",
        "app/src/features/customer-information-request/CustomerInformationRequestHistory.tsx",
        "app/src/features/customer-information-request/WorkforceInformationRequestPanel.tsx",
        "app/src/features/customer-information-request/customerInformationRequest.proof.tsx",
        "app/src/features/customer-information-request/customerInformationRequestClient.ts",
        "app/src/features/auth/postLoginNavigation.proof.ts",
        "app/src/features/auth/postLoginNavigation.ts",
        "app/src/pages/DashboardPage.tsx",
        "scripts/proofs/app-customer-information-request.proof.ts",
        "scripts/proofs/app-information-request-email-context.proof.ts",
        "scripts/proofs/app-workflow-email-worker.proof.ts",
        "scripts/proofs/enval-local-dev.proof.mjs",
        "scripts/tools/enval-local-dev.mjs",
        "supabase/functions/_shared/app_control_plane_runtime_reader.ts",
        "supabase/functions/_shared/app_customer_information_request.ts",
        "supabase/functions/_shared/app_workflow_email_context.ts",
        "supabase/functions/_shared/workflow_email_worker.ts",
        "supabase/functions/api-app-customer-information-request/index.ts",
        "supabase/functions/api-app-presentation-bootstrap/index.ts",
        "supabase/functions/api-app-evidence-review-correction-publish/index.ts",
        "supabase/migrations/20260911151223_app_customer_information_request_v1.sql",
        "supabase/migrations/20260912100926_app_customer_information_request_integration_guards.sql",
        "supabase/migrations/20260912121643_app_customer_information_request_history_v1.sql",
        "supabase/migrations/20260913192155_information_request_email_v1.sql",
        "supabase/migrations/20260913202433_app_customer_information_request_withdraw_lock_fix.sql",
        "supabase/migrations/20260914153311_customer_information_request_terminal_at_v1.sql",
        "supabase/migrations/20260921130223_brand_configuration_workflow_email_sender_snapshot_v1.sql",
      ]),
    }),
    checks: Object.freeze([
      "customer-information-request-client-pure",
      "customer-information-request-local",
      "information-request-email-context-pure",
      "workflow-email-worker-pure",
      "evidence-review-case-detail-pure",
      "customer-timeline-client-pure",
      "portal-context-authority-pure",
      "edge-static-check",
      "app-typecheck-build",
      "tenant-migration-chain-local",
      "local-runtime-pure",
    ]),
  }),
  Object.freeze({
    id: "workflow-email-foundation-v1",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "scripts/proofs/app-workflow-email-foundation.proof.ts",
        "scripts/proofs/app-workflow-email-worker.proof.ts",
        "supabase/functions/_shared/local_mailpit_smtp.ts",
        "supabase/functions/_shared/workflow_email_transport.ts",
        "supabase/functions/_shared/workflow_email_worker.ts",
        "supabase/functions/workflow-email-worker/index.ts",
        "supabase/migrations/20260913161612_app_workflow_email_foundation_v1.sql",
        "supabase/migrations/20260921130223_brand_configuration_workflow_email_sender_snapshot_v1.sql",
      ]),
    }),
    checks: Object.freeze([
      "workflow-email-worker-pure",
      "workflow-email-foundation-local",
      "edge-static-check",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "customer-portal-ia-v1-current",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/features/dashboard/CustomerApplicationList.tsx",
        "app/src/features/dashboard/customerPortalIa.proof.tsx",
        "app/src/features/dashboard/dashboardRoutes.ts",
        "supabase/migrations/20260911103144_app_customer_application_index_read_v1.sql",
      ]),
    }),
    checks: Object.freeze([
      "customer-portal-ia-pure",
      "portal-context-authority-pure",
      "post-signing-convergence-pure",
      "customer-portal-application-index-local",
      "edge-static-check",
      "app-typecheck-build",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "customer-case-timeline-v1",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/features/dashboard/ActivePrivateDashboard.tsx",
        "app/src/features/dashboard/CustomerTimeline.tsx",
        "app/src/features/dashboard/dashboardReadClient.proof.ts",
        "app/src/features/dashboard/dashboardReadClient.ts",
        "app/src/features/dashboard/dashboardStatusPresentation.ts",
        "app/src/features/dashboard/dashboardTypes.ts",
        "app/src/styles/components.css",
        "scripts/proofs/app-customer-timeline-client.proof.mjs",
        "scripts/proofs/app-customer-case-timeline.proof.mjs",
        "supabase/functions/api-app-dashboard-get/index.ts",
        "supabase/migrations/20260910195608_app_customer_case_timeline_read_v1.sql",
      ]),
    }),
    checks: Object.freeze([
      "customer-timeline-client-pure",
      "customer-timeline-local",
      "edge-static-check",
      "app-typecheck-build",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "tenant-migration-chain-proof",
    match: Object.freeze({
      type: "exact",
      value: "scripts/proofs/enval-migration-chain.proof.mjs",
    }),
    checks: Object.freeze([
      "node-check-changed",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "tenant-migration-chain-manifest",
    match: Object.freeze({
      type: "exact",
      value: "scripts/tools/enval-migration-chain-manifest.mjs",
    }),
    checks: Object.freeze([
      "node-check-changed",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "signup-proof-migration-provenance",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "scripts/proofs/app-signup-document-first-ui.proof.ts",
        "scripts/proofs/app-signup-party-name-crosscheck.proof.ts",
        "scripts/proofs/app-signup-document-crosscheck-parity.proof.ts",
        "scripts/proofs/app-signup-fact-resolution.proof.ts",
        "scripts/proofs/app-signup-unified-presentation.proof.ts",
        "scripts/proofs/app-signup-energy-document-crosscheck.proof.ts",
        "scripts/proofs/app-signup-signing-layout.proof.ts",
        "scripts/proofs/app-signup-signature-core.proof.ts",
        "scripts/proofs/app-signup-organization-document-first.proof.ts",
        "scripts/proofs/app-signup-document-decision-policy.proof.ts",
        "scripts/proofs/app-signup-fact-applicability-summary.proof.ts",
        "scripts/proofs/app-signup-ean-preflight.proof.ts",
        "scripts/proofs/app-signup-document-first-review.proof.ts",
        "scripts/proofs/app-signup-generic-document-facts.proof.ts",
        "scripts/proofs/app-signup-signed-receipt.proof.ts",
        "scripts/proofs/app-post-signing-convergence.proof.ts",
      ]),
    }),
    checks: Object.freeze(["deno-check-changed"]),
  }),
  Object.freeze({
    id: "signup-resolution-provenance-browser-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/features/signup/presentation/factPresentationModel.ts",
        "app/src/features/signup/signing/canonicalSigningFacts.ts",
        "app/src/features/signup/signing/signingIntent.ts",
        "app/src/features/signup/DocumentFirstSigningSummary.tsx",
        "app/src/features/signup/signupSigningClient.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-app-changed",
      "signup-resolution-provenance-pure",
    ]),
  }),
  Object.freeze({
    id: "signup-resolution-provenance-server-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/functions/_shared/signup_resolution_provenance.ts",
        "supabase/functions/api-app-signup-signing-finalize/index.ts",
        "scripts/proofs/app-signup-resolution-provenance.proof.ts",
        "scripts/proofs/app-signup-signing-runtime.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "signup-resolution-provenance-pure",
    ]),
  }),
  Object.freeze({
    id: "archived-migration-reference-proof",
    match: Object.freeze({
      type: "exact",
      value: "scripts/proofs/enval-archived-migration-reference.proof.mjs",
    }),
    checks: Object.freeze([
      "node-check-changed",
      "archived-migration-reference-pure",
    ]),
  }),
  Object.freeze({
    id: "tenant-current-baseline",
    match: Object.freeze({
      type: "exact",
      value: "supabase/migrations/20260816150000_app_current_baseline.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "tenant-migration-archive",
    match: Object.freeze({
      type: "prefix",
      value: "supabase/migration-archive/",
    }),
    checks: Object.freeze(["tenant-migration-chain-local"]),
  }),
  Object.freeze({
    id: "workforce-policy-foundation-proof",
    match: Object.freeze({
      type: "exact",
      value: "scripts/proofs/app-workforce-policy-foundation.proof.ts",
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "workforce-policy-foundation-local",
    ]),
  }),
  Object.freeze({
    id: "workforce-policy-foundation-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260816160000_app_workforce_policy_foundation.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "workforce-policy-foundation-local",
    ]),
  }),
  Object.freeze({
    id: "compliance-workforce-view-proof",
    match: Object.freeze({
      type: "exact",
      value: "scripts/proofs/app-compliance-workforce-view.proof.ts",
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "compliance-workforce-view-local",
    ]),
  }),
  Object.freeze({
    id: "compliance-workforce-view-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260817120000_app_compliance_workforce_view.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "compliance-workforce-view-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "compliance-source-event-ledger-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "platform/runtime/compliance/delivery_year_compliance_source_event.ts",
        "scripts/proofs/app-compliance-source-event-ledger.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "compliance-source-event-ledger-local",
    ]),
  }),
  Object.freeze({
    id: "compliance-source-event-ledger-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260817160000_app_compliance_source_event_ledger.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "compliance-source-event-ledger-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "compliance-source-event-capture-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/functions/api-app-compliance-source-event/index.ts",
        "scripts/proofs/app-compliance-source-event-capture.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "compliance-source-event-capture-pure",
      "compliance-source-event-capture-local",
    ]),
  }),
  Object.freeze({
    id: "compliance-source-event-capture-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260817190000_app_compliance_source_event_capture.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "compliance-source-event-capture-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "compliance-worklist-read-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "platform/runtime/compliance/delivery_year_compliance_calendar_registry.ts",
        "supabase/functions/_shared/app_compliance_worklist.ts",
        "supabase/functions/api-app-compliance-worklist/index.ts",
        "scripts/proofs/app-compliance-worklist-read.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "compliance-worklist-read-pure",
      "compliance-worklist-read-local",
    ]),
  }),
  Object.freeze({
    id: "compliance-worklist-read-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260817210000_app_compliance_worklist_read.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "compliance-worklist-read-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "parser-observation-foundation-app",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/features/invoice-analysis/documentObservationEnvelope.ts",
        "app/src/features/invoice-analysis/parserObservationProjection.ts",
        "app/src/features/signup/DocumentFirstDocumentsStep.tsx",
        "app/src/features/signup/documentFactRegistry.ts",
        "app/src/features/signup/signupQuarantineUploadClient.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-app-changed",
      "parser-observation-foundation-pure",
      "parser-observation-foundation-local",
      "parser-current-intake-served-local",
    ]),
  }),
  Object.freeze({
    id: "parser-observation-foundation-server",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "platform/runtime/document-parsing/document_fact_vocabulary.ts",
        "platform/runtime/document-parsing/document_parser_contract.ts",
        "platform/runtime/document-parsing/document_parser_profiles.ts",
        "platform/runtime/document-parsing/document_parser_core.ts",
        "supabase/functions/_shared/app_document_parser_pdf_adapter.ts",
        "supabase/functions/_shared/app_parser_observation_persistence.ts",
        "supabase/functions/_shared/signup_quarantine.ts",
        "supabase/functions/api-app-signup-upload-confirm/index.ts",
        "scripts/proofs/app-parser-observation-foundation.proof.ts",
        "scripts/proofs/app-parser-current-intake-served.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "parser-observation-foundation-pure",
      "parser-observation-foundation-local",
      "parser-current-intake-served-local",
    ]),
  }),
  Object.freeze({
    id: "parser-observation-foundation-migration",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/migrations/20260820180000_app_parser_observation_foundation.sql",
        "supabase/migrations/20260820183000_app_parser_observation_acl_fix.sql",
      ]),
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "parser-observation-foundation-local",
      "parser-current-intake-served-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-decision-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/functions/api-app-evidence-review-decision/index.ts",
        "scripts/proofs/app-evidence-review-decision.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "evidence-review-decision-pure",
      "evidence-review-foundation-local",
      "evidence-review-case-detail-local",
      "evidence-review-worklist-read-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-foundation-proof",
    match: Object.freeze({
      type: "exact",
      value: "scripts/proofs/app-evidence-review-foundation.proof.ts",
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "evidence-review-decision-pure",
      "evidence-review-foundation-pure",
      "evidence-review-foundation-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-foundation-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260817230000_app_evidence_review_foundation.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "evidence-review-foundation-pure",
      "evidence-review-foundation-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-correction-details-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260818210000_app_evidence_review_correction_details.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "evidence-review-decision-pure",
      "evidence-review-foundation-pure",
      "evidence-review-foundation-local",
      "evidence-review-case-detail-local",
      "evidence-review-worklist-read-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-case-detail-decide-affordance-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260818220000_app_evidence_review_case_detail_decide_affordance.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "evidence-review-decision-pure",
      "evidence-review-foundation-pure",
      "evidence-review-foundation-local",
      "evidence-review-case-detail-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-worklist-read-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/functions/_shared/app_evidence_review_worklist.ts",
        "supabase/functions/_shared/app_evidence_review_overall_status.ts",
        "supabase/functions/api-app-evidence-review-worklist/index.ts",
        "scripts/proofs/app-evidence-review-worklist-read.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "evidence-review-worklist-read-pure",
      "evidence-review-worklist-read-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-worklist-read-migration",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/migrations/20260818090000_app_evidence_review_worklist_read.sql",
        "supabase/migrations/20260819120000_app_evidence_review_worklist_fact_cutover.sql",
        "supabase/migrations/20260819123000_app_evidence_review_worklist_activity_fix.sql",
      ]),
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "evidence-review-worklist-read-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-overall-status-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260819160000_app_evidence_review_overall_status.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "evidence-review-worklist-read-local",
      "evidence-review-case-detail-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-case-detail-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/functions/_shared/app_evidence_review_case_detail.ts",
        "supabase/functions/_shared/app_evidence_review_overall_status.ts",
        "supabase/functions/api-app-evidence-review-case-detail/index.ts",
        "scripts/proofs/app-evidence-review-case-detail.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "evidence-review-case-detail-pure",
      "evidence-fact-review-round-pure",
      "evidence-review-case-detail-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-fact-review-round-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/functions/api-app-evidence-review-round-finalize/index.ts",
        "scripts/proofs/app-evidence-fact-review-round.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "evidence-fact-review-round-pure",
      "evidence-review-case-detail-local",
      "evidence-fact-review-round-served-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-fact-review-round-served-proof",
    match: Object.freeze({
      type: "exact",
      value: "scripts/proofs/app-evidence-fact-review-round-served.proof.mjs",
    }),
    checks: Object.freeze([
      "node-check-changed",
      "evidence-fact-review-round-served-local",
      "correction-handoff-supersession-served-local",
      "customer-correction-replacement-served-local",
      "customer-correction-document-finalization-finalize-reentry-served-local",
    ]),
  }),
  Object.freeze({
    id: "correction-handoff-supersession-served-proof",
    match: Object.freeze({
      type: "exact",
      value:
        "scripts/proofs/app-correction-handoff-supersession-served.proof.mjs",
    }),
    checks: Object.freeze([
      "node-check-changed",
      "correction-handoff-supersession-served-local",
      "customer-correction-replacement-served-local",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-replacement-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "platform/runtime/document-parsing/document_parser_contract.ts",
        "supabase/functions/_shared/app_customer_correction_replacement.ts",
        "supabase/functions/_shared/app_evidence_review_correction_handoff.ts",
        "supabase/functions/_shared/app_parser_observation_persistence.ts",
        "supabase/functions/_shared/signup_quarantine.ts",
        "supabase/functions/api-app-customer-correction-upload-url/index.ts",
        "supabase/functions/api-app-customer-correction-upload-confirm/index.ts",
        "supabase/functions/api-app-customer-correction-upload-remove/index.ts",
        "supabase/functions/api-app-customer-correction-handoff/index.ts",
        "scripts/proofs/app-customer-correction-withdrawal.proof.ts",
        "scripts/proofs/app-customer-correction-replacement.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "customer-correction-withdrawal-pure",
      "customer-correction-replacement-pure",
      "customer-correction-replacement-served-local",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-candidate-selection-migrations",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/migrations/20260823120000_app_customer_correction_candidate_selection.sql",
        "supabase/migrations/20260823123000_app_customer_correction_resolution_null_projection.sql",
        "supabase/migrations/20260823130000_app_customer_correction_withdraw_scope_fix.sql",
        "supabase/migrations/20260823133000_app_customer_correction_handoff_null_fix.sql",
        "supabase/migrations/20260823134500_app_customer_correction_handoff_preflight_fix.sql",
      ]),
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "customer-correction-withdrawal-pure",
      "customer-correction-replacement-pure",
      "customer-correction-replacement-served-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-replacement-served-proof",
    match: Object.freeze({
      type: "exact",
      value:
        "scripts/proofs/app-customer-correction-replacement-served.proof.mjs",
    }),
    checks: Object.freeze([
      "node-check-changed",
      "customer-correction-replacement-served-local",
      "customer-correction-document-finalization-finalize-reentry-served-local",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-replacement-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260820220000_app_customer_correction_replacement_staging.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "customer-correction-replacement-pure",
      "parser-observation-foundation-local",
      "customer-correction-replacement-served-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "correction-handoff-supersession-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/functions/api-app-evidence-review-correction-supersede/index.ts",
        "scripts/proofs/app-correction-handoff-supersession.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "correction-handoff-supersession-pure",
      "correction-handoff-supersession-served-local",
    ]),
  }),
  Object.freeze({
    id: "correction-cover-message-v1",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/features/dashboard/CustomerCorrectionHandoffPanel.tsx",
        "app/src/features/dashboard/customerCorrectionHandoffClient.ts",
        "app/src/features/evidence-review/EvidenceReviewCaseDetailPage.tsx",
        "app/src/features/evidence-review/evidenceReviewDetailClient.ts",
        "app/src/features/evidence-review/useEvidenceCorrectionPublish.ts",
        "supabase/functions/_shared/app_evidence_review_correction_handoff.ts",
        "supabase/functions/api-app-customer-correction-handoff/index.ts",
        "supabase/functions/api-app-evidence-review-correction-publish/index.ts",
        "supabase/functions/api-app-evidence-review-correction-supersede/index.ts",
        "scripts/proofs/app-correction-cover-message.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "deno-check-app-changed",
      "correction-cover-message-pure",
      "evidence-review-correction-handoff-pure",
      "customer-correction-handoff-v2-pure",
      "correction-handoff-supersession-pure",
      "evidence-review-case-detail-ui-pure",
    ]),
  }),
  Object.freeze({
    id: "correction-cover-message-v1-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260916120526_correction_cover_message_v1.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "correction-cover-message-pure",
      "evidence-review-correction-handoff-local",
      "correction-handoff-supersession-served-local",
      "evidence-fact-review-round-served-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "correction-handoff-supersession-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260820210000_app_correction_handoff_supersession.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "correction-handoff-supersession-pure",
      "customer-correction-handoff-v2-pure",
      "customer-correction-submission-pure",
      "correction-handoff-supersession-served-local",
      "evidence-review-worklist-read-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-correction-handoff-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/functions/_shared/app_evidence_review_correction_handoff.ts",
        "supabase/functions/api-app-customer-correction-handoff/index.ts",
        "supabase/functions/api-app-evidence-review-correction-publish/index.ts",
        "scripts/proofs/app-evidence-review-correction-handoff.proof.ts",
        "scripts/proofs/app-customer-correction-handoff-contract-v2.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "evidence-review-correction-handoff-pure",
      "customer-correction-handoff-v2-pure",
      "customer-correction-handoff-v2-local-read",
      "evidence-review-correction-handoff-local",
      "evidence-fact-review-round-served-local",
      "evidence-review-worklist-read-local",
      "evidence-review-case-detail-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-correction-handoff-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260819190000_app_evidence_review_correction_handoff.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "evidence-review-correction-handoff-pure",
      "evidence-review-correction-handoff-local",
      "evidence-fact-review-round-served-local",
      "evidence-review-worklist-read-local",
      "evidence-review-case-detail-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-handoff-fact-projection-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260822120000_app_customer_correction_handoff_fact_projection.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "evidence-review-correction-handoff-pure",
      "customer-correction-handoff-v2-pure",
      "evidence-review-correction-handoff-local",
      "customer-correction-handoff-v2-local-read",
      "evidence-fact-review-round-served-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-correction-publication-target-fix-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260819220000_app_evidence_review_correction_publication_target_fix.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "evidence-review-correction-handoff-pure",
      "evidence-review-correction-handoff-local",
      "evidence-fact-review-round-served-local",
      "evidence-review-worklist-read-local",
      "evidence-review-case-detail-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-correction-publish-affordance-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260819210000_app_evidence_review_correction_publish_affordance.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "evidence-review-case-detail-pure",
      "evidence-review-case-detail-ui-pure",
      "evidence-review-case-detail-local",
      "evidence-fact-review-round-served-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-resolution-routing-app",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/features/dashboard/CustomerCorrectionHandoffPanel.tsx",
        "app/src/features/dashboard/customerCorrectionHandoffClient.ts",
        "app/src/features/dashboard/customerCorrectionWorkspace.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-app-changed",
      "customer-correction-resolution-routing-pure",
      "customer-correction-resolution-routing-served",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-resolution-routing-server",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/functions/_shared/app_customer_correction_resolution.ts",
        "supabase/functions/_shared/app_customer_correction_submission.ts",
        "supabase/functions/api-app-customer-correction-signing-challenge/index.ts",
        "supabase/functions/api-app-customer-correction-signing-finalize/index.ts",
        "scripts/proofs/app-customer-correction-resolution-routing.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "customer-correction-resolution-routing-pure",
      "customer-correction-resolution-routing-served",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-resolution-routing-served-proof",
    match: Object.freeze({
      type: "exact",
      value:
        "scripts/proofs/app-customer-correction-resolution-routing-served.proof.mjs",
    }),
    checks: Object.freeze([
      "node-check-changed",
      "customer-correction-resolution-routing-pure",
      "customer-correction-resolution-routing-served",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-resolution-routing-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260824100000_app_customer_correction_resolution_routing.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "customer-correction-resolution-routing-pure",
      "customer-correction-resolution-routing-served",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-document-finalization-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/functions/_shared/app_customer_correction_submission.ts",
        "supabase/functions/api-app-customer-correction-signing-challenge/index.ts",
        "supabase/functions/api-app-customer-correction-signing-finalize/index.ts",
        "scripts/proofs/app-customer-correction-document-finalization.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "customer-correction-document-finalization-pure",
      "customer-correction-document-finalization-upload-candidate-served-local",
      "customer-correction-document-finalization-challenge-replay-served-local",
      "customer-correction-document-finalization-finalize-reentry-served-local",
      "customer-correction-document-finalization-transactional-rollback-served-local",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-document-finalization-served-proof",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "scripts/proofs/app-customer-correction-document-finalization-served.proof.mjs",
        "scripts/proofs/app-customer-correction-document-finalization-upload-candidate-served.proof.mjs",
        "scripts/proofs/app-customer-correction-document-finalization-challenge-replay-served.proof.mjs",
        "scripts/proofs/app-customer-correction-document-finalization-finalize-reentry-served.proof.mjs",
        "scripts/proofs/app-customer-correction-document-finalization-transactional-rollback-served.proof.mjs",
      ]),
    }),
    checks: Object.freeze([
      "node-check-changed",
      "customer-correction-document-finalization-upload-candidate-served-local",
      "customer-correction-document-finalization-challenge-replay-served-local",
      "customer-correction-document-finalization-finalize-reentry-served-local",
      "customer-correction-document-finalization-transactional-rollback-served-local",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-document-finalization-migration",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/migrations/20260821100000_app_customer_correction_document_finalization.sql",
        "supabase/migrations/20260917180214_customer_correction_terminal_projection_null_v1.sql",
        "supabase/migrations/20260918073504_customer_correction_terminal_fact_projection_v1.sql",
        "supabase/migrations/20260918081541_customer_correction_terminal_fact_projection_customer_binding_fix_v1.sql",
      ]),
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "customer-correction-document-finalization-pure",
      "customer-correction-submission-pure",
      "customer-correction-document-finalization-finalize-reentry-served-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-submission-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/functions/_shared/app_customer_correction_submission.ts",
        "supabase/functions/_shared/signing_otp_transport.ts",
        "supabase/functions/api-app-customer-correction-signing-challenge/index.ts",
        "supabase/functions/api-app-customer-correction-signing-finalize/index.ts",
        "scripts/proofs/app-customer-correction-submission.proof.ts",
        "scripts/proofs/app-customer-correction-handoff-contract-v2.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "customer-correction-submission-pure",
      "customer-correction-handoff-v2-pure",
      "customer-correction-handoff-v2-local-read",
      "customer-correction-submission-local-read",
      "evidence-fact-review-round-served-local",
      "evidence-review-worklist-read-local",
      "evidence-review-case-detail-local",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-submission-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260820090000_app_customer_correction_submissions.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "customer-correction-submission-pure",
      "customer-correction-submission-local-read",
      "evidence-fact-review-round-served-local",
      "evidence-review-worklist-read-local",
      "evidence-review-case-detail-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-handoff-v2-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260820120000_app_customer_correction_handoff_contract_v2.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "customer-correction-handoff-v2-pure",
      "customer-correction-handoff-v2-local-read",
      "customer-correction-submission-pure",
      "customer-correction-submission-local-read",
      "evidence-review-correction-handoff-pure",
      "evidence-fact-review-round-served-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-signer-authority-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260820150000_app_customer_correction_signer_authority.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "customer-correction-submission-pure",
      "customer-correction-submission-local-read",
      "customer-correction-handoff-v2-pure",
      "customer-correction-handoff-v2-local-read",
      "customer-correction-handoff-ui-pure",
      "evidence-review-correction-handoff-pure",
      "evidence-fact-review-round-served-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-fact-review-round-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260818230000_app_evidence_fact_review_rounds.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "evidence-fact-review-round-pure",
      "evidence-review-case-detail-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-current-round-read-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260819090000_app_evidence_review_current_round_read.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "evidence-review-case-detail-pure",
      "evidence-review-case-detail-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-case-detail-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260818120000_app_evidence_review_case_detail_read.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "evidence-review-case-detail-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "signup-resolution-provenance-projection-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260818180000_app_signup_resolution_provenance_projection.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "signup-resolution-provenance-pure",
      "evidence-review-case-detail-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-preview-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/functions/_shared/app_evidence_review_preview.ts",
        "supabase/functions/api-app-evidence-review-preview/index.ts",
        "scripts/proofs/app-evidence-review-preview.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "evidence-review-preview-pure",
      "evidence-review-preview-local",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-preview-migration",
    match: Object.freeze({
      type: "exact",
      value:
        "supabase/migrations/20260818150000_app_evidence_review_preview_read.sql",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "evidence-review-preview-local",
      "tenant-migration-chain-local",
    ]),
  }),
  Object.freeze({
    id: "authenticated-internal-navigation",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/App.tsx",
        "app/src/routes/types.ts",
        "app/src/features/auth/AccountPage.tsx",
        "app/src/features/auth/AuthProvider.tsx",
        "app/src/features/auth/DashboardRouteGuard.tsx",
        "app/src/features/auth/authBootstrapClient.ts",
        "app/src/features/auth/authTypes.ts",
        "app/src/features/auth/authorizedPortalAccess.ts",
        "app/src/features/auth/postLoginNavigation.ts",
        "app/src/features/auth/postLoginNavigation.proof.ts",
        "app/src/features/dashboard/DashboardSidebar.tsx",
        "app/src/features/evidence-review/evidenceReviewWorklistClient.ts",
        "app/src/features/evidence-review/evidenceReviewRoutes.ts",
        "app/src/features/operator/OperatorRouteGuard.tsx",
        "app/src/features/operator/operatorNavigation.ts",
        "app/src/features/operator/operatorContextClient.ts",
        "app/src/shared/components/AppHeader.tsx",
        "app/src/pages/ComplianceWorklistPage.tsx",
        "app/src/pages/EvidenceReviewCaseDetailPage.tsx",
        "app/src/pages/EvidenceReviewWorklistPage.tsx",
        "app/src/pages/OperatorOverviewPage.tsx",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-app-changed",
      "authenticated-internal-navigation-pure",
    ]),
  }),
  Object.freeze({
    id: "auth-ux-recovery-resend",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/App.tsx",
        "app/src/pages/AccountPage.tsx",
        "app/src/features/auth/AccountPage.tsx",
        "app/src/features/auth/AuthEmailRequestPage.tsx",
        "app/src/features/auth/AuthPageLayout.tsx",
        "app/src/features/auth/AuthProvider.tsx",
        "app/src/features/auth/PasswordRecoveryPage.tsx",
        "app/src/features/auth/authClient.ts",
        "app/src/features/auth/authErrorMapping.ts",
        "app/src/features/auth/authTypes.ts",
        "app/src/features/auth/authUxFlow.ts",
        "app/src/features/auth/authUxFlow.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-app-changed",
      "auth-ux-recovery-resend-pure",
    ]),
  }),
  Object.freeze({
    id: "auth-shell-f01",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/App.tsx",
        "app/src/pages/AccountPage.tsx",
        "app/src/pages/HomePage.tsx",
        "app/src/features/auth/authShell.proof.tsx",
        "app/src/features/signup/SignupPageShell.tsx",
        "app/src/shared/components/AppHeader.tsx",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-app-changed",
      "public-auth-identity-v1-pure",
    ]),
  }),
  Object.freeze({
    id: "compliance-worklist-ui",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/App.tsx",
        "app/src/features/compliance/ComplianceWorklistPage.proof.tsx",
        "app/src/features/compliance/ComplianceWorklistPage.tsx",
        "app/src/features/compliance/complianceWorklistClient.ts",
        "app/src/features/compliance/useComplianceWorklist.ts",
        "app/src/pages/ComplianceWorklistPage.tsx",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-app-changed",
      "compliance-worklist-ui-pure",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-worklist-ui",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/App.tsx",
        "app/src/features/evidence-review/EvidenceReviewWorklistPage.proof.tsx",
        "app/src/features/evidence-review/EvidenceReviewWorklistPage.tsx",
        "app/src/features/evidence-review/evidenceReviewRoutes.ts",
        "app/src/features/evidence-review/evidenceReviewWorklistClient.ts",
        "app/src/features/evidence-review/useEvidenceReviewWorklist.ts",
        "app/src/pages/EvidenceReviewWorklistPage.tsx",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-app-changed",
      "evidence-review-worklist-ui-pure",
    ]),
  }),
  Object.freeze({
    id: "evidence-review-case-detail-ui",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/App.tsx",
        "app/src/features/auth/postLoginNavigation.proof.ts",
        "app/src/features/auth/postLoginNavigation.ts",
        "app/src/features/documents/documentDownloadClient.ts",
        "app/src/features/evidence-review/EvidenceReviewCaseDetailPage.proof.tsx",
        "app/src/features/evidence-review/EvidenceReviewCaseDetailPage.tsx",
        "app/src/features/evidence-review/EvidenceReviewPreviewPane.tsx",
        "app/src/features/evidence-review/EvidenceReviewWorklistPage.proof.tsx",
        "app/src/features/evidence-review/EvidenceReviewWorklistPage.tsx",
        "app/src/features/evidence-review/evidenceReviewDetailClient.ts",
        "app/src/features/evidence-review/evidenceReviewRoutes.ts",
        "app/src/features/evidence-review/useEvidenceFactReviewDraft.ts",
        "app/src/features/evidence-review/useEvidenceCorrectionPublish.ts",
        "app/src/features/evidence-review/useEvidenceReviewCaseDetail.ts",
        "app/src/pages/EvidenceReviewCaseDetailPage.tsx",
        "app/src/pages/EvidenceReviewWorklistPage.tsx",
        "app/src/styles/components.css",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-app-changed",
      "evidence-review-case-detail-ui-pure",
      "evidence-review-case-detail-local",
      "evidence-review-preview-local",
    ]),
  }),
  Object.freeze({
    id: "legacy-dev-unlock-caller-html",
    match: Object.freeze({
      type: "exact",
      value: "dossier.html",
    }),
    checks: Object.freeze([
      "legacy-dev-unlock-caller-retirement-pure",
      "legacy-ev-direct-caller-retirement-pure",
    ]),
  }),
  Object.freeze({
    id: "legacy-dev-unlock-caller-javascript",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "assets/js/pages/dossier.js",
        "scripts/proofs/legacy-dev-unlock-caller-retirement.proof.mjs",
        "scripts/proofs/legacy-access-update-caller-retirement.proof.mjs",
      ]),
    }),
    checks: Object.freeze([
      "node-check-changed",
      "legacy-dev-unlock-caller-retirement-pure",
      "legacy-access-update-caller-retirement-pure",
    ]),
  }),
  Object.freeze({
    id: "legacy-ev-direct-caller-retirement-javascript",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "assets/js/eligibility.js",
        "assets/js/script.js",
        "scripts/proofs/legacy-ev-direct-caller-retirement.proof.mjs",
      ]),
    }),
    checks: Object.freeze([
      "node-check-changed",
      "legacy-ev-direct-caller-retirement-pure",
    ]),
  }),
  Object.freeze({
    id: "legacy-ev-direct-caller-retirement-static",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "_redirects",
        "aanmelden.html",
        "hoe-het-werkt.html",
        "index.html",
        "pricing.html",
        "privacyverklaring.html",
        "regelgeving.html",
        "voorwaarden.html",
        "sitemap.xml",
      ]),
    }),
    checks: Object.freeze([
      "legacy-ev-direct-caller-retirement-pure",
    ]),
  }),
  Object.freeze({
    id: "browser-webapi-proof",
    match: Object.freeze({
      type: "exact",
      value: "scripts/proofs/app-signup-signing-kiss.proof.ts",
    }),
    checks: Object.freeze(["deno-check-browser-proof"]),
  }),
  Object.freeze({
    id: "ops-location-authoritative-gate",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/functions/_shared/app_workforce_authorization.ts",
        "supabase/functions/api-app-ops-location-observation-record/index.ts",
        "supabase/functions/api-app-ops-location-root-create/index.ts",
        "supabase/functions/api-app-ops-location-version-accept/index.ts",
        "supabase/functions/api-app-ops-location-version-correct/index.ts",
        "scripts/proofs/api-app-ops-location-callers.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "ops-location-authoritative-gate-pure",
      "app-tenant-resolution-shadow-pure",
      "app-tenant-resolution-shadow-local",
    ]),
  }),
  Object.freeze({
    id: "trusted-ingress-boundary",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "platform/runtime/tenant-resolution/trusted_ingress.ts",
        "platform/runtime/tenant-resolution/tenant_resolution.ts",
        "supabase/functions/_shared/app_foundation.ts",
        "supabase/functions/_shared/app_tenant_resolution_shadow.ts",
        "scripts/proofs/trusted-ingress-boundary.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "trusted-ingress-boundary-pure",
    ]),
  }),
  Object.freeze({
    id: "app-tenant-resolution-shadow",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "supabase/functions/_shared/app_foundation.ts",
        "supabase/functions/_shared/app_tenant_resolution_shadow.ts",
        "scripts/proofs/app-tenant-resolution-shadow.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "app-tenant-resolution-shadow-pure",
      "app-tenant-resolution-shadow-local",
    ]),
  }),
  Object.freeze({
    id: "delivery-year-compliance",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "platform/runtime/compliance/delivery_year_compliance.ts",
        "platform/runtime/compliance/compliance_action_plan.ts",
        "platform/runtime/compliance/compliance_worklist.ts",
        "scripts/proofs/delivery-year-compliance.proof.ts",
        "scripts/proofs/compliance-action-plan.proof.ts",
        "scripts/proofs/compliance-worklist.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "delivery-year-compliance-pure",
      "compliance-action-plan-pure",
      "compliance-worklist-pure",
    ]),
  }),
  Object.freeze({
    id: "tenant-resolution-composition",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "platform/runtime/tenant-resolution/tenant_resolution.ts",
        "platform/runtime/tenant-resolution/tenant_resolution_composition.ts",
        "platform/runtime/tenant-resolution/trusted_ingress.ts",
        "platform/runtime/tenant-resolution/adapters/platform_control_plane_v1.ts",
        "platform/runtime/tenant-resolution/adapters/static_single_tenant_v1.ts",
        "scripts/proofs/tenant-resolution-composition.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "tenant-resolution-composition-pure",
      "trusted-ingress-boundary-pure",
    ]),
  }),
  Object.freeze({
    id: "presentation-brand-config",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "platform/runtime/presentation/presentation_brand_config.ts",
        "platform/runtime/presentation/enval_presentation_defaults.ts",
        "scripts/proofs/presentation-brand-config.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "presentation-brand-config-pure",
    ]),
  }),
  Object.freeze({
    id: "presentation-brand-sources",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "platform/runtime/presentation/presentation_brand_source.ts",
        "platform/runtime/presentation/presentation_source_composition.ts",
        "platform/runtime/presentation/adapters/platform_control_plane_presentation_v1.ts",
        "platform/runtime/presentation/adapters/static_presentation_config_v1.ts",
        "scripts/proofs/presentation-brand-sources.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "presentation-brand-sources-pure",
      "control-plane-foundation-local",
    ]),
  }),
  Object.freeze({
    id: "document-fact-matrix",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/features/documents/DocumentEvidenceUploadCard.tsx",
        "app/src/features/documents/DocumentEvidenceWorkflow.tsx",
        "app/src/features/documents/DocumentEvidenceWorkflow.proof.tsx",
        "app/src/features/documents/CustomerDocumentFactInteraction.tsx",
        "app/src/features/documents/CustomerDocumentFactInteraction.proof.tsx",
        "app/src/features/documents/DocumentFactMatrix.tsx",
        "app/src/features/documents/DocumentFactMatrix.proof.tsx",
        "app/src/features/documents/customerDocumentFactRegistry.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-app-changed",
      "document-evidence-workflow-pure",
      "customer-document-interaction-c3c8-pure",
      "document-fact-matrix-pure",
      "customer-document-fact-registry-c3c7-pure",
      "signup-unified-presentation-pure",
      "customer-correction-handoff-ui-pure",
    ]),
  }),
  Object.freeze({
    id: "document-parser-normalization",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "platform/runtime/document-parsing/document_parser_core.ts",
        "app/src/features/invoice-analysis/invoicePdfParserAdapter.ts",
        "supabase/functions/_shared/app_document_parser_pdf_adapter.ts",
        "scripts/proofs/app-document-parser-normalization.proof.ts",
        "scripts/proofs/app-document-parser-fresh-installation.proof.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "document-parser-normalization-pure",
      "parser-observation-foundation-pure",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-handoff-ui",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/features/dashboard/ActivePrivateDashboard.tsx",
        "app/src/features/dashboard/CustomerCorrectionHandoffPanel.tsx",
        "app/src/features/dashboard/DashboardPageShell.tsx",
        "app/src/features/dashboard/DashboardSidebar.tsx",
        "app/src/features/dashboard/customerCorrectionHandoffClient.ts",
        "app/src/features/dashboard/customerCorrectionHandoff.proof.tsx",
        "app/src/features/dashboard/customerCorrectionReplacementUpload.ts",
        "app/src/features/dashboard/customerCorrectionWorkspace.ts",
        "app/src/features/dashboard/useCustomerCorrectionHandoff.ts",
        "app/src/features/documents/documentUploadTransport.ts",
        "app/src/features/signup/signing/SignerPanel.tsx",
        "app/src/features/signup/signing/methods/typedNameOtpV1.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-app-changed",
      "customer-correction-handoff-ui-pure",
      "customer-document-interaction-c3c8-pure",
      "customer-document-fact-registry-c3c7-pure",
      "evidence-fact-review-round-served-local",
    ]),
  }),
  Object.freeze({
    id: "customer-correction-invalid-style",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/styles/components.css",
        "app/src/styles/global.css",
        "assets/css/form-states.css",
        "assets/css/style.css",
      ]),
    }),
    checks: Object.freeze(["customer-correction-handoff-ui-pure"]),
  }),
  Object.freeze({
    id: "presentation-brand-consumers",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/main.tsx",
        "app/src/shared/presentation/PresentationBrandProvider.tsx",
        "app/src/shared/presentation/PresentationBrandRuntime.tsx",
        "app/src/shared/presentation/PresentationBrandProvider.proof.tsx",
        "app/src/shared/components/AppHeader.tsx",
        "app/src/pages/HomePage.tsx",
        "app/src/pages/PrivacyPage.tsx",
        "app/src/pages/AccountPage.tsx",
        "app/src/features/auth/AccountPage.tsx",
        "app/src/features/auth/AuthPageLayout.tsx",
        "app/src/features/dashboard/ActivePrivateDashboard.tsx",
        "app/src/features/dashboard/ContactChoicePanel.tsx",
        "app/src/features/dashboard/DashboardSidebar.tsx",
        "app/src/features/documents/DocumentFactMatrix.tsx",
        "app/src/features/signup/SignupConnectionSection.tsx",
        "app/src/features/signup/SignupSubmitStatusPanel.tsx",
        "app/src/features/signup/ConsentSignatureSection.tsx",
        "app/src/features/signup/DocumentFirstSigningSummary.tsx",
        "app/src/features/signup/signing/browserHtmlLegalBundleV1.ts",
        "app/src/features/signup/signing/legalBundleDocument.ts",
        "app/src/features/signup/presentation/FactTable.tsx",
        "app/src/features/evidence-review/evidenceReviewStatusPresentation.ts",
        "app/src/pages/NotFoundPage.tsx",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-app-changed",
      "presentation-brand-consumers-pure",
    ]),
  }),
  Object.freeze({
    id: "app-presentation-server-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "platform/control-plane/supabase/config.toml",
        "supabase/functions/_shared/app_control_plane_runtime_reader.ts",
        "supabase/functions/_shared/app_presentation_bootstrap.ts",
        "supabase/functions/api-app-presentation-bootstrap/index.ts",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "app-presentation-runtime-pure",
    ]),
  }),
  Object.freeze({
    id: "app-presentation-browser-runtime",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([
        "app/src/features/auth/authRuntimeConfig.ts",
        "app/src/shared/presentation/PresentationBrandRuntime.tsx",
        "app/src/shared/presentation/presentationBootstrapClient.ts",
        "app/src/shared/presentation/PresentationBrandRuntime.proof.tsx",
      ]),
    }),
    checks: Object.freeze([
      "deno-check-app-changed",
      "app-presentation-runtime-pure",
      "presentation-brand-consumers-pure",
    ]),
  }),
  Object.freeze({
    id: "control-plane-foundation-proof",
    match: Object.freeze({
      type: "exact",
      value: "scripts/proofs/platform-control-plane-foundation.proof.ts",
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "control-plane-foundation-local",
    ]),
  }),
  Object.freeze({
    id: "control-plane-target-guard",
    match: Object.freeze({
      type: "exact",
      value: "scripts/tools/enval-supabase-target.mjs",
    }),
    checks: Object.freeze([
      "node-check-changed",
      "control-plane-foundation-local",
    ]),
  }),
  Object.freeze({
    id: "control-plane-runtime",
    match: Object.freeze({
      type: "prefix",
      value: "platform/runtime/tenant-resolution/",
    }),
    checks: Object.freeze([
      "deno-check-changed",
      "control-plane-foundation-local",
    ]),
  }),
  Object.freeze({
    id: "control-plane-migration",
    match: Object.freeze({
      type: "prefix",
      value: "platform/control-plane/supabase/migrations/",
    }),
    checks: Object.freeze([
      "migration-or-sql-review",
      "control-plane-foundation-local",
    ]),
  }),
  Object.freeze({
    id: "control-plane-config",
    match: Object.freeze({
      type: "exact",
      value: "platform/control-plane/supabase/config.toml",
    }),
    checks: Object.freeze(["control-plane-foundation-local"]),
  }),
  Object.freeze({
    id: "local-readonly-sql-fixture",
    match: Object.freeze({
      type: "exact",
      value: "scripts/proofs/enval-local-readonly-catalog.proof.sql",
    }),
    checks: Object.freeze(["local-readonly-catalog"]),
  }),
  Object.freeze({
    id: "invoice-browser-precheck",
    match: Object.freeze({
      type: "exact",
      value: "scripts/tools/invoice-image-precheck.mjs",
    }),
    checks: Object.freeze([
      "node-check-changed",
      "invoice-browser-precheck-gated",
    ]),
  }),
  Object.freeze({
    id: "remote-baseline-node-proof",
    match: Object.freeze({
      type: "exact",
      value: "scripts/proofs/in-place-baseline-phase0-proof.mjs",
    }),
    checks: Object.freeze(["remote-baseline-proof"]),
  }),
  Object.freeze({
    id: "remote-postgrest-node-proof",
    match: Object.freeze({
      type: "exact",
      value: "scripts/proofs/postgrest-authorized-health.proof.mjs",
    }),
    checks: Object.freeze(["remote-postgrest-proof"]),
  }),
  Object.freeze({
    id: "remote-readonly-sql",
    match: Object.freeze({ type: "suffix", value: "-remote-readonly.sql" }),
    checks: Object.freeze(["remote-sql-review"]),
  }),
  Object.freeze({
    id: "baseline-proposals",
    match: Object.freeze({
      type: "prefix",
      value: "supabase/baseline-proposals/",
    }),
    checks: Object.freeze(["baseline-proposal-review"]),
  }),
  Object.freeze({
    id: "migration-and-sql",
    match: Object.freeze({ type: "suffix", value: ".sql" }),
    checks: Object.freeze(["migration-or-sql-review"]),
  }),
  Object.freeze({
    id: "dependency-locks",
    match: Object.freeze({
      type: "basenameOneOf",
      value: Object.freeze([
        "deno.lock",
        "package-lock.json",
        "npm-shrinkwrap.json",
        "pnpm-lock.yaml",
        "yarn.lock",
      ]),
    }),
    checks: Object.freeze(["dependency-review"]),
  }),
  Object.freeze({
    id: "package-manifests",
    match: Object.freeze({
      type: "basenameOneOf",
      value: Object.freeze([
        "package.json",
        "deno.json",
        "deno.jsonc",
        "import_map.json",
      ]),
    }),
    checks: Object.freeze(["dependency-review"]),
  }),
  Object.freeze({
    id: "javascript-static",
    match: Object.freeze({
      type: "extensions",
      value: Object.freeze([".js", ".mjs", ".cjs"]),
    }),
    checks: Object.freeze(["node-check-changed"]),
  }),
  Object.freeze({
    id: "signup-client-typescript",
    match: Object.freeze({ type: "prefix", value: "app/src/features/signup/" }),
    checks: Object.freeze([
      "deno-check-app-changed",
      "signup-journey-pure",
      "signup-unified-presentation-pure",
      "customer-document-fact-registry-c3c7-pure",
    ]),
  }),
  Object.freeze({
    id: "app-typescript-static",
    match: Object.freeze({ type: "prefix", value: "app/src/" }),
    checks: Object.freeze(["deno-check-app-changed"]),
  }),
  Object.freeze({
    id: "typescript-static",
    match: Object.freeze({
      type: "extensions",
      value: Object.freeze([".ts", ".tsx", ".mts", ".cts"]),
    }),
    checks: Object.freeze(["deno-check-changed"]),
  }),
  Object.freeze({
    id: "shell-and-python",
    match: Object.freeze({
      type: "extensions",
      value: Object.freeze([".sh", ".py"]),
    }),
    checks: Object.freeze(["shell-or-python-review"]),
  }),
  Object.freeze({
    id: "static-content",
    match: Object.freeze({
      type: "extensions",
      value: Object.freeze([
        ".css",
        ".html",
        ".json",
        ".md",
        ".svg",
        ".toml",
        ".txt",
        ".yaml",
        ".yml",
      ]),
    }),
    checks: Object.freeze([]),
  }),
  Object.freeze({
    id: "extensionless-repo-contract",
    match: Object.freeze({
      type: "oneOf",
      value: Object.freeze([".gitignore", "AGENTS.md"]),
    }),
    checks: Object.freeze([]),
  }),
]);

export const MIGRATION_BASELINE = Object.freeze({
  exceptions: Object.freeze({}),
});

export const MIGRATION_INVENTORIES = Object.freeze([
  Object.freeze({
    target: "TENANT_ENVAL",
    root: "supabase/migrations",
    candidateFilenamePattern: "^[0-9]{14}_[a-z0-9][a-z0-9_]*\\.sql$",
  }),
  Object.freeze({
    target: "CONTROL_PLANE",
    root: "platform/control-plane/supabase/migrations",
    candidateFilenamePattern: "^[0-9]{14}_[a-z0-9][a-z0-9_]*\\.sql$",
  }),
]);

export const VERIFY_MANIFEST = Object.freeze({
  modes: MODES,
  commands: COMMANDS,
  globalChecks: GLOBAL_CHECKS,
  pathRules: PATH_RULES,
  migrationBaseline: MIGRATION_BASELINE,
  migrationInventories: MIGRATION_INVENTORIES,
  tenantMigrationChain: TENANT_ENVAL_MIGRATION_CHAIN,
});
