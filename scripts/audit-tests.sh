...
intake_audit_assert_idem_reason() {
  ...
  if ! echo "$aud" | grep -q "\"idempotency_key\":\"$idem\""; then
    ...
    FAILED_CASES+=("$label (intake audit missing idem)")
    return 1
  fi
  ...
  if ! echo "$aud" | grep -q "\"reason\":\"$expected_reason\""; then
    ...
    FAILED_CASES+=("$label (intake audit missing reason)")
    return 1
  fi
...
assert_no_lead_for_email_since_start() {
  ...
  if echo "$res" | grep -q "\"id\""; then
    ...
    FAILED_CASES+=("$label (lead exists)")
    return 1
  fi
...
assert_no_dossier_for_lead_email_since_start() {
  ...
  if echo "$res" | grep -q "\"id\""; then
    ...
    FAILED_CASES+=("$label (dossier exists)")
    return 1
  fi
...
if [[ "$HTTP_IN_NL" != "400" ]]; then
  ...
  echo "$BODY_IN_NL"
  FAIL=$((FAIL+1))
  FAILED_CASES+=("INTAKE in_nl=false (HTTP $HTTP_IN_NL != 400)")
else
  PASS=$((PASS+1))
  intake_audit_assert_idem_reason "$IDEM_IN_NL" "in_nl_false" "INTAKE in_nl=false audit" || FAIL=$((FAIL+1))
  assert_no_lead_for_email_since_start "$EMAIL_IN_NL" "INTAKE in_nl=false no lead" || FAIL=$((FAIL+1))
  assert_no_dossier_for_lead_email_since_start "$EMAIL_IN_NL" "INTAKE in_nl=false no dossier" || FAIL=$((FAIL+1))
fi

# Idempotency replay must be identical + no duplicate audit rows
...
if [[ "$BODY_IN_NL_REPLAY" != "$BODY_IN_NL" ]]; then
  ...
  FAIL=$((FAIL+1))
  FAILED_CASES+=("INTAKE in_nl=false replay (body mismatch)")
else
  PASS=$((PASS+1))
fi
...
if [[ "$HTTP_HAS_MID" != "400" ]]; then
  ...
  echo "$BODY_HAS_MID"
  FAIL=$((FAIL+1))
  FAILED_CASES+=("INTAKE has_mid=false (HTTP $HTTP_HAS_MID != 400)")
else
  PASS=$((PASS+1))
  intake_audit_assert_idem_reason "$IDEM_HAS_MID" "has_mid_false" "INTAKE has_mid=false audit" || FAIL=$((FAIL+1))
  assert_no_lead_for_email_since_start "$EMAIL_HAS_MID" "INTAKE has_mid=false no lead" || FAIL=$((FAIL+1))
  assert_no_dossier_for_lead_email_since_start "$EMAIL_HAS_MID" "INTAKE has_mid=false no dossier" || FAIL=$((FAIL+1))
fi
...
if [[ "$BODY_HAS_MID_REPLAY" != "$BODY_HAS_MID" ]]; then
  ...
  FAIL=$((FAIL+1))
  FAILED_CASES+=("INTAKE has_mid=false replay (body mismatch)")
else
  PASS=$((PASS+1))
fi
