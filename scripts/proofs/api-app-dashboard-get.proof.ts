...
function requireConfig() {
  ...
  return {
    supabaseUrl,
    functionBaseUrl: assertLocalUrl(
      Deno.env.get("FUNCTION_BASE_URL")?.trim() ||
...
function uuidArray(values: Set<string>): string {
  const items = [...values];
  for (const value of items) {
    assert(
      /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i
        .test(value),
      "fixture_uuid_invalid",
    );
  }
  return items.length
    ? `array[${items.map((value) => `'${value}'::uuid`).join(",")}]`
    : "array[]::uuid[]";
}

function sqlText(value: string): string {
  assert(!/[\0\r\n]/.test(value), "fixture_sql_text_invalid");
  return `'${value.replaceAll("'", "''")}'`;
}

async function localAdminSql(sql: string): Promise<void> {
  ...
  if (!result.success) throw new Error("local_fixture_sql_failed");
}

async function insertAccessGrant(
  ctx: ProofContext,
  grant: {
    authUserId: string;
    customerId: string;
    grantedCaseId: string | null;
    accessBasis: string;
    sourceClass: string;
    sourceRef: string;
  },
): Promise<void> {
  const grantedCase = grant.grantedCaseId
    ? `${sqlText(grant.grantedCaseId)}::uuid`
    : "null";
  await localAdminSql(`
    insert into public.app_customer_access_grants (
      ...
      ${sqlText(grant.authUserId)}::uuid,
      ${sqlText(grant.customerId)}::uuid,
      ${grantedCase},
      ${sqlText(grant.accessBasis)},
      ${sqlText(grant.sourceClass)},
      ${sqlText(grant.sourceRef)},
      ${sqlText(`dashboard-proof-${crypto.randomUUID()}`)}
    );
  `);
}
...
async function request(
  ...
  const res = await fetch(`${ctx.functionBaseUrl}/${path}`, {
    ...init,
    headers,
  });
...
function expectCode(res: HttpResult, status: number, code: string): void {
  assert(
    res.status === status,
    `expected status ${status}, got ${res.status}, code ${
      String(res.body?.code)
    }`,
  );
  assert(
    res.body?.ok === false,
    `expected ok false, code ${String(res.body?.code)}`,
  );
  assert(
    res.body?.code === code,
    `expected code ${code}, got ${String(res.body?.code)}`,
  );
}
...
function expectDashboardSuccess(res: HttpResult): void {
  assert(
    res.status === 200,
    `expected 200, got ${res.status}, code ${String(res.body?.code)}`,
  );
  assert(
    res.body?.ok === true,
    `expected ok true, code ${String(res.body?.code)}`,
  );
  ...
  assert(
    typeof res.body?.selected_dossier?.dossier_id === "string",
    "expected selected_dossier",
  );
  ...
  assert(
    Array.isArray(res.body?.document_slots),
    "expected document_slots array",
  );
  assert(
    Array.isArray(res.body?.legal_acceptances),
    "expected legal_acceptances array",
  );
}

function expectApplicationIndexSuccess(res: HttpResult): void {
  assert(
    res.status === 200,
    `expected 200, got ${res.status}, code ${String(res.body?.code)}`,
  );
  ...
  assert(
    res.body?.mode === "dashboard_application_index_v1",
    "expected dashboard_application_index_v1",
  );
  assert(typeof res.body?.request_id === "string", "expected request_id");
  assert(Array.isArray(res.body?.applications), "expected applications array");
}
...
async function createCustomerIdentity(
  ctx: ProofContext,
  email: string,
  accountType: AccountType,
) {
  ...
  if (identity.error || !identity.data?.id) {
    throw new Error("identity_insert_failed");
  }
...
async function createDossierGraph(
  ...
  const dossier = await ctx.service.from("app_customer_dossiers").insert([{
    customer_id: customerId,
    dossier_number: `${accountType.toUpperCase()}-${
      crypto.randomUUID().slice(0, 8)
    }`,
    account_type: accountType,
    status: "submitted",
  }]).select("id").single();
  if (dossier.error || !dossier.data?.id) {
    throw new Error("dossier_insert_failed");
  }
  ...
  ctx.tracker.caseIds.add(String(appCase.data.id));
  ...
  for (
    ...
    for (
      let chargerIndex = 0;
      chargerIndex < chargersPerLocation;
      chargerIndex += 1
    ) {
      const charger = await ctx.service.from("app_dossier_chargers").insert([{
        dossier_id: dossierId,
        location_id: locationId,
        client_charger_id: `${accountType}-charger-${locationIndex + 1}-${
          chargerIndex + 1
        }`,
        status: "submitted",
        brand_label: "Alfen",
        model_label: "Eve Single Pro Line",
        serial_number: `SER-${accountType}-${locationIndex + 1}-${
          chargerIndex + 1
        }`,
        mid_number: `MID-${accountType}-${locationIndex + 1}-${
          chargerIndex + 1
        }`,
      ...
      if (charger.error || !charger.data?.id) {
        throw new Error(
          "charger_insert_failed",
        );
      }
      ...
      const slot = await ctx.service.from("app_dossier_document_slots").insert([
        {
          ...
          client_slot_id: `${accountType}-invoice-${locationIndex + 1}-${
            chargerIndex + 1
          }`,
  ...
  if (withCurrentVersion && firstSlotId) {
    ...
    const version = await ctx.service.from("app_dossier_document_versions")
      .insert([{
        dossier_id: dossierId,
        document_slot_id: firstSlotId,
    ...
    const updatedSlot = await ctx.service.from("app_dossier_document_slots")
      .update({
        status: "uploaded",
    ...
    if (updatedSlot.error) {
      throw new Error("slot_current_version_update_failed");
    }
  }

  return { id: dossierId, caseId: String(appCase.data.id), accountType };
}

async function createBoundFixture(ctx: ProofContext): Promise<BoundFixture> {
  const auth = await createAuthUser(ctx);
  const { customerId, identityId } = await createCustomerIdentity(
    ctx,
    auth.email,
    "particulier",
  );
  const particulier = await createDossierGraph(
    ctx,
    customerId,
    "particulier",
    1,
  ...
  const business = await createCustomerIdentity(ctx, auth.email, "zakelijk");
  const zakelijk = await createDossierGraph(
    ctx,
    business.customerId,
    "zakelijk",
    2,
  ...
  const association = await createCustomerIdentity(ctx, auth.email, "vve");
  const vve = await createDossierGraph(
    ctx,
  ...
  const accessGrant = await ctx.service.rpc(
    "app_sync_auth_customer_access_v1",
    {
  ...
  await insertAccessGrant(ctx, {
    authUserId: auth.userId,
    customerId: business.customerId,
    grantedCaseId: null,
    accessBasis: "bound_customer_identity",
    sourceClass: "app_customer_identity",
    sourceRef: business.identityId,
  });
  await insertAccessGrant(ctx, {
    authUserId: auth.userId,
    ...
    accessBasis: "bound_customer_identity",
    sourceClass: "app_customer_identity",
    sourceRef: association.identityId,
  });
...
async function countRows(ctx: ProofContext, table: string): Promise<number> {
  const result = await ctx.service.from(table).select("id", {
    count: "exact",
    head: true,
  });
...
async function cleanup(ctx: ProofContext): Promise<void> {
  await localAdminSql(`
    ...
    update public.app_dossier_document_slots
       set current_version_id = null, current_version_number = null
     where dossier_id = any(${uuidArray(ctx.tracker.dossierIds)});
    delete from public.app_dossier_document_versions
     where dossier_id = any(${uuidArray(ctx.tracker.dossierIds)});
    delete from public.app_dossier_document_files
     where dossier_id = any(${uuidArray(ctx.tracker.dossierIds)});
    delete from public.app_customer_access_grants
     where auth_user_id = any(${uuidArray(ctx.tracker.authUserIds)})
        or customer_id = any(${uuidArray(ctx.tracker.customerIds)});
    delete from public.app_cases
     where id = any(${uuidArray(ctx.tracker.caseIds)});
    delete from public.app_customer_dossiers
     where id = any(${uuidArray(ctx.tracker.dossierIds)});
    delete from public.app_customer_identities
     where id = any(${uuidArray(ctx.tracker.identityIds)});
    delete from public.app_customers
     where id = any(${uuidArray(ctx.tracker.customerIds)});
    commit;
  `);
  for (const userId of ctx.tracker.authUserIds) {
    const deleted = await ctx.service.auth.admin.deleteUser(userId);
    if (deleted.error) throw new Error("auth_fixture_cleanup_failed");
  }
}
...
try {
  ...
  const scopedCustomer = await createCustomerIdentity(
    ctx,
    `${proofId()}@example.test`,
    "zakelijk",
  );
  const scopedVisible = await createDossierGraph(
    ctx,
    scopedCustomer.customerId,
    "zakelijk",
    1,
    0,
    false,
  );
  const scopedHidden = await createDossierGraph(
    ctx,
    scopedCustomer.customerId,
    "zakelijk",
    1,
  ...
  await insertAccessGrant(ctx, {
    ...
    customerId: scopedCustomer.customerId,
    grantedCaseId: scopedVisible.caseId,
    accessBasis: "signed_case_contact",
    sourceClass: "app_signup_promotion",
    sourceRef: scopedVisible.caseId,
  });
  ...
  await runCase(ctx, "R4_additional_body_fields_reject", async () => {
    const res = await postDashboard(ctx, fixture.token, {
      dossier_id: particulier.id,
      extra: true,
    });
  ...
  await runCase(ctx, "R7_invalid_authorization_rejects_safely", async () => {
    const res = await postDashboard(ctx, "invalid-token", {
      dossier_id: particulier.id,
    });
  ...
  await runCase(ctx, "R8_unbound_auth_identity_rejects", async () => {
    const auth = await createAuthUser(ctx);
    const res = await postDashboard(ctx, auth.token, {
      dossier_id: particulier.id,
    });
  ...
  await runCase(ctx, "R10_cross_customer_dossier_same_safe_404", async () => {
    ...
    const otherCustomer = await createCustomerIdentity(
      ctx,
      otherAuth.email,
      "particulier",
    );
    const otherDossier = await createDossierGraph(
      ctx,
  ...
  await runCase(ctx, "R11_particulier_dossier_succeeds", async () => {
    ...
    assert(
      res.body.selected_dossier.account_type === "particulier",
      "wrong_account_type",
    );
  });
  ...
  await runCase(ctx, "R12_zakelijk_multiple_locations_succeeds", async () => {
    const res = await postDashboard(ctx, fixture.token, {
      dossier_id: zakelijk.id,
    });
    expectDashboardSuccess(res);
    assert(
      res.body.selected_dossier.account_type === "zakelijk",
      "wrong_account_type",
    );
  ...
  await runCase(ctx, "R13_vve_multiple_locations_succeeds", async () => {
    ...
    expectDashboardSuccess(res);
    assert(
      res.body.selected_dossier.account_type === "vve",
      "wrong_account_type",
    );
    assert(res.body.locations.length === 2, "expected_two_locations");
  });
  ...
  await runCase(ctx, "R14A_application_index_exact_R7_and_labels", async () => {
    const res = await postDashboard(ctx, fixture.token, {
      mode: "applications",
    });
    expectApplicationIndexSuccess(res);
    const references = res.body.applications.map((item: any) =>
      item.case_reference
    );
    assert(references.length === 4, "application_index_cardinality_mismatch");
    assert(
      new Set(references).size === references.length,
      "application_index_duplicate",
    );
    assert(
      references.includes(`CASE-${particulier.id}`) &&
        references.includes(`CASE-${zakelijk.id}`) &&
        references.includes(`CASE-${vve.id}`) &&
        references.includes(`CASE-${scopedVisible.id}`),
      "authorized_application_missing",
    );
    assert(
      !references.includes(`CASE-${scopedHidden.id}`),
      "case_scoped_grant_leaked_sibling_case",
    );
    const singleLocation = res.body.applications.find((item: any) =>
      item.case_reference === `CASE-${particulier.id}`
    );
    const multipleLocations = res.body.applications.find((item: any) =>
      item.case_reference === `CASE-${zakelijk.id}`
    );
    assert(
      String(singleLocation?.application_label).includes("Proofstraat 10") &&
        multipleLocations?.application_label ===
          multipleLocations?.dossier_number,
      "application_label_strategy_mismatch",
    );
  });

  await runCase(ctx, "R14B_case_scoped_hidden_detail_safe_404", async () => {
    const res = await postDashboard(ctx, fixture.token, {
      dossier_id: scopedHidden.id,
    });
    expectCode(res, 404, "dossier_not_found_or_forbidden");
  });

  await runCase(ctx, "R14C_direct_RPC_roles_denied", async () => {
    for (const authorization of [null, `Bearer ${fixture.token}`]) {
      ...
      const response = await fetch(
        `${ctx.supabaseUrl}/rest/v1/rpc/app_customer_application_index_read_v1`,
        {
          method: "POST",
          headers,
          body: JSON.stringify({ p_auth_user_id: fixture.authUserId }),
        },
      );
      assert(
        [401, 403, 404].includes(response.status),
        "direct_rpc_role_allowed",
      );
    }
  });
  ...
  await runCase(ctx, "R17_document_slots_link_correctly", async () => {
    ...
    const chargerIds = new Set(
      res.body.chargers.map((charger: any) => charger.charger_id),
    );
    assert(
      res.body.document_slots.every((slot: any) =>
        !slot.charger_id || chargerIds.has(slot.charger_id)
      ),
      "slot_charger_mismatch",
    );
  });
  ...
  await runCase(ctx, "R18_current_document_version_projection", async () => {
    ...
    expectDashboardSuccess(res);
    assert(
      res.body.document_slots.some((slot: any) =>
        slot.current_version_number === 1 && slot.current_file_name
      ),
  ...
  await runCase(
    ctx,
    "R19_empty_optional_collections_deterministic",
    async () => {
      const res = await postDashboard(ctx, fixture.token, {
        dossier_id: vve.id,
      });
      ...
      assert(
        Array.isArray(res.body.legal_acceptances),
        "legal_acceptances_not_array",
      );
    },
  );

  await runCase(ctx, "R20_response_ordering_deterministic", async () => {
    const first = await postDashboard(ctx, fixture.token, {
      dossier_id: zakelijk.id,
    });
    const second = await postDashboard(ctx, fixture.token, {
      dossier_id: zakelijk.id,
    });
    ...
    assert(
      JSON.stringify(
        first.body.locations.map((row: any) => row.location_id),
      ) === JSON.stringify(second.body.locations.map((row: any) =>
        row.location_id
      )),
      "location_order_changed",
    );
    assert(
      JSON.stringify(first.body.chargers.map((row: any) => row.charger_id)) ===
        JSON.stringify(second.body.chargers.map((row: any) => row.charger_id)),
      "charger_order_changed",
    );
  });
  ...
  await runCase(ctx, "R21_no_N_plus_one_static_query_plan", async () => {
    ...
    assert(
      !/for \([^)]*\)[\s\S]{0,200}\.from\(/.test(source),
      "query_inside_loop_marker",
    );
  });

  await runCase(
    ctx,
    "R22_success_read_zero_audit_idempotency_writes",
    async () => {
      ...
      expectDashboardSuccess(res);
      const indexRes = await postDashboard(ctx, fixture.token, {
        mode: "applications",
      });
      expectApplicationIndexSuccess(indexRes);
      ...
      assert(
        idemAfter === idemBefore,
        "success_read_idempotency_write_detected",
      );
    },
  );
  ...
  await runCase(ctx, "R25_no_storage_url_token_or_hash", async () => {
    ...
    assert(
      !/storage_path|storage_bucket|signed|token|sha256|hash/i.test(serialized),
      "storage_or_hash_marker_found",
    );
  });
  ...
  await runCase(ctx, "R26_no_raw_audit_or_idempotency_records", async () => {
    ...
    const serialized = JSON.stringify(res.body);
    assert(
      !/app_audit_events|app_idempotency_keys|event_data|idempotency/i.test(
        serialized,
      ),
      "raw_audit_or_idem_marker_found",
    );
  });
  ...
  await runCase(ctx, "R27_multiple_account_types_one_customer", async () => {
    ...
    const accountTypes = new Set(
      res.body.dossiers.map((dossier: any) => dossier.account_type),
    );
    assert(
      accountTypes.has("particulier") && accountTypes.has("zakelijk") &&
        accountTypes.has("vve"),
      "account_types_missing",
    );
  });
  ...
  await runCase(ctx, "R28_auth_bootstrap_regression_green", async () => {
    ...
    assert(
      source.includes('SB.rpc("app_bootstrap_customer_auth_v7"') &&
        (await Deno.stat("scripts/proofs/api-app-auth-bootstrap.proof.ts"))
          .isFile,
  ...
  await runCase(ctx, "R30_no_legacy_dependency_static_scan", async () => {
    ...
    assert(
      !/api-dossier|dossier_sessions|session_token|dossier_documents|dossier_audit_events|from\\(\"idempotency_keys\"\\)/
        .test(source),
      "legacy_marker_found",
    );
  });
...
console.log("APP_DASHBOARD_GET_LOCAL=PASS");
