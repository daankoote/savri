...
- Use a supported browser or computer-use tool when available. Capture or cite
  screenshots when the tool supports them.
- If required routes, states, fixtures, viewports, browser control, screenshots,
...
2. Review every supplied route and state at the supplied viewports. Where the
...
3. Check clipping, overflow, alignment, spacing, layout hierarchy, responsive
...
4. Inspect console and runtime errors when the browser mechanism exposes them.
5. Compare findings only with explicit acceptance and current project authority.
   Do not invent requirements, copy, future modules, or a redesign.
6. Return one evidence-bounded verdict with concrete findings.
